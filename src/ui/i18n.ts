...
export const STRINGS: Record<Lang, Strings> = {
  en: {
    ...
    editor: {
      ...
      uwbScheduleMms: 'an MMS cycle is laid out before the block starts — a tag–anchor pair owns a round (or every anchor of the session, with the one-to-many switch on), and inside it every fragment owns a slot, one millisecond at a time — so there is no response window left to contend for. Picking the mode also takes the slot to the draft’s own 600 RSTU (4ab draft 15-22/0381r5 Table 1.2.3.2).',
      ...
      uwbMms: 'MMS train',
      uwbMmsHint: 'the multi-millisecond packet of the 802.15.4ab draft: instead of one burst, the ranging signal is a train of short fragments sent a millisecond apart in a pair round (further apart one-to-many). Each fragment may spend a whole millisecond’s 37 nJ energy allowance (regulation) inside its own much shorter length, and X fragments combine coherently for another 10·log10(X) dB. Everything in this section is paraphrased from the TG4ab contributions — the balloted D5.0 may differ in numbering and detail.',
