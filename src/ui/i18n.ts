  ...
  en: {
    ...
    frameDetail: {
      ...
      whatIs: {
        ...
        ampTrigger: 'The AP’s ambient-power trigger: it names N uplink slots and the rules for choosing one.',
        ampAck: 'The AP’s acknowledgement of one AMP uplink slot, sent on-body power rather than a radio’s.',
        ampResp: 'A backscatter tag’s answer, sent with power harvested from the AP’s own carrier rather than a battery.',
      },
      next: {
        ...
        ampTrigger: 'A tag that chose a slot answers there; the AP acknowledges every slot.',
        ampAck: 'The round moves on to the next slot, or closes once every slot has been answered and acknowledged.',
        ampResp: 'One SIFS later the AP sends an Ack for this slot, whether or not a tag actually answered in it.',
      },
  ...
  zh: {
    ...
    frameDetail: {
      ...
      whatIs: {
        ...
        ampTrigger: 'AP 的环境能量触发帧：它命名 N 个上行时隙，以及挑选时隙的规则。',
        ampAck: 'AP 对某个 AMP 上行时隙的确认，靠自身供电发送，而不是电池供电的电台。',
        ampResp: '反向散射标签的应答，用从 AP 载波上收集到的能量发送，而不是电池。',
      },
      next: {
        ...
        ampTrigger: '选中某个时隙的标签在该时隙内应答；AP 会确认每一个时隙。',
        ampAck: '轮次进入下一个时隙，或在所有时隙都被应答并确认后结束。',
        ampResp: '一个 SIFS 之后 AP 会为这个时隙发送一个 Ack，无论该时隙里是否真的有标签应答。',
      },
      nextTitle: '接下来会发生什么',
