...
export function fmtRecord(r: TLRecord): string {
  switch (r.type) {
    ...
    case 'IFS_START': return `${r.node} ${r.kind} wait until ${fmtNs(r.untilNs)}`
    case 'IFS_END': return `${r.node} IFS complete`
    case 'BACKOFF_DRAW': return `${r.node} backoff draw ${r.value} (CW=${r.cw})`
    ...
    case 'TX_START': return `${r.node} → ${r.frame.dst} ${r.frame.kind.toUpperCase()} ${r.frame.bytes} B @${r.frame.mbps} Mbps (${fmtUs(r.frame.txTimeNs)})${r.frame.retryFlag ? ' RETRY' : ''}`
    ...
    case 'CW_CHANGE': return `${r.node} CW → ${r.cw}`
