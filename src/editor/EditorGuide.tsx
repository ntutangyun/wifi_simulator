  ...
  return (
    <div style={{ padding: '4px 12px 16px', overflowY: 'auto', fontSize: 12 }}>
      ...
      <D t="测距模式">
        ...
        <b>多毫秒测距 MMS</b>（802.15.4ab 草案）又回到双向测距，但它是成对进行的：一个轮次只含一个标签
      ...
      <p style={p}>
        这些字段只在<i>多毫秒测距 MMS</i> 测距模式下出现，它们同时描述两件事：每台设备发出的片段序列的
        形状，以及承载其控制交互与测量报告的那套控制面——控制面本身也是一个选项（见下方<b>控制面配置</b>），
        选到<i>配置 1</i> 时那套窄带电台根本不存在——模式名不再说“窄带辅助”，正是因为窄带只是这套控制面的三种形态之一。
        此处全部内容都是对 P802.15.4ab 草案提案文稿的
        转述——已进入投票的草案文本可能有所不同。该模式开启时，<b>测距方式</b>下拉框会置灰：MMS 采用单边
      ...
      <D t="控制面配置">
        草案给 MMS 写了两套控制面，这个下拉框在两者之间选。<b>配置 2</b>（窄带辅助，narrowband-assisted,
        NBA）就是上面那几个窄带字段描述的东西：POLL、RESP、REPORT 三条消息跑在一套 2.5 MHz 的窄带电台上，
        每个窗口占两个测距时隙。<b>配置 1</b>（UWB 驱动，UWB-driven, UWBD）假设设备只有一个电台，于是同样
        这三条消息改成 UWB 物理层上的 SP0（BASIC_PACKET）包，窗口缩到一个时隙，轮次也跟着短一截。草案写这
        一套，是因为「再加一个窄带收发机」对最便宜的那一类标签来说恰恰是它装不起的成本。选中配置 1 时，
        <b>窄带信道</b>与<b>先听后说</b>会置灰并清空——那里已经没有窄带电台可供它们作用，引擎在这种配置下
        也确实一个窄带信道都不抽，而校验规则会直接拒绝一份「给不存在的电台」的设置。切回配置 2 时，两项会
        写回草案的默认值（信道 3、LBT 自动）：一份空的允许列表是配置 2 不接受的，而这个控件不替你记住上一
        份列表——记住它就等于把同一件事在会话里存两份，正是<i>参数集</i>那一栏刻意避开的做法。
      </D>
      <D t="控制相位">
        只有配置 1 才有得选。草案用轮询与响应两个时隙数来描述控制相位：取 1–15 时相位里发 <b>SP0 控制帧</b>；
        两者都取 0 时相位<b>长度为零</b>——一条控制帧都不发，测距包自己的包首 SYNC+SFD 同时充当轮询与响应，
        整个轮次只剩测距相位与报告相位。省下的正是那两个控制窗口，代价是两端都得事先知道这一轮的形状
        （谁发几个片段、在哪里），因为已经没有一条消息来告诉对方了。配置 2 下本项置灰：那里的 POLL/RESP
        窗口排在窄带电台上，选零长度什么也改变不了，所以校验规则宁可拒绝，也不让它静静留在计划里。
      </D>
      <D t="RSF 带 SFD">
        让每个 RSF 片段后面都跟一个帧起始定界符（start-of-frame delimiter, SFD），于是<i>任何</i>一个片段
        都能充当整包的开头。默认只有包首那一个片段带 SYNC+SFD：它一丢，后面的片段即使全部收到也打不开这个
        包——而「把测距信号摊进好几个毫秒」本来就是为了在听不见的地方还能凑出一次测量，第一个片段恰恰是最
        可能被墙挡掉的那个。代价是每个片段都多花一段 SFD 的时间。两条限制都来自草案：它只在配置 1 下有意义
        （配置 2 的测距包根本没有包首 SYNC+SFD 可丢），而且只在 <b>N_MSR 为 32 或 64</b> 时有定义——SFD 要
        塞进片段自己那段长度里，更长的片段草案没有给出这种形态。任一条不满足时本项置灰，鼠标悬停会说明是
        哪一条。
      </D>
      <D t="非交织子轮">
        把一个轮次拆成若干<b>子轮</b>（sub-round），每台设备一个：子轮里只有它自己在发，它那一列片段连续发
        完，下一台设备的子轮才开始。默认的<i>交织</i>形态正相反——同一毫秒里每台设备各发一个片段。非交织换
        来的是一个明确的时刻「对方整个包收完了」，下面两项都建立在它之上，所以它俩在交织形态下一律置灰。
        代价直接写在轮次长度里：一个子轮就是一整列片段的长度，设备数直接乘上去，在草案默认的 600 RSTU 时隙
        下，三个响应方的一对多轮次要 <b>100 个时隙（50 ms）</b>，而交织形态是 52 个。于是本节下方那行只读
        文字的数字会明显变大，测距块装不下时红色提示也会比交织时更早出现——那不是故障，正是这个形态真实的
        代价，提示里的时隙数与那行只读文字算的是同一个轮次。
      </D>
      <D t="固定回复时间">
        草案的 <code>macMmsFixedReplyTime</code>：应答方不再等到自己子轮的开头才发，而是在<b>收完对方整个
        MMS 包</b>之后，固定这么久再发。勾选框写入草案的 600 RSTU（一个测距时隙），旁边的数字可以改成
        300…612000 RSTU（约 0.25…510 ms）之间的任意整数，按回车或移开焦点生效；越界的值不会被保存——会话
        保留上一个可用的值，字段下方用红字说明取值范围。它把回复时间从「排定的」改成「约定的」：两端各自
        从同一个常量算出同一个时刻，发起方不必再等时隙边界。三种情况下本项置灰：交织形态（没有「收完了」
        这个起点）、一对多轮次（草案把这个时间放在 One-to-one Response Compact 帧里，共用一个常量会让所有
        应答方同时开发），以及已经打开的<i>反序</i>——最后一条是本仿真器自己的自洽要求，不是草案的禁令。
      </D>
      <D t="反序轮次">
        让应答方先发自己的 MMS 包、发起方随后（发起方自进入测距阶段起偏移 600 RSTU，本引擎把这个偏移放在
        它自己的子轮内）。它换掉的是角色：<b>先发的那一方量到的是往返时间，后发的量到的是回复时间</b>，而
        一次测距要这两个时间各一个——于是「谁算得出距离」也跟着换到另一边，配合上面的<b>报告方式</b>就能看
        清这一点。同样只属于非交织形态：交织时两端在同一毫秒里各发一个片段，没有「谁先发」可以调换。
      </D>
