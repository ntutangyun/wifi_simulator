...
export const retriesQueues: Lesson = {
  ...
  module: 1,
  title: '重传、丢帧与队列',
  why: '一帧没人回答，并不等于它已经丢了：发送方再发一次就是了。但“再发一次”从来不是免费的，也不可能一直发下去。当一帧死活发不出去、被一次次重来时，排在它后面的一切都在等。这一课我们看着一条老是失败的链路，和一条越排越长的队，然后问一个问题：什么时候，把一帧扔掉才是这台设备能做的最厚道的事。',
  outcomes: [
  ...
  picture: [
    { heading: '没人回答', text: '站点（STA）听不见自己那一帧有没有到。它只知道，自己等的那个来自接入点（AP）的 ACK 一直没来。于是它把同一帧原样再发一次——这就是重传——位置仍在队列最前面，只是置上一个比特，好让接收端分得清这是重复的还是新的。' },
    { heading: '每一次尝试都比上一次贵', text: '重传不只是“把那一帧再来一遍”。发送方把这份沉默当成人多的信号，于是把 CW 翻倍、抽一个更长的退避值，结果每一次新的尝试都比上一次开始得更晚。帧本身往往也发得更慢：连着失败几次之后，无线电会降到一种更结实、也更慢的发法上，帧在空中拉得更长，也就成了下一次碰撞更大的靶子。' },
    { kind: 'watch', jump: 0, heading: '盯住一帧', text: '载入仿真，跳到第一次重传。盯住左边那台上传站点的某一帧：同一帧一次、又一次地发出去，每次尝试都比上次隔得更远——直到它不再出现。' },
    { heading: '七次之后，放手', text: '没有哪一帧可以永远重来。每一帧都记着自己已经被发过多少次，而这个数的上限叫做重传上限——这里是七。一到这个数，这一帧就被放弃，后面那一帧挪到队首。这不是规则出了问题，正是规则在起作用：一条七次都推不过去的链路，七十次也推不过去。' },
    { heading: '它身后的队伍', text: '上面的应用还在不停地交下新的帧，它们排成一列等着轮到自己，这就是队列。它们要等信道——也要等排在前面那一帧的每一次尝试。队首一帧发不动，后面一百帧都跟着卡住。一次重传真正的代价，从来不是它烧掉的那点空口时间，而是它塞给其余所有帧的那段等待。' },
    { heading: '过时的数据不如没有', text: '所以一帧身上除了计数，还有一只钟，而这只钟的上限叫做它的生存期——这里是半秒。在队列里等得比这还久，无线电里决定什么时候开口的那一部分——也就是 MAC——就把它扔掉，不再给它上空口的机会。对一个视频帧或一段语音采样来说，这么做完全正确：它本该属于的那一刻已经过去了，迟到地送达只会花掉空口时间，对谁都没有好处。' },
  ...
  numbers: [
    { kind: 'steps', heading: '一帧要走的流程，一步一步', items: [
      ...
      '队首那一帧发出去，发送方同时起一只钟：回答必须在这一帧结束后的 45 µs 之内开始——16 µs 的间隔、一个 9 µs 的时隙，再加电台报出“开始收了”所需的 20 µs。',
      '钟走完了还没动静，就有三个计数往前走：这一帧自己的尝试次数加一；这条队列的连续失败次数加一；竞争窗口则加宽成自己的两倍再加一，最多到 1023。',
      '这一帧随后被放回队首，重新排上号发送：序列号照旧，只是带上了重复标志——所以每一次尝试都是同一帧，不是新的一帧。',
    ...
    { heading: '这张表怎么读', text: '七次尝试的序列号都是同一个。速率每失败两次降一档，于是第一次尝试占 276 µs 空口时间，第七次要 704 µs。排在它后面那一帧直到 27 741 µs 才发出去，序列号是新的，重复标志清零。' },
  ...
  observe: [
    '27 689 µs 放弃那一帧时，Hidden B 同一瞬间记下丢弃与窗口复位为 15；它的下一帧序列号是 1，重复标志清零。',
  ...
  tryThis: [
    ...
    '在编辑器里把 RTS 门限设为 500 B——隐藏节点那一课给你的解法——再重新载入。看看两台上传站点的“放弃”和 AP 的队列。',
  ],
  quiz: [
    {
