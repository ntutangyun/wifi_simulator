...
export const bianchiVsSim: Lesson = {
  ...
  picture: [
    ...
    { heading: '一个会改主意的发送方', text: '把同样这五台站点挪到近处——链路在那里能把帧发快九倍——实跑却只交付了纸上说法的五分之一。规则没坏。坏的是“性能异常”那一课说过的速率控制——发送方自己决定发多快的那一套：丢帧的发送方会降速，而它分不清“碰撞”和“信号弱”。于是它在人多的时候越降越慢，让每一帧都更长、每一次相撞都更容易发生。' },
    ...
    { heading: '一次事件，三只钟', text: '论文让所有人在同一瞬间重启，真实的站点不会。碰撞的那两台各自等满自己的期限。锁上了重叠帧之一却没能读出来的邻居，欠的是那段长长的惩罚等待 EIFS。两个都没锁上的邻居，只欠短的那个 DIFS。一次事件，三种重启时刻——而那条链里根本没有描述这件事的状态。' },
  ...
  numbers: [
    { kind: 'steps', heading: '两列数字是怎么做出来的', items: [
      ...
      '每一条携带数据帧的 TX_START 记录算一次尝试；每一条重传记录（RETRY）算一次相遇——MAC 在一次尝试没等到回答时写下它。后者除以前者，就是实测的碰撞率。',
      '每发回一个确认帧，算一次成功交付。乘上一个净荷的 12,000 比特，再除以那十秒，就是实测的吞吐。',
    ...
    { kind: 'list', heading: '较小的差异，各值多少', items: [
      '时隙时钟。链会让等待中的计数器在忙周期上继续减一，而标准是把它冻住。把引擎改成继续减一，二十台站点的实测值就从 45.83% 升到 47.9% 左右——大约是它与预测值 49.59% 之间差距的一半。',
  ...
  deeper: [
    ...
    { heading: '在模型的极端角落考它', text: '给圆弧上五台站点全挂上“根本没有窗口”的篡改驱动，方程会说：每台站点在每个时隙都发送，每次尝试都碰撞，吞吐为零。实跑给出的正是如此：23,335 次尝试，其中 23,330 次碰撞，一个回答也没有，3,330 帧被放弃。只给一台挂上作弊，它就拿走 4,639 次尝试中的 4,634 次，四台守规矩的站点十秒里一共只发出五帧，而信道仍跑出 5.557 Mb/s——正是单站点的天花板：一个净荷除以一次干净交互的代价。' },
  ],
  sources: [
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: 1,
      explain: '重传上限是把碰撞率往上推而不是往下压，而前导规则恰恰让“无捕获”假设成立。冻结则让每台等待中的站点晚一个时隙。',
    },
    {
