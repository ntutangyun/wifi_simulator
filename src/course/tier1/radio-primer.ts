...
export const radioPrimer: Lesson = {
  ...
  picture: [
    ...
    { heading: '要记两个数，不是一个', text: '所以接收端要记两个数。第一个是想听的信号到达时有多响——这就是接收信号强度指示（RSSI），用 dBm 写，这把尺子上每差十格，功率就差十倍。第二个是把屋里那点始终在响的噪声——也就是噪声地板（noise floor）——减掉之后还剩多少：信号减噪声就是信噪比（SNR）；真正决定这条链路能做什么的是它，而不是 RSSI。' },
    { kind: 'watch', jump: 0, heading: '去看一眼', text: '载入仿真，跳到笔记本的第一个数据帧（data frame），然后切换变体：书桌、书房、客厅、远端墙边。笔记本和它的业务自始至终没变，变的只是它到达时有多响——而帧上的速率，就跟着一路往下走。' },
    { heading: '这一路要被拿走多少', text: '信号在路上会被两样东西吃掉：一是距离，功率朝各个方向摊开，在室内比在空旷处衰减得更快；二是两根天线（antenna）之间那条直线穿过的东西——石膏板拿走一点，砖墙拿走很多。把这两样从发射端发出的功率里减掉，剩下的就是 RSSI。' },
    { heading: '同时还有别人在说话', text: '噪声是这个房间的地板，始终都在。邻居发出的一帧则是另一回事：它在空中的这段时间里，功率会叠到那块地板上，真正能用的就成了信号高出“噪声加干扰”的那部分——也就是信干噪比（SINR）。真实的房子从来都不安静，所以这才是诚实的那个数。' },
  ],
  numbers: [
    ...
    { kind: 'steps', heading: '整条链路，一步一步算', items: [
      '从发送端设定的发射功率（transmit power）出发：笔记本是 15 dBm。',
      '减去路径损耗（path loss）：第一米 46.7 dB，此后距离每涨十倍再减 30 dB——9 m 处一共 75.3 dB。',
