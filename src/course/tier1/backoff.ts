...
export const backoff: Lesson = {
  ...
  numbers: [
    ...
    { kind: 'steps', heading: { en: 'The draw and the countdown, exactly', zh: '抽数与倒数，精确版' }, items: [
      ...
      { en: 'On the seventh straight failure the frame is given up: the station stops retrying it, and the window drops back to its floor for whatever comes next. Two counters move together to make that one sentence true — the frame’s own attempts and the queue’s run of failures — and a later lesson shows the run where they come apart.',
        zh: '连续失败到第七次，这一帧就被放弃：站点不再重传它，窗口也为接下来的事回到下限。这一句话能成立，是因为两个计数器在这里步调一致：一个是这一帧自己的尝试次数，一个是这条队列连续失败的次数；它们分家的那一刻，后面的课会拿一次仿真指给你看。' },
