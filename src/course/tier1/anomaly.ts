...
export const anomaly: Lesson = {
  ...
  picture: [
    { heading: '公平的是轮次', text: '两台站点，队列都永远排不空，轮流上空口。每一台都先熬过规定的空闲时间，再数完自己抽到的那个随机的时隙数，数到零就发。这套流程里没有任何一步去问帧有多大、要发多久。跑得久了，两台抢到空口的次数差不多相等——规则承诺的，恰恰就是这个。' },
    { heading: '可一轮的长短并不固定', text: '两台之中有一台在公寓的另一头，隔着一堵墙。它的信号到达接入点时已经很弱，用不了近端那种又快又娇气的编码，只能退到更慢、更结实的一档，每个符号装的比特更少。字节一样，帧也一样——空口时间却是好几倍。而当它的帧开始失败，它又会自己往下退一档——这就是速率控制——于是每一轮更长了。' },
    { kind: 'watch', jump: 0, heading: '把两种轮次摆在一起看', text: '载入仿真，跳到第一个数据帧。两条泳道在同一瞬间起跑。现在看两个绿色块各自伸到哪里：它们装的字节数是一样的。' },
    { heading: '被分掉的其实是时钟', text: '轮次是均匀发下去的，可每一轮花掉的都是大家共用的那一样东西：空口上的时间。慢站点那些长长的轮次把时钟填满了，于是一秒钟里装得下的轮次总数变少——本可以跑很多个短轮次的快站点，拿到的轮次就少得多。它自己的链路什么都没变，吞吐量却掉了下来。' },
  ...
  numbers: [
    ...
    { kind: 'steps', heading: '从“轮次相等”到“吞吐不等”，一步一步', items: [
      '两台站点手里永远有帧。每一台都先等空口连续安静一个 DIFS：一个 16 µs 的间隔加两个 9 µs 的时隙，合 34 µs。刚刚有一帧没能解出来的站点，等的则是更长的 EIFS。',
      '接着各自在 0 到自己的竞争窗口之间抽一个整数时隙数——窗口从 15 起步——每过一个空闲时隙就减一。两台抽的是同一个窗口，所以跑得久了，谁归零的次数都差不多。',
      ...
      '接入点回一个 14 字节的确认帧，整套流程重新来过。轮次是均匀发下去的，每一轮花掉的那些秒却不是。',
  ...
  deeper: [
    ...
    { heading: '为什么赢的是更强的前导，而不是更早的', text: '这 40 dB 是 11 m 的房长加一道砖墙拉开的。接收机在检测到前导之前什么也解不出来，而前导只有比空中其他一切高出至少 4 dB 才会被检测到。近端的前导以 40 dB 的富余越过这道门槛，于是接入点锁定了它；远端的前导根本没被检测到，只是添了一份干扰。一旦锁定，接收机把此后到达的一切都当作噪声——不过在它还在捕获阶段时，一个明显更强的前导会让它丢掉手上的接收、改去同步新来者。' },
  ...
  sources: [
    ...
    '捕获效应——接收机守住它锁定的第一个前导，只有遇到明显更强的前导才改同步——并非标准强制规定，而是真实接收机的行为，本仿真器把它显式建模了出来。',
  ],
