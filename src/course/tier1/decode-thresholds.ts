...
export const decodeThresholds: Lesson = {
  ...
  deeper: [
    { heading: '那张表里还藏着 5 dB', text: '标准的灵敏度表除了假设 10 dB 的噪声系数，还预先扣掉了 5 dB 的实现余量，用来覆盖相位噪声、信道估计误差之类。这 5 dB 留在要求之内没有减回去，因为接收机的这些损伤对干扰和对噪声一样起作用。另外，本仿真器自己的 7 dB 噪声系数比标准假设的 10 dB 好 3 dB。这 3 dB 抵掉的正是发送端留的 3 dB 速率余量——注意抵消的不是两个数字本身，而是选级的结果：本仿真器按「7 dB 噪声系数 + 3 dB 余量」挑出来的那一级，恰好等于一台按标准假设（10 dB 噪声系数）的接收机不留任何余量时会挑的那一级。这一点与带宽无关，20、40、80、160 MHz 都成立，因为两个 3 dB 都是比值，而 kTB 对两边同样放大。',
    },
