...
export const tier1Project: Lesson = {
  ...
  outcomes: [
    ...
    '给一个数据帧（data frame）、以及围绕它的那次交换，按微秒标价',
  ...
  picture: [
    ...
    { kind: 'table', heading: '题面', head: [
      ...
      ['书房笔记本', '(5, 4), 15 dBm', '饱和上传，1500 字节载荷（payload）'],
  ...
  numbers: [
    ...
    { heading: '回答有它自己的速率', text: '还是“空口时间（airtime）”那一课第 5 步的规则：确认帧用的是不超过数据帧自身参考速率的那个最高强制速率（mandatory rate）——快帧之后是 24 Mb/s，慢帧之后是 12 Mb/s。而一次碰撞付的是 ACK 超时（ACK timeout）。' },
    { kind: 'table', heading: '（c）预测：两台饱和站点', head: [
      '量', '预测',
    ], rows: [
      ['窗口、倍增、尝试', '16, 6, 7'],
      ['某个时隙（slot time）里发送的概率', 'τ = 0.1046'],
      ['撞上别人的概率', 'p = 10.46 %'],
    ...
    { kind: 'steps', heading: '你要执行的计划', items: [
      ...
      '到这一步才跑十秒。每台笔记本的等级与帧长，从它的第一个数据帧读；丢帧从重传（retry）记录读；成功次数和空口时间，从它自己的计数器读。',
  ...
  jumps: projectJumps,
  observe: [
    '跳到第一个数据帧：书房笔记本的色块是调制与编码方式（MCS）13、长 129.6 µs，客厅笔记本的是 MCS 2、长 524.0 µs。两台站点在发出的第一帧上就选定了自己的等级。',
    '从那个色块步进到它后面的 ACK：中间的间隙是 16 µs，一个短帧间间隔（SIFS），而回答本身是 28 µs。其后还跟着 34 µs 的等待——你刚刚定价的那次交换，就在屏幕上。',
    '跳到第一次退避（backoff）抽签，读一下计数器：一个整数个时隙，而且窗口已经翻过一倍了——开局的两个帧撞在了一起。这次抽签完全不问那个帧要发多久。',
  ],
  tryThis: [
