...
export const ifs: Lesson = {
  ...
  picture: [
    ...
    { heading: { en: 'The longer gap: asking for a turn', zh: '长间隙：请求一个发言机会' }, text: {
      en: 'A station with something new to send is not continuing anything, so it must wait longer, and that longer wait is the DIFS: the short gap plus two slots. Those two extra slots are what keeps it out of the way. By the time a contender has heard enough silence to be allowed to start, an answer that was going to come has already come.',
      zh: '手里攥着新东西要发的站点，并不是在续接什么，所以它必须等得更久，而这段更长的等待就是 DIFS：短间隙再加两个时隙。多出来的这两个时隙，就是让它不碍事的那点额外等待。等一个竞争者听够了安静、终于获准开口时，本该回来的那个回答早就回来了。',
    } },
