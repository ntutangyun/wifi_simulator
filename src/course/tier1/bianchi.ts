...
export const bianchi: Lesson = {
  ...
  why: {
    ...
    zh: '到目前为止，我们都是先看着碰撞发生，再回头去数。还有另一条路。只凭接入规则本身——抽一个数、倒着数完、失败之后把窗口加宽——你可以提前算出：一屋子都想说话的终端，彼此打断的频率会是多少，最终又能送出去多少。这一课我们就把这个预测做出来。',
  },
  outcomes: [
  ...
  picture: [
    { heading: { en: 'From watching to predicting', zh: '从看，到算' }, text: {
      ...
      zh: '此前的一切，都是看着它发生。这一次，我们先把它算出来。给定一群同时都想要信道的终端，光是接入规则本身，就已经钉死了它们彼此打断的频率。房间、射频、业务、距离——这些都不进入答案。纸上两个方程，数就出来了。',
    } },
    { heading: { en: 'Everybody always has something to send', zh: '每个人手里都还有话要说' }, text: {
      ...
      zh: '这个预测需要一个很强的假设：饱和。一帧刚离开，下一帧已经在等着了，于是没有哪台终端是因为“没话可说”才安静下来的。这不是一个正常的网络。它是最坏的情况——也正是值得算清楚的那一种，因为它刻画的是：当你向信道索取它的全部时，它会怎么回答。',
    } },
    { kind: 'watch', jump: 1, heading: { en: 'The events being counted', zh: '要数的就是这种事件' }, text: {
      ...
      zh: '载入仿真，跳到第一次碰撞。五台终端，都处于饱和，彼此都在听力范围内。从这里往后的每一次重叠，都是接下来那两个方程要数的事件。',
    } },
    { heading: { en: 'Two unknowns, each defined by the other', zh: '两个未知数，各自由对方定义' }, text: {
      ...
      zh: '把“某一台终端在给定的空闲时隙里开口”的概率叫作它的发送概率。把“一次尝试撞上别人的尝试”的概率叫作碰撞概率。前者取决于后者：碰撞越多，窗口越宽，于是每台终端出手越稀。后者又取决于前者：其余人出手越勤，撞上的可能就越大。两个数，谁都没法单独算出来。',
    } },
    { heading: { en: 'One pair of values fits both', zh: '只有一对数能同时成立' }, text: {
      ...
      zh: '于是你去找那一对能让两句话同时成立的数。先猜一个碰撞概率；算出在这个前提下退避规则会让终端怎么做；再问这种行为反过来会造出多大的碰撞概率。一个量随另一个量上升而下降，所以两者只在唯一一处相遇；不断把区间对半砍，就能一步步走到那里。',
    } },
  ...
  numbers: [
    { kind: 'formula', heading: { en: 'The pair of equations', zh: '那一对方程' }, text: {
      ...
      zh: 'τ 是发送概率，p 是碰撞概率，n 是终端数。W = 16 是最小的窗口，m = 6 是它之上还能翻几倍，于是窗口从 15 一路走到 1023。左边那个方程是退避规则的解，右边那个是“撞上”的定义。',
    } },
    ...
    { kind: 'formula', heading: { en: 'What a success and a pile-up cost', zh: '一次成功与一次撞车各值多少' }, text: {
      ...
      en: 'A 1500-byte frame is 2064 µs of air at the slow rate. A success adds a SIFS, the ACK and a DIFS; a collision adds the ACK timeout and a DIFS instead, because this MAC starts the retry’s wait only when the deadline expires. Every one of those constants is the engine’s own, not fitted.',
      zh: '1500 字节的帧在慢速率上占 2064 µs 空口时间。一次成功要再加一个 SIFS、一个 ACK 和一个 DIFS；一次碰撞加的则是 ACK 超时和一个 DIFS，因为本 MAC 要等期限到期才开始计重传前的等待。这些常数没有一个是拟合来的，全都取自引擎自身。',
    } },
    { heading: { en: 'The prediction against one run', zh: '预测对上一次实跑' }, text: {
      ...
      zh: '把五台终端的场景跑十秒，数一数发生了什么：5302 次尝试，其中 1370 次撞上了别人。也就是 25.84%，而预测是 27.22%——很接近，但偏差的方向每次都一样。读懂这个方向，是下一课的事。',
    } },
  ],
  deeper: [
    { heading: { en: 'Where the first equation comes from', zh: '第一个方程是怎么来的' }, text: {
      ...
      zh: '它是退避链在长期行为下的解。一台已经失败 i 次的终端，从 2^i·W 个取值里均匀抽签，每个空闲时隙走一步，于是它停留在这一阶段的时间正比于 p^i(2^i·W + 1)/2，而 τ 就是其中计数读数为零的那部分时隙的比例。求解器把 (1 − (2p)^m)/(1 − 2p) 按 Σ_{k<m}(2p)^k 这个和来算，于是 p = ½ 只是一个普通点，而不是 0/0。',
    } },
    { heading: { en: 'The bold assumption is the second equation', zh: '大胆的是第二个方程' }, text: {
      ...
      zh: 'p = 1 − (1−τ)^(n−1) 把其余 n − 1 台终端当成每个时隙独立抛掷的硬币，不论它们各自处在哪个阶段、刚刚发生过什么，用的都是同一个 τ。这就是解耦，也正是闭式解得以存在的原因。它同时也是最先松动的那个假设，而且 n 越小越糟：只有一个对手时，根本没有“人群”可供平均。',
    } },
    { heading: { en: 'The chain stops at seven', zh: '这条链在第七次停下' }, text: {
      ...
      zh: '原始的链是无限重传的。而本 MAC 给一帧七次机会，之后就把它丢掉——这就是 Wu 等人（INFOCOM 2002）的有限重传变体：用光机会的终端会从最小的窗口重新开始，而不是一直待在最大的窗口上，于是它出手略勤一些。在二十台终端时，这把预测的碰撞概率从 48.09% 抬到 49.59%。本课里的每一个数，用的都是有限链。',
    } },
    { heading: { en: 'Why the stations whisper', zh: '终端为什么要低声细语' }, text: {
      ...
      zh: '模型只有一种数据速率，而本仿真器的速率控制器不是。连续两次失败就降一档，而碰撞在它看来与信道衰落一模一样。因此这个场景把每台终端放得足够远、发得足够轻，让最慢的那一档之下再无可降，于是固定速率的假设按构造成立。等强度的接收电平还意味着两个重叠的帧谁也不会被锁定——“无捕获”这个假设，是被布置出来的，而不是被假定的。',
    } },
  ],
  sources: [
    ...
    { en: 'The finite-retry chain is Wu, Peng, Long, Cheng and Ma, INFOCOM 2002. The arc of stations, the −20 dBm transmit power, seed 7, the ten-minute MSDU lifetime and the ten-second sample are this simulator’s model choices, made so the paper’s assumptions hold.',
      zh: '有限重传链出自 Wu、Peng、Long、Cheng 与 Ma，INFOCOM 2002。终端排成的圆弧、−20 dBm 的发射功率、种子 7、十分钟的 MSDU 生存期与十秒的采样时长，都是本仿真器的模型取值，其目的正是让论文的假设成立。' },
  ],
  ...
  observe: [
    { en: 'The run opens with its worst collision: the medium has been idle since before the start, so all five stations transmit at t = 0 and the pile-up clears at 2.064 ms.', zh: '仿真以它最糟的一次碰撞开场。介质从开始之前就一直空闲，于是五台终端在 t = 0 同时发送，这堆叠加直到 2.064 ms 才结束。' },
  ...
  tryThis: [
    { en: 'Predict, then switch. Read the ten-station row, run that variant ten seconds and count: 5678 attempts, 1992 collided, 3684 answers — 4.421 Mb/s against a predicted 4.275.', zh: '先预测，再切换。从表里读出十台终端那一行，把该变体跑十秒再数：5678 次尝试，其中 1992 次碰撞，3684 个回答——4.421 Mb/s，而预测是 4.275。' },
    { en: 'Change the seed and run the five-station scene again. Seeds 7, 8 and 12345 give 25.84 %, 25.53 % and 25.71 %: the draws move, the statistic does not.', zh: '在编辑器里换一个种子，把五台终端的场景再跑一遍。种子 7、8、12345 给出 25.84%、25.53%、25.71%：抽到的数变了，统计量没变。' },
  ],
  quiz: [
    {
      q: { en: 'Move every station next to the AP so each frame flies much faster. What does the prediction say the collision probability does?', zh: '把每台终端都挪到 AP 旁边，让每一帧飞得快上八倍。预测说碰撞概率会怎样？' },
      options: [
    ...
    },
    {
      ...
      options: [
        ...
        { en: 'Because the number of stations is unknown as well', zh: '因为终端数本身也是未知的' },
      ],
      answer: 1,
      explain: { en: 'How often a station attempts depends on how often it fails, and how often it fails depends on how often the others attempt. Only the pair where both hold is consistent.', zh: '一台终端出手多勤，取决于它失败得多频繁；而它失败得多频繁，又取决于其余人出手多勤。两者同时成立的那一对数，是唯一自洽的说法。' },
    },
    {
      ...
      answer: 1,
      explain: { en: 'Without it you would have to model when each station has something to say. With it the answer depends on the access rules alone.', zh: '没有它，你就得去建模“每台终端什么时候有话要说”。有了它，答案只取决于接入规则——这既是最坏的情况，也是最干净的情况。' },
    },
  ],
}
