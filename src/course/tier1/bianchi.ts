...
export const bianchi: Lesson = {
  ...
  picture: [
    ...
    { heading: '两个未知数，各自由对方定义', text: '“某一台站点在给定的空闲时隙里开口”的概率，叫作发送概率，记作 τ。“一次尝试撞上别人的尝试”的概率，叫作碰撞概率，记作 p。前者取决于后者：碰撞越多，窗口越宽，于是每台站点出手越稀。后者又取决于前者：其余人出手越勤，撞上的可能就越大。两个数，谁都没法单独算出来。' },
    { heading: '只有一对数能同时成立', text: '于是你去找那一对能让两句话同时成立的数。先猜一个碰撞概率；算出在这个前提下退避规则会让站点怎么做；再问这种行为反过来会造出多大的碰撞概率。一个量随另一个量上升而下降，所以两者只在唯一一处相遇；不断把区间对半砍，就能一步步走到那里。' },
  ...
  numbers: [
    { kind: 'formula', heading: '本课要解的那一对方程', text: 'τ = Σ_{i<L} p^i / Σ_{i<L} p^i (W_i + 1)/2        p = 1 − (1−τ)^(n−1)', note: 'n 是站点数，L = 7 是一帧拿到的尝试次数，W_i = 2^min(i,m)·W 是第 i 次尝试时的窗口，其中 W = 16、m = 6。左边那个方程，是按“一帧过了 L 次就放弃”这套 MAC 解出的退避规则；右边那个是“撞上”的定义。' },
    ...
    { heading: '碰撞概率与什么无关', text: '帧长、数据速率、空口时间统统缺席：进入方程的只有人数和窗口。有人在发送时计数器不走，所以长帧不会给任何人多一次撞上的机会。也请留意，在慢速率上人群的代价有多小——5.17 掉到 3.86 Mb/s——而在快速率上是 31.3 掉到 24.9。' },
    { kind: 'formula', heading: '一次成功与一次撞车各值多少', text: 'T_s = 2064 + 16 + 44 + 34 = 2158 µs        T_c = 2064 + 45 + 34 = 2143 µs', note: '1500 字节的帧在慢速率上占 2064 µs 空口时间。一次成功要再加一个 SIFS、一个 ACK——这里是 44 µs：按“空口时间”那条规则，回答走的也是同一档速率，而不是 28 µs——以及一个 DIFS。一次碰撞加的则是 ACK 超时和一个 DIFS，因为重传前的等待要等期限到期才开始计。' },
    { kind: 'steps', heading: '拿一只计算器把这一对数算出来', items: [
      ...
      '最后一除。一次成功送走的净荷 E[P] = 12,000 比特，也就是 1500 字节；于是吞吐等于 P_s·P_tr·E[P] 除以平均时隙长度。',
    ] },
  ...
  sources: [
    ...
    '1500 字节的净荷在空口上是 1528 字节，按式 17-29 在 6 Mb/s 下为 2064 µs；ACK 按 §10.6 的控制响应速率发送，44 µs；介质忙时计数器冻结见 §10.23.2.4。碰撞代价 T_c 依的是本 MAC 而非论文：重传前的 DIFS 从 ACK 超时结束时开始计（§10.3.2.9）。',
  ...
  observe: [
    '仿真以它最糟的一次碰撞开场。介质从开始之前就一直空闲，于是五台站点在 t = 0 同时发送，这堆叠加直到 2.064 ms 才结束。',
    '每个数据帧都是同样的长度，2064 µs，每个回答都在一个 SIFS 之后到来。正是这种一致性，让一次成功可以用一个数来定价，而不是一个分布。',
    'AP 在一次碰撞里根本没有启动接收，而不是收到了乱码：两个前导以同样的强度到达，哪个都没被锁住。这就是“无捕获”假设的可见形态。',
  ],
  tryThis: [
