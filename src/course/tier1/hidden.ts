...
export const hidden: Lesson = {
  ...
  picture: [
    { heading: '两个房间，一条走廊', text: '接入点站在走廊上，与两端的房间各隔一堵墙，每个房间里放一台站点。两台站点够到它都毫不费力。可两台站点之间隔着两堵砖墙，传到对方天线上的能量，比无线电愿意称之为“信号”的那条线还要低。它们共用一个网络、一条信道，而对方在自己耳朵里根本不存在——这就是隐藏节点：不是安静，是压根听不见。' },
    { heading: '为什么“先听”救不了你', text: '两台站点都严格照章办事：先听，空口干净了才开口。当所有人都听得见所有人时，这就够了。在这里却不够。一台站点的长帧才发到一半，另一台什么也没听见，断定信道是空的，于是开始发送。两帧在接入点处叠在一起——它两边都听得见——于是双双报废。把退避窗口开得更大也没用：两者根本看不见彼此，谈何相让。' },
    { kind: 'watch', jump: 0, heading: '看两帧撞在一起', text: '载入仿真，跳到第一次碰撞。从那里往回走：两台站点谁的计数器都没有在对方的帧里冻结过，因为它们根本没有什么可听的。' },
    { heading: '先问一句，让对方大声回答', text: '解法是让接入点替你说话。发长帧之前，站点先发一个很小的帧，讨要一段空口，这就是 RTS。接入点回一个同样小的帧表示“可以”，这就是 CTS；由于说话的是接入点，两个房间都听得见。这个 CTS 带着一个覆盖本次交互剩余部分的 Duration，于是那台隐藏的站点装上 NAV，整段时间都安静下来。' },
    { kind: 'watch', heading: '看预约落进另一个房间', text: '切到受保护的那个变体，按播放。盯着远端站点的泳道：一个与它毫无关系的回答传了过来，它下方随即出现一条预约，一直管到这次交互结束。' },
    { heading: '现在撞的是那句“请问”', text: '两台隐藏的站点当然还可能同时开口发问，那时相撞的就是两句“请问”。可一句“请问”只有几十个字节，而一个数据帧一千多字节，于是房间损失的只是一闪，而不是一整轮。这就是那笔交易：每个长帧都要为一问一答买单，而剩下那些出岔子的事都很便宜。从多大的帧开始买单，由 RTS 门限决定。' },
  ],
  numbers: [
    ...
    { kind: 'steps', heading: '一次受保护的交互，一步一步', items: [
      '站点先把要发的那一帧加起来：载荷、24 字节帧头、4 字节校验。总数高过 RTS 门限——受保护的变体里是 500 字节——它就先问一句。',
      ...
      '数据帧与确认帧都跑在这段预约里，而预约到期的那一微秒，正是确认帧结束的那一微秒。远端站点等满一个 DIFS，从冻结时的那个数接着数。',
    ] },
