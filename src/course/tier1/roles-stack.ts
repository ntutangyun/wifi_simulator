...
export const rolesStack: Lesson = {
  ...
  outcomes: [
    ...
    '跟着一份载荷（payload）走完全程：从上层交下来，发到空口，再在对端交上去',
  ...
  observe: [
    '悬停第一跳与第二跳的数据块：载荷编号相同，先发往接入点，再由它发给手机 A。这里没有任何数据帧（data frame）是站点直发站点的。',
    '在第一个上行帧处，笔记本先发一个短的预约帧，接着是一个突发：50 份载荷跟在同一个前导码后面。电视那条泳道上一个数据帧也没有：只收不发的设备仍然要发送，但只是为了作答。跑满一秒，它的接收数值稳定在约 0.7 ms，而手机互传是 1.3 ms。',
  ],
  tryThis: [
