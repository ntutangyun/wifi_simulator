...
export const frameAnatomyBytes: Lesson = {
  ...
  module: 0,
  title: '一帧在空口上要花多少',
  why: '上一课把一帧打开、把字段都点了名。可这些都不是白来的：你的数据前面每多一个字节，就多一段别人用不了的空口时间；而在这一帧本身之前，射频还要放上更长的一段东西——前导。这一课就把字节数到微秒——前面放的是什么，帧头和校验各添了多少，以及为什么“多帧共用一个前导”是标准里最划算的一招。',
  outcomes: [
    '说出射频放在每一帧前面的那几段是什么，以及每一段换来了什么',
    '从载荷、帧头和校验，把一帧的字节数加出来',
  ...
  terms: [
    ...
    { term: 'preamble', plain: '前导：每一帧前面那段固定的已知信号——就是上面这三个字段，加上新一代在它们后面添的东西' },
    { term: 'U-SIG', plain: 'Wi-Fi 7 的射频在那三段之后再加的一个字段，说明后面那种新格式到底是什么' },
  ],
  picture: [
    { heading: '人人都读得懂的那一段', text: '接收端没法从中间开始。在帧的任何部分之前，射频先发出一小段重复的图案，也就是 L-STF，它唯一的任务就是被察觉到：有东西开始了，节拍落在这儿。接着是一段更长的已知图案，也就是 L-LTF，接收端拿它来量这个房间——信号一路过来被墙抹成了什么样——好在比特到达时把这份涂抹还原回去。' },
    { heading: '再用一个字段说清有多长', text: '这时接收端已经锁住了信号，可对后面是什么仍一无所知。接下来那个字段就是 L-SIG，它只告诉接收端两件事：后面这段用多快的速率编码、一共有多长。这就足够让一台根本解不出这一帧的射频——邻居的，或者更老的设备——知道空口什么时候会重新空出来。每一代都照样发它，一个字不改，而且用的是最慢的那档速率。这三段合起来，就是前导。' },
    { kind: 'watch', jump: 0, heading: '看看那几条', text: '载入仿真，先跳到旧笔记本的第一帧，再跳到手机的那一帧，各自展开“空中字段”。两个块都会分成“前导”和“正身”两截；比一比各自的前导占了多少。' },
    { heading: '新射频，前导照样按老规矩', text: '一帧 Wi-Fi 7 的帧本可以用更适合它自己的开头，但它没有。它的前导照样是那三段，好让楼里每一台设备都能读出它有多长；之后才加上一个新字段，也就是 U-SIG，说明后面那种新格式究竟是什么。这里的向后兼容不是客气，而是唯一能防止邻居们互相压着说话的东西。' },
    { heading: '把字节数出来', text: '帧本身很好加：帧头，加上你的载荷，再加上校验。帧头是固定长度的，而那个业务类别标记会让它长两个字节。可空口不是按字节卖的，是按整个符号卖的：一帧要向上凑成整数个符号，于是多出来的那两个字节，常常就消失在本来就要付的那点凑整里。' },
    { heading: '很多帧，共用一个前导', text: '再把那个前导和一个短帧放在一起看。一次只发一个小帧，就意味着每次都要付一遍前导。于是客户端设备——站点（STA）——可以把许多造好的帧排成一队，每一帧仍有自己的帧头和校验，跟在同一个前导后面作为一个 PPDU 发出；回来的也是一个覆盖全部的回复。而在这么长的一个突发之前，它会先用一个短的预约帧把房间要下来，等到放行帧再发。' },
  ],
  numbers: [
    ...
    { kind: 'steps', heading: '从字节到微秒，一步一步算', items: [
      ...
      '把这些字节换成比特，再加上射频放在它们前面的那个短字段和后面的尾比特：16 + 8 × 1528 + 6 个比特。',
      '除以当前速率下一个符号能装的比特数——54 Mb/s 时是 216——然后向上取整，因为没装满的符号也要整个发出去：57 个符号。',
      '先付这一代必发的那个前导，再付符号：20 µs，加上 57 × 4 µs，一共 248 µs 空口时间。',
      '更新的射频只改这两个尺寸：前导变成 44 或 48 µs，符号变成 13.6 µs，而一个符号能装的比特多得多。',
    ...
    { kind: 'table', heading: '小帧只带非带不可的东西', head: [
      ...
      ['Ack, CTS', '帧控制、持续时间、地址 1、FCS', '14 B'],
      ['RTS', '同样的内容，再加地址 2——回复得找得到它', '20 B'],
      ['BlockAck', 'BlockAck（一整串帧只换一个回答）：两个地址、一个起始序号、一张 64 位位图', '32 B'],
    ] },
    { heading: '一个小帧要花多少', text: '一个 14 B 的回复在 24 Mb/s 下占两个符号，要 28 µs 空口时间，其中 20 µs 是前导。' },
    { kind: 'formula', heading: '把它们排在一个前导后面', text: '队列里的一个位置 = 4 B 的分隔符 + 那一帧 + 补齐到 4 的倍数的填充', note: 'Wi-Fi 5 笔记本的十四帧一起走：13 × 1536 + 1534 = 21 502 B，全都跟在同一个前导后面。只有最后一个不必填充。' },
  ...
  deeper: [
    ...
    { heading: '为什么更新的前导只是代表值', text: '上面的 40、44、48 µs 是 20 MHz、单空间流下的单用户取值。真实的 VHT、HE 或 EHT 前导会随空间流数增长——每条流都要有自己的训练字段——也会随多用户帧所服务的用户数增长，多出的那 4 µs HE-SIG-B 或 EHT-SIG 正是为此付的。仿真器没有建模这种增长，每一代只给一个代表值。' },
  ...
  sources: [
    '非 HT 前导与空口时长公式见 IEEE Std 802.11-2024 §17.3.2 与 §17.4.3；L-STF、L-LTF、L-SIG 这几个名字也出自第 17 章。20 µs 的前导与 4 µs 的符号是标准里的数值，不是模型取值。',
    '40、44、48 µs 的前导与 13.6 µs 的符号，是本仿真器在 20 MHz、单流下的代表性单用户取值；真实值会随空间流数与用户数增长（第 21、27、36 章）。Wi-Fi 7 帧里跟在传统字段之后的 U-SIG 见 §36.3.12。',
  ...
  observe: [
    '对比这两条。旧笔记本那一帧是 16 µs 的图案加 4 µs 的长度字段，之后才是 57 个数据符号；手机那一帧（Wi-Fi 6）是 44 µs 的前导加一个 13.6 µs 的符号——几乎全是前导。',
  ...
  tryThis: [
    ...
    '把旧笔记本改成 Wi-Fi 5。它原本一帧一帧地发，变成许多帧共用一个前导的突发，每个突发前还有一次预约；那条泳道也从许多小块变成几个长块。',
  ],
  quiz: [
    {
    ...
    {
      q: '十四帧共用一个前导，真正省下的是什么？',
      options: [
        '省下帧头和校验：十四帧变成了一帧',
        '省下十三个前导、十三次排队，以及十三个各自的回复',
        '空口上什么也没省；省的只是发送端的活儿',
      ],
      answer: 1,
