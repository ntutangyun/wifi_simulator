 ...
 * is counted in symbols, and what it buys to send many frames behind one front.
 *
...
export const frameAnatomyBytes: Lesson = {
  ...
  outcomes: [
    ...
    { en: 'say what one front buys when fourteen frames travel together', zh: '说出十四帧一起走时，一个前脸买到了什么' },
  ],
  needs: ['frame-anatomy'],
  terms: [
    { term: 'L-STF', plain: {
  ...
  numbers: [
    ...
    { kind: 'formula', heading: { en: 'Lining them up behind one front', zh: '把它们排在一个前脸后面' }, text: {
      ...
      en: 'Fourteen of the Wi-Fi 5 laptop\'s frames go together: 13 × 1536 + 1534 = 21 502 B, all of it behind one front. Only the last one needs no padding.',
      zh: 'Wi-Fi 5 笔记本的十四帧一起走：13 × 1536 + 1534 = 21 502 B，全都跟在同一个前脸后面。只有最后一个不必填充。',
    } },
  ...
  deeper: [
    ...
    { heading: { en: 'Why the newer fronts are only representative', zh: '为什么更新的前脸只是代表值' }, text: {
  ...
  sources: [
    { en: 'The non-HT preamble and the airtime formula are IEEE Std 802.11-2024 §17.3.2 and §17.4.3; the L-STF, L-LTF and L-SIG names are Clause 17\'s own. The 20 µs front and the 4 µs symbol are the standard\'s values, not model choices.',
      zh: '非 HT 前导与空口时长公式见 IEEE Std 802.11-2024 §17.3.2 与 §17.4.3；L-STF、L-LTF、L-SIG 这几个名字也出自第 17 章。20 µs 的前脸与 4 µs 的符号是标准里的数值，不是模型取值。' },
    { en: 'The 40, 44 and 48 µs fronts and the 13.6 µs symbol are this simulator\'s representative single-user values for 20 MHz and one stream; the real ones grow with streams and users (Clause 21, 27 and 36). The U-SIG that follows the legacy fields in a Wi-Fi 7 frame is §36.3.12.',
      zh: '40、44、48 µs 的前脸与 13.6 µs 的符号，是本仿真器在 20 MHz、单流下的代表性单用户取值；真实值会随空间流数与用户数增长（第 21、27、36 章）。Wi-Fi 7 帧里跟在传统字段之后的 U-SIG 见 §36.3.12。' },
  ...
  observe: [
    {
      en: 'Compare the two bars. The old laptop\'s frame is 16 µs of pattern and 4 µs of length field before 57 data symbols; the phone\'s Wi-Fi 6 frame is a 44 µs front and one 13.6 µs symbol — nearly all front.',
      zh: '对比这两条。旧笔记本那一帧是 16 µs 的图案加 4 µs 的长度字段，之后才是 57 个数据符号；手机那一帧（Wi-Fi 6）是 44 µs 的前脸加一个 13.6 µs 的符号——几乎全是前脸。',
    },
    {
  ...
  tryThis: [
    ...
    {
      en: 'Change the old laptop to Wi-Fi 5. Its lone frames become bursts of many behind one front, each opened by a reservation; its lane goes from many small blocks to a few long ones.',
      zh: '把旧笔记本改成 Wi-Fi 5。它原本一帧一帧地发，变成许多帧共用一个前脸的突发，每个突发前还有一次预约；那条泳道也从许多小块变成几个长块。',
    },
  ],
  quiz: [
    {
    ...
    },
    {
      ...
      options: [
        { en: 'The mark is sent in the front, which is a fixed length', zh: '标记是放在前脸里发的，而前脸是固定长度' },
    ...
    {
      q: { en: 'What does sending fourteen frames behind one front actually save?', zh: '十四帧共用一个前脸，真正省下的是什么？' },
      options: [
        { en: 'The headers and the checks: the fourteen frames become one', zh: '省下帧头和校验：十四帧变成了一帧' },
        { en: 'Thirteen fronts, thirteen turns at waiting, and thirteen separate answers', zh: '省下十三个前脸、十三次排队，以及十三个各自的回复' },
