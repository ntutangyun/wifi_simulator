...
export const uwbUlTdoa: Lesson = {
  ...
  module: 14,
  title: '每个标签一次闪发',
  why: '只听的标签（被定位的那一端就叫标签），仍然要有接收机、要有自己的钟、还要跑一套解算。挂在医院工牌带上的胸牌，这三样一个都不想要；而给这栋楼布网的人，也宁愿在屏幕上直接读出每个胸牌在哪儿，而不是挨个去问。那就把这条链路再翻一次：标签说话，楼来听。',
  outcomes: [
  ...
  picture: [
    { heading: '一帧，然后什么也没有', text: '还是上一课那个房间：锚点在四角，胸牌在胸口高度。把轮次再翻一次，于是只有胸牌在发送。它每个块占一个时隙，用来发一帧闪发（blink）——一小段广播，里面没有任何时间——发完，它的射频就关到下一个块。' },
    { heading: '由别人来定位', text: '闪发帧没有任何回应。每个听到它的锚点，都在大家共用的那条时基上记下到达时刻；时隙结束时，参考锚点用其余三个时刻各减去自己的那个，再解出和上一课一样的双曲线。只有标签发送的这种做法，就是上行形态（UL-TDoA），它什么也不告诉胸牌：它根本没开接收机，也没有任何一条记录回到它那里。' },
  ...
  numbers: [
    ...
    { kind: 'steps', heading: '一次定位，一步一步', items: [
      ...
      '听到这帧闪发的每个锚点，都给它的 RMARKER 打戳。它会用自己的晶振写下一个计数值供日志显示——就是你能读到的那行 UWB_TS——但定位并不是用这个计数值算的。',
      '定位用的是锚点公共时基上的那个到达时刻：真实飞行时间，加上这台接收机的时间戳噪声，再加上这个锚点自己的校准剩余误差。有线同步已经把它的晶振除掉了。',
  ...
  tryThis: [
    '载入“1 ns 的同步误差”。空口上什么也没变——还是那些闪发、还是那些空口时间——可每一次定位都挪了位置。房间中部那个胸牌的椭圆胀到 32.1 × 16.8 cm；七十次定位的平均误差，从 3.2 cm 变成 16.7。',
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        '两者都倾向上行：只有它把位置留在运营方能读到的地方',
        '两者都倾向另一边：只听的那一轮多少个都能服务，而只听的一方数都数不出来',
        '规模倾向上行，因为那边是基础设施在干活',
      ],
      answer: 1,
      explain: '上行的理由在标签本身，而不在数量：不用接收机、不用时钟速率修正、不用解算器。四百个标签需要四个块的时隙。',
    },
  ],
}
