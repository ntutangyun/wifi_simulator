...
export const uwbFrame: Lesson = {
  ...
  outcomes: [
    ...
    '解释为什么消息占的空口时间（airtime）比引出它的那段图案还少',
  ],
  needs: ['uwb-intro'],
  terms: [
  ...
  numbers: [
    ...
    { kind: 'steps', heading: '这一帧上的时间戳是怎么取下来的', items: [
      '发送端按上表那个固定顺序把帧搭起来：64 个 SYNC 前导符号（preamble symbol）、8 个 SFD 符号（symbol）、夹在两段 512 码片间隔中间的 STS、PHR，最后才是 PSDU——整帧里只有这一段的长度由消息本身决定。',
      ...
      '它在那一刻读自己的计数器：真实到达的时刻，加上它对首径（first path）边沿估计偏掉的那一点——这条链路（link）上 1σ 是 100 ps。读数再按测距计数器的一格取整，一格是 15.650 ps，标准叫它一个 RCTU（ranging counter time unit）。',
    ...
    { kind: 'formula', heading: '为什么 30 个字节要花 37.179 µs', text: '240 个数据比特 + 48 个校验比特 + 2 个符号的尾 = 290 个符号 × 64 码片', note: '按 6.81 Mb/s 算，光是消息本身只要 35.2 µs 的空口时间。这一帧的开销大头是编码，不是载荷（payload）本身。' },
  ],
  sources: [
