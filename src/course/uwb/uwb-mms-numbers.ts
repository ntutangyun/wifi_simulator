...
export const uwbMmsNumbers: Lesson = {
  ...
  why: '一串片段要么越过了接收机的门限，要么没有，而决定这件事的，是一笔在信封背面就能算完的算术。这一课就来算它：一毫秒的能量值多少，一个片段花掉其中多少，一串片段加起来是多少——以及这份改善里，有多少真正属于那个新想法，又有多少只是因为老式发射机浪费惯了。',
  outcomes: [
    '算出一串片段加起来是多少，以及它有没有越过接收机门限',
    '说清为什么把一串片段砍掉一半，整个房间就一次测距也没有了',
    '把增益里属于“想法”的那部分，与属于发射功率的那部分分开',
  ],
  needs: ['uwb-mms'],
  terms: [
  ...
  picture: [
    ...
    { kind: 'watch', jump: 1, heading: '看那笔加法被算出来', text: '载入仿真，跳到对第一串片段的判定。那一行把整笔加法念了出来：收到几个片段、每个多响、合成增益加了多少、余下多少作为余量，以及——只有余量为正时才有的——那个时钟比值。' },
    ...
    { heading: '能用与报废之间的三个分贝', text: '把一串砍成一半，房间里别的什么都没变。每个片段和先前一样响，而且一个不落地都收到了；变小的只有那个和，正好小三个分贝，于是它落到了门限之下。三个原本每块都测距的锚点，如今一次也测不出，每一轮都以超时收场。' },
    { heading: '一把十四毫秒长的尺子', text: '这一串片段同时也是一把尺子。它的各个片段在发送方的时钟上每隔一个轮次间隔发出——在这一轮里是四个时隙、两毫秒——于是用自己的计数器量这段跨度，就是在直接比较两块晶振：这就是时钟比值，量在整串上，不是量在一帧上。也正因为如此，这里的单边交互可以不做第二次往返。' },
  ...
  numbers: [
    ...
    { kind: 'formula', heading: '把那把尺子换成数字', text: 'ratio = 实测跨度 / ((j − i) × gap)      σ_ratio = √2 · σ_ts / ((j − i) × gap)', note: '这一轮的间隔是 2 ms，所以从一串的第一个片段到第八个是 14 ms；100 ps 的时间戳在这段跨度上给出 σ_ratio = 0.0101 ppm——而这里的晶振是设定的、不是抽样的，所以有真值可以对照着查。' },
    ...
    { kind: 'steps', heading: '整笔加法，一个符号一个符号地算', items: [
      '先算 E，一毫秒的能量。上限写成“每兆赫多少平均功率”；把它乘上信道的带宽，再持续一毫秒，得到的就是一份能量——发射机爱怎么花就怎么花。',
      '再算 t，一个片段的长度。一个符号是扩频因子乘以“序列长度加上两侧的间隔”；把这个符号按参数集规定的遍数重复，再除以码片速率。',
      '再算 P，一个片段值多少。片段把整个 E 花在 t 这么短的一段里，于是 P = 10·log10(E / t)：片段越短，它就越响。',
      '再算 rx，到达时的电平。从 P 里依次扣掉：一米处的损耗、按引擎自己的路径损耗指数算出的距离项，以及射线每穿一道墙的固定收费。',
      '再算 G，合成增益。接收机把听到的 X 个片段加起来，而等量相加给出 G = 10·log10(X)——整笔加法里，只有这一项是“许多毫秒”买来的。',
      '最后算余量，并给出判定。把接收机的灵敏度 S 从这个和里减掉：余量 = rx + G − S。不小于零就算检出；小于零，这一串就丢了，什么也量不出来。',
    ] },
