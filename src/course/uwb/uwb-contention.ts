...
export const uwbContention: Lesson = {
  ...
  module: 14,
  title: '控制器不知道有谁在场',
  why: '到目前为止，每一轮测距都是一次点名：轮询帧点出每个锚点的名字和它该作答的时隙，于是从来不会发生碰撞——因为从来没有谁需要挑选。可这一套成立的前提，是控制器手里有一份“谁在场”的名单。而一部刚走进陌生仓库的手机，根本没有名单。这一课讲的是另一种模式：轮询帧谁的名字也不点，只开出一段窗口，让听见它的人自己抽一个时隙。',
  outcomes: [
    '说清一个不点名的轮询帧为什么会引起碰撞，以及大致会有多少',
    '从日志里读出一次抽取、一次碰撞，以及一台设备整轮不出声的那一次（空过）',
    '在“更宽的应答窗口”与“它换来的测距误差”之间做权衡',
  ],
  needs: ['uwb-coexist'],
  terms: [
  ...
  picture: [
    { heading: '点名要先有名单', text: 'UWB 第一阶段里的每一轮都是点名：轮询帧点出每个锚点和它的时隙，于是没有东西会碰撞——因为没有东西需要挑选。可这一切都建立在“控制器已经知道场上有谁”之上——而很多时候，它并不知道。' },
    { heading: '于是轮询帧谁也不点', text: '取而代之的是开出一串不属于任何特定设备的时隙，这就是应答窗口。轮询帧里有一张短清单（RCPS），写明这里一共有几个时隙。每一台解出这个轮询帧的设备都从中随机挑一个，在那里作答；控制器则靠听见他们，来知道场上有谁。' },
    { kind: 'watch', jump: 1, heading: '看六个锚点各抽一个', text: '载入仿真、按下播放，然后跳到第一个抽到时隙的锚点。六个锚点在同一刻、根据同一个轮询帧各抽一次，日志一口气打出六行。读一读这些时隙号，看看有没有两个撞在一起。' },
    { heading: '同一个时隙里的两路应答，双双阵亡', text: '两个锚点在同一个时隙里作答，碰上的是介质那条寻常的规则：若一路明显比另一路响，就解出较响的那一路，否则两路皆失。而在这个房间里，每个锚点离手机的距离相同、发射功率也相同，于是谁也压不过谁。每一个被共用的时隙，都要赔上两路应答。' },
    { heading: '没有谁会去通知失败者', text: '在这种模式里，测量在手机那一侧就结束了，没有任何东西回到锚点，所以应答死掉的锚点永远不会被告知。模型改为在轮次边界上把这个回路闭上：手机没有测到的锚点，下一轮回来再试一次。它能试几次，由轮询帧里的另一张清单（RCMA）说了算；次数用完，它就整整一轮不出声，这就是空过。' },
    { kind: 'watch', jump: 3, heading: '看一个锚点放弃', text: '跳到第一个用完尝试次数的锚点。连着三轮它都抽了时隙，却一次也没被听见；到第四轮，它干脆不作声了。那一轮它的泳道上什么也没有——没有抽取，没有应答，也没有失败。' },
    { heading: '抽到第几个时隙，就等多久', text: '这里有一笔容易被忽略的账。锚点是在自己抽到的那个时隙里作答的，抽到第几号时隙，手机就得等多久——而等得越久，两只晶振漂开得也越多。点名式的轮次里，这段等待由日程表钉死；而在这里它是一次掷骰子，窗口越宽，骰子的面数越多。' },
