...
export const uwbContention: Lesson = {
  ...
  numbers: [
    ...
    { text: {
      en: 'A single-sided measurement keeps a leftover of 3.0 cm for every millisecond the answer waits, so 6.0 cm a slot.',
      zh: '单边测距会留下一项剩余误差：应答每多等一毫秒，就是 3.0 cm，于是每个时隙 6.0 cm。',
    } },
    { kind: 'steps', heading: { en: 'Getting a turn without being named, step by step', zh: '没被点名也能拿到发言权，一步一步' }, items: [
      { en: 'The phone’s poll opens the round and advertises two figures taken from the session: the window holds 8 response slots, and a responder may make 3 tries.',
        zh: '手机的轮询帧开启这一轮，并通告两个取自会话的数：应答窗口里有 8 个时隙，一个应答者可以尝试 3 次。' },
      ...
      { en: 'The phone listens through every slot of the window without naming a peer, so an empty slot is simply silence: no timeout record is written for it.',
        zh: '手机逐个时隙听过去，不指名任何对端，所以空的时隙就只是安静：不会为它写下超时记录。' },
      { en: 'Two answers in one slot overlap at the phone. The medium lets the stronger through only if it leads by 6 dB; every anchor here is 3.50 m away at the same power, so both are lost. One collision record per slot, not per answer.',
        zh: '同一个时隙里的两个回答在手机处相叠。介质只有在较强的一路领先 6 dB 时才放它过去；而这里每个锚点都在 3.50 m 处、功率相同，于是两路皆失。碰撞记录一个时隙一条，不是每丢一路一条。' },
      { en: 'At the round’s end every anchor that drew a slot learns whether the phone ranged it — from the model, at the round boundary, with nothing sent over the air. Heard: budget back to three. Not heard: one try gone.',
        zh: '这一轮结束时，每个抽过时隙的锚点都会得知手机有没有测到自己——由模型在轮次边界上告知，空中不发送任何东西。被听见：预算恢复成三次；没被听见：少掉一次尝试。' },
  ...
  deeper: [
    ...
    { heading: { en: 'The error ramp, slot by slot', zh: '一格一格爬上去的误差' }, text: {
      en: 'The base run shows the leftover directly. Its per-slot RMS range error climbs 7.3, 14.9, 16.2, 23.5, 28.2, 32.0, 22.8 and 47.1 cm across the eight response slots — a ramp of roughly 6 cm a slot, with the scatter of a handful of samples in each bucket. Over the whole run that is 26.8 cm, against 14.6 at four slots and 51.4 at sixteen: 51.4 / 14.6 is three and a half.',
      zh: '基准场景把这项剩余误差直接摆了出来。八个应答时隙上的分时隙测距误差 RMS 依次是 7.3、14.9、16.2、23.5、28.2、32.0、22.8 与 47.1 cm——大约每格 6 cm 的一道斜坡，各格里样本不多，所以带着起伏。整段运行合起来是 26.8 cm，而四个时隙是 14.6 cm、十六个时隙是 51.4 cm：51.4 除以 14.6，正好三点五倍。',
    } },
