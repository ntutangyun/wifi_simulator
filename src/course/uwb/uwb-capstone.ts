...
import { J, LESSON_6G_WIDTH_MHZ, anchor, firstUwbAoa, firstUwbPoll, firstUwbPosition, firstUwbRange, hallwayHouse, node, uwbSc, uwbTag, wifi6g, type Lesson } from '../lessonKit'
...
export const uwbCapstone: Lesson = {
  ...
  module: 16,
  title: '在这套房子里把手机定位出来',
  why: '在此之前的每一课，都是先给你一个场景，再让你去读懂它。这一课给你的是一套房子和三个决定，而场景取决于你怎么决定。要求是：在一套走廊里放着路由器、房间之间隔着砖墙的住宅里，把一部手机定位到半米以内。第三个锚点摆在哪儿、会话多久量一次、锚点是一个一个作答还是一起作答，都由你来定——然后，你要用日志来为自己的选择辩护。',
  outcomes: [
    '选定一种锚点布置，并用它产生的定位结果——而不是用示意图——为它辩护',
    '用它花掉的空口时间和招来的干扰，给“量得更勤”标出价格',
    '说清“一轮问遍所有锚点”买到了什么、又让出了什么',
    '找出这套房子里那个有系统偏差的距离，并说出它的成因',
  ],
  needs: ['uwb-position', 'uwb-aoa', 'uwb-mms', 'uwb-coexist'],
  terms: [
    { term: 'brief', plain: '你要完成的任务书：做完之后必须成立的那件事' },
    { term: 'duty cycle', plain: '测距会话真正占用空口的时间占比' },
  ],
  picture: [
    { heading: '这套房子，以及对它的要求', text: '两个房间，中间夹着一条走廊，隔断全是砖。手机在右手边那个房间里。它对面的墙上已经装了两个锚点，一头一个，第三个立在走廊里。而走廊里那台路由器，正用着和测距会话相同的那段兆赫在和一台笔记本通信。你要做到的那件事，叫做任务书（brief）：把手机定位到半米以内，而且你要说得出为什么。' },
    { kind: 'watch', jump: 2, heading: '先把现状读一遍', text: '载入仿真，跳到第一次定位。先别动别的：把一个块从头读到尾，数清它产出了什么、又丢掉了什么。这一次运行，就是下面每个决定都要拿来对照的基准。' },
    { heading: '决定其一：第三个锚点摆在哪儿', text: '同一面墙上的两个锚点定不出一个点，几何形状由第三个说了算。把它放远，张角就打开了，这正是一次好定位想要的。可把它放远，它也就躲到了更多砖墙后面，而这正是一帧你必须听见的信号最不想要的。变体把它挪到了另一个房间，两个愿望于是当众较起劲来。' },
    { heading: '决定其二：多久量一次', text: '一个块就是一次测量。让块的节奏快一倍，手机的轨迹就跟得平滑一倍——代价是会话占用空口的时间也多了一倍，于是它撞上路由器的次数翻倍，被路由器打掉的帧也翻倍。单次定位的精度一点没变。翻倍的只是会话真正在发射的时间占比，也就是占空比；而为它让路的吞吐量，是别人的。' },
    { heading: '决定其三：一轮，还是三轮', text: '到目前为止，手机都是挨个去问每一个锚点。多毫秒模式让它问一次、由三个锚点一起作答：一串片段发出去，每个锚点听到的都是同一串，各自在自己的时隙里回话。一次交互顶掉三次——代价是空口时间多得多，因为这一串每隔一个间隔，就要轮次里的每台设备各发一个片段。这个变体还把测向一并去掉了：这个模式只量距离。' },
    { heading: '你应该找出来的那个单向误差', text: '这套房子里的三个距离中，有一个不只是“有噪声”，它每个块都朝同一个方向错。去找那个与手机之间的直线要穿过东西的锚点，把它报出的距离和旁边印着的真值比一比。噪声是对称的，平均之后就没了。这个不是；而把它一起平均进去的定位结果，会朝着一个方向偏出去。' },
  ],
  numbers: [
    { kind: 'table', heading: '四个场景，七个块', head: [
      '数字，以及从哪里读',
      '现状', '锚点 3 挪远',
      '每 100 ms 一个块', '一轮问遍三个',
    ], rows: [
      ['定位次数——运行里的 UWB_POSITION 记录',
        '20', '10', '36', '7'],
      ['其中三距离交汇——记录里写着三个锚点的那些',
        '5', '0', '9', '7'],
      ['超时——检视器里的 timeouts 计数，即 UWB_TIMEOUT',
        '18', '40', '36', '0'],
      ['被 Wi-Fi 打掉——它下面那行计数，即 UWB_INTERFERED',
        '6', '4', '12', '14'],
      ['发射次数——手机与三个锚点的 TX_START',
        '44', '34', '81', '294'],
      ['空口时间——就是上面那些帧的发射时长相加',
        '8.5 ms', '6.5 ms', '15.7 ms', '62.5 ms'],
    ] },
    { text: '每一列都是同一个 1300 ms 窗口；凡不是三距离交汇的定位，都是某一个锚点单独测出的方位。' },
    { kind: 'table', heading: '现状下的三个距离', head: [
      '锚点——手机测距表里的一行',
      '真值', '平均误差', '成因',
    ], rows: [
      ['anchor-1', '3.99 m', '−0.02 m', '直视路径'],
      ['anchor-2', '3.99 m', '−0.00 m', '直视路径'],
      ['anchor-3', '3.23 m', '+0.60 m', '一道砖墙：遮挡路径的额外时延，模型取值'],
    ] },
    { kind: 'formula', heading: '这个偏差对定位的影响', text: '三距离交汇的定位：平均误差 0.55 m，最差 0.57 m，GDOP 1.25', note: '离散度极小；误差几乎全是那个固定的偏移，而一个没有理由去怀疑那个距离的解算器，把这份偏长原样推了过去。好的几何条件救不了它，因为这里出错的东西根本不是随机的。' },
    { heading: '有谁达到要求了吗', text: '光那个偏移就有 0.60 m，所以没有哪个场景能把每一次定位都压在半米以内。第二个实验做到了：把这份偏移从 anchor-3 的距离里扣掉、再解一次，这个块就只差 0.02 m。' },
    { kind: 'table', heading: '好答案长什么样', head: [
      '决定', '好答案长什么样',
    ], rows: [
      ['锚点 3', '把它留在走廊里：挪到另一个房间会让定位次数减半、超时大致翻倍，而且一次三距离交汇的定位也剩不下。剩下那些测向结果误差更小，并不等于变好了。'],
      ['块的节奏', '它买到的是平滑而不是精度：定位误差没变，而空口占用、超时数和被 Wi-Fi 打掉的接收都恰好翻倍。'],
      ['一轮还是三轮', '给“一次交互对三次”标价：发射次数与空口时间都多得多，被路由器打掉的也更多。它不更省，而且它把测向去掉了。'],
      ['那个偏差', '点名 anchor-3，给出偏差的符号与大小，归因于遮挡路径而非噪声，并说清为什么取平均消不掉。'],
      ['任务要求', '直说四个场景原样都达不到半米，点出那个能达标的修正，并照抄检视器打出来的那个误差。'],
    ] },
    { kind: 'steps', heading: '这套做法，一步一步', items: [
      '先取基线：按现状载入这套房子，打开手机的检视器。它的定位一栏打出最近一次定位、真值，以及两者之间的误差；它的测距表则每个锚点一行——测得的距离、真实的距离、误差、品质字节。',
      '在动任何东西之前，先把那个单向误差找出来：一块接一块地，把每个测得的距离和旁边的真值比一比。有两个始终只差几厘米；有一个每次都偏长半米上下，而且它的品质字节也更差。',
      '把上面第一张表里的数字抄下来，然后一次只改一样：从菜单里载入一个变体，绝不要两个一起上；每载入一个，就把同样那几个数字再读一遍。',
      '比的时候一行一行地比，而不是一个场景一个场景地看：一个决定值多少，既看它让什么动了，也同样看它让什么没动；空口时间翻了倍而定位误差纹丝不动，买到的就是平滑，不是精度。',
      '拿这个排名去对任务书：有没有哪个场景，能把每一次定位都压在半米以内？然后交上来：你会选哪个决定；你否决了哪个、是什么否决了它；那个有偏的距离；还有这套房子没有建模什么。',
    ] },
  ],
  deeper: [
    { heading: '东西丢了，谁来付账', text: '一个片段的丢失只发生在某一个接收端，别处不受影响：捕获与检出都是逐接收端、逐片段判定的，所以漏掉一个片段的锚点，是唯一因此变穷的那一个。真正让三个一起受损的，是手机发出的消息——开启这一轮的那帧 Poll，或者手机自己在控制信道上的那次判忙，而后者正是窄带共存那一课的全部主题。一对多轮次该付的账是空口时间；至于“丢片段的风险大家一起担”，那不是它的账。' },
    { heading: '为什么定位结果并不比最差的那个距离好多少', text: '在三个距离上做最小二乘，只比两个未知量——地面上的两个坐标，高度是按住不动的——多出一个测量，而一个自由度不够让拟合看出哪条在说谎。所以单个有偏的距离，几乎会把答案挪动它自己那么多，而检视器打出来的东西没有一样会说破这件事：结果和数据是自洽的，只是数据本身错了。要让解算器察觉到这一点，最便宜的办法就是加第四个锚点。' },
    { heading: '品质字节一直都在', text: '日志里每一个距离都带着一个品质因子，而那个被遮挡的距离，它的品质因子比邻居差。本仿真器的解算器完全没有去读它。一个会去读它的部署，会对三个距离加权，而不是一视同仁地相信它们；这是对这套房子改善最大的那一个改动——而且它一点空口时间也不花。' },
    { heading: '这套房子没有建模的东西', text: '手机不动，家具不动，也没有人站在手机和锚点之间。在真实的住宅里，最主要的误差来源是人体：几分贝、几纳秒，而且出现与消失的时间尺度就和一个块差不多。上面所有结论讲的都是一个静态房间——把这一点写出来，也是这份报告的一部分。' },
  ],
  sources: [
    '测距块与时隙结构、双向测距交互、测距计数器，见 IEEE Std 802.15.4-2024 的 §10.32.2、§10.32.3 与 §10.29；多毫秒模式、它的一对多轮次以及它的窄带控制面，见 P802.15.4ab（4ab 草案 15-22/0381r5 Table 1.6.3.1 与 §1.1）。',
    '200 ms 的测距块与 2 ms 的测距时隙来自 FiRa 的配置文件，不是标准正文；600 RSTU 是本仿真器允许 MMS 轮次使用的最短时隙，因为两个时隙必须装得下这一轮里最长的那条窄带消息。',
    '这套房子里的模型取值，列出来方便你质疑：砖墙给遮挡路径增加的额外时延、用来衡量它的每个时间戳 100 ps 的 1σ 噪声、低于就判为被 Wi-Fi 打掉的 −12 dB 信干比门限，以及那个对每个距离一视同仁的最小二乘解算器。',
    '由两根天线之间的相位差得到到达角、以及它 ±90° 的视场，都是本仿真器的双阵元模型；标准自身对到达角的支持见 §10.32 与第 16 章。',
  ],
  scenario: () => uwbCapstoneScenario('base'),
  variants: [
    { label: '锚点 3 挪到另一个房间', scenario: () => uwbCapstoneScenario('farAnchor') },
    { label: '每 100 ms 一个块', scenario: () => uwbCapstoneScenario('fastBlock') },
    { label: '一轮问遍三个', scenario: () => uwbCapstoneScenario('oneToMany') },
  ],
  jumps: [
    J('Poll 帧开启一轮', firstUwbPoll),
    J('一个距离算出来了', firstUwbRange),
    J('手机被定位出来', firstUwbPosition),
    J('锚点测出一个方位', firstUwbAoa),
  ],
  observe: [
    '把现状下的一个块从头读到尾：三个距离、三个方位、一次三锚点定位。然后读 anchor-3 那条 UWB_RANGE，把它和旁边的真值比一比，再看看它的品质字节。',
    '在“一轮问遍三个”的变体里找一条 UWB_MMS_TRAIN，读它的应答者名单。一串片段，三个名字：省下来的东西，全在这一行里。',
  ],
  tryThis: [
    '在载入任何东西之前，先把三个决定写下来，并写明你预期每个决定会让哪个数字怎么变。然后把四个场景都跑一遍，对照上表给自己打分。在这里，预测错了比对了更有价值。',
    '把 anchor-3 的平均误差从它的每一个距离里扣掉，再手算一个块的定位。这次修正带来的收益比加第四个锚点还大——而它恰恰是会话自己量不出来的那一样东西。',
  ],
  quiz: [
    {
      q: '把锚点 3 挪到另一个房间会把张角打开。实际发生的却是什么？',
      options: [
        '张角打开了，定位也相应地变好了',
        '它躲到了第二道砖墙后面，回答再也到不了，于是一次三距离交汇的定位也剩不下',
        '解算器不接受别的房间里的锚点',
      ],
      answer: 1,
      explain: '定位次数少了一半，超时翻了一倍。剩下的是单锚点的测向结果，它误差更小，只是因为它是另一种测量，而不是更好的测量。',
    },
    {
      q: '把块的间隔减半，买到的是什么？',
      options: [
        '更准的定位，因为测量次数多了，平均起来更好',
        '每秒更多次定位，代价是空口占用翻倍；而每一次定位的精度分毫未变',
        '被 Wi-Fi 打掉的接收更少，因为每个块更短了',
      ],
      answer: 1,
      explain: '这里一个块并不会和相邻的块做平均。节奏翻倍，定位次数、空口占用和损失也一起翻倍。',
    },
    {
      q: '为什么取平均消不掉 anchor-3 的误差？',
      options: [
        '块的数量还不够多，平均不起来',
        '它每个块的符号都一样：它是加在路径上的一段时延，而不是围绕路径的噪声',
        '解算器把第三个距离丢掉了',
      ],
      answer: 1,
      explain: '取平均能杀死对称的误差。而遮挡路径只会更长、不会更短，所以平均之后它原封不动地留着。',
    },
  ],
}
