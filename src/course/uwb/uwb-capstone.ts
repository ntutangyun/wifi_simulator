...
export const uwbCapstone: Lesson = {
  ...
  outcomes: [
    ...
    '用它花掉的空口时间（airtime）和招来的干扰，给“量得更勤”标出价格',
  ...
  picture: [
    ...
    { heading: '决定其二：多久量一次', text: '一个块就是一次测量。让块的节奏快一倍，手机的轨迹就跟得平滑一倍——代价是会话占用空口的时间也多了一倍，于是它撞上路由器的次数翻倍，被路由器打掉的帧也翻倍。单次定位的精度一点没变。翻倍的只是会话真正在发射的时间占比，也就是占空比（duty cycle）；而为它让路的吞吐量，是别人的。' },
    { heading: '决定其三：一轮，还是三轮', text: '到目前为止，手机都是挨个去问每一个锚点。多毫秒（multi-millisecond, MMS）模式让它问一次、由三个锚点一起作答：一串片段（fragment）发出去，每个锚点听到的都是同一串，各自在自己的时隙里回话。一次交互顶掉三次——代价是空口时间多得多，因为这一串每隔一个间隔，就要轮次里的每台设备各发一个片段。这个变体还把测向一并去掉了：这个模式只量距离。' },
  ...
  numbers: [
    ...
    { kind: 'steps', heading: '这套做法，一步一步', items: [
      '先取基线：按现状载入这套房子，打开手机的检视器。它的定位一栏打出最近一次定位、真值，以及两者之间的误差；它的测距表则每个锚点一行——测得的距离、真实的距离、误差、品质因数（figure of merit, FoM）。',
      '在动任何东西之前，先把那个单向误差找出来：一块接一块地，把每个测得的距离和旁边的真值比一比。有两个始终只差几厘米；有一个每次都偏长半米上下，而且它的品质因数也更差。',
  ...
  deeper: [
    ...
    { heading: '品质因数一直都在', text: '日志里每一个距离都带着一个品质因数，而那个被遮挡的距离，它的品质因数比邻居差。本仿真器的解算器完全没有去读它。一个会去读它的部署，会对三个距离加权，而不是一视同仁地相信它们；这是对这套房子改善最大的那一个改动——而且它一点空口时间也不花。' },
  ...
  observe: [
    '把现状下的一个块从头读到尾：三个距离、三个方位、一次三锚点定位。然后读 anchor-3 那条 UWB_RANGE，把它和旁边的真值比一比，再看看它的品质因数。',
