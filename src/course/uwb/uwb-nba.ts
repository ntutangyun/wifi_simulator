 ...
 * one of the thirty-two narrowband channels that fit inside this router's 80 MHz,
 * and turns on the listen-before-talk rule the draft brings with it.
 *
...
/**
 * The coexistence lesson's room and its two Wi-Fi devices, with an MMS session on top: four
...
export const uwbNba: Lesson = {
  ...
  body: [
    { text: {
      en: 'Nothing about this narrowband radio is in IEEE Std 802.15.4-2024. It comes from P802.15.4ab, at D5.0 in Sponsor-ballot recirculation in September 2026; that draft is members-only, so this is paraphrased from two TG4ab contributions: 15-22/0381r5 (the channel plan, the listen-before-talk rule and the block-wise hop) and 15-23/0100r2 (the narrowband PHY and the channel counts). The balloted draft may differ. One layer is regulation — the −75 dBm/MHz threshold the draft takes from the ETSI EN 303 687 frame-based rules — and the rest is model: the channel-centre formula is reconstructed from the published counts and band edges, the block-wise hop uses the simulator’s own string hash where the draft specifies AES-128-CTR keyed by the session seed, and one instantaneous power reading stands in for the draft’s 9 µs assessment.',
      zh: '这部窄带电台在 IEEE Std 802.15.4-2024 里一处也找不到。它来自 P802.15.4ab：截至 2026 年 9 月仍处于 Sponsor 投票再循环阶段，版本 D5.0；该草案仅对会员开放，所以这里改写自 TG4ab 的两篇提案文稿：15-22/0381r5（信道规划、先听后发规则与按块跳变）与 15-23/0100r2（窄带 PHY 与信道数目）。已投票的草案可能与之不同。其中只有一层来自法规——草案援引 ETSI EN 303 687“基于帧的设备”规则所给的 −75 dBm/MHz 能量检测门限——其余都是模型：信道中心频率的公式是依据已公开的信道数目与频段边界反推出来的；按块跳变用的是仿真器自己的字符串散列，而草案规定的是以会话种子为密钥的 AES-128-CTR；草案要求评估至少 9 µs，这里以一次瞬时功率读数代之。',
    } },
    { heading: { en: 'Two hundred and fifty channels of 2.5 MHz', zh: '二百五十个 2.5 MHz 信道' }, text: {
      en: 'The narrowband radio has 250 channels 2.5 MHz wide: 50 in UNII-3 from 5726.25 MHz up, and 200 in UNII-5 — the 6 GHz Wi-Fi band — from 5926.25 MHz up. One 20 MHz Wi-Fi channel covers eight of them, and this router’s 80 MHz (802.11ax channel 71, 6265 to 6345 MHz) wholly contains thirty-two, numbers 186 to 217. The session’s control channel is 200, at 6301.25 MHz — one of the thirty-two. Channel 100 would be 6051.25 MHz, with 212.5 MHz of empty spectrum between its upper edge and the router’s lower one.',
      zh: '窄带电台共有 250 个信道，每个 2.5 MHz：UNII-3 里 50 个，自 5726.25 MHz 起；UNII-5——也就是 6 GHz Wi-Fi 频段——里 200 个，自 5926.25 MHz 起。一个 20 MHz 的 Wi-Fi 信道能盖住其中八个；而这台路由器的 80 MHz——802.11ax 的 71 号信道，6265 至 6345 MHz——整整包住三十二个，编号 186 到 217。本会话的控制信道是 200 号，中心 6301.25 MHz，正是那三十二个之一。若改用 100 号，中心是 6051.25 MHz，其上边沿与路由器的下边沿之间隔着 212.5 MHz 的空白频谱。',
    } },
    { kind: 'formula', heading: { en: 'The rule the band comes with', zh: '这个频段附带的规则' }, text: {
      ...
      en: 'Before each narrowband transmission the device assesses the channel for at least 9 µs and stays silent at −75 dBm/MHz or more; over 2.5 MHz that is −71.02 dBm. The three lines are arithmetic, not a measurement: an 80 MHz PPDU at 20 dBm puts 4.95 dBm into any 2.5 MHz slice of itself, and under the Wi-Fi link’s own indoor law it crosses the threshold 8.62 m from the access point. Inside that radius the check stops being about distance.',
    ...
    { heading: { en: 'A check that only asks who is talking', zh: '只问“此刻谁在发”的检测' }, text: {
      en: 'The tag stands at (4.00, 3.50, 1.00) and the router at (5.00, 4.00, 2.00): √(1² + 0.5² + 1²) = 1.50 m apart, well inside the 8.62. An 80 MHz PPDU from the router reads −48.23 dBm there, 22.8 dB over the threshold; the router’s 20 MHz control frames read −42.21 dBm; the laptop, 3.35 m away at 15 dBm, reads −63.72, still 7.3 dB over. So the check can only discover whether anyone is on the air just then — and the laptop is saturated.',
    ...
    { heading: { en: 'What one busy check costs', zh: '一次“忙”的代价' }, text: {
      en: 'A busy check is not a deferral: the draft’s discontinuation rule stops every narrowband transmission of that device for the rest of the ranging block, and with no POLL there is no cycle. At t = 0 the channel happens to be clear: POLL, RESP at 1.000 ms, eight fragments each way at 34.5 dB of margin — anchor 1 is 4.76 m off, the UWB side never in doubt — and the anchor’s REPORT at 12.000 ms gives the tag the only range of the run. Then at 13.000 ms the tag’s own report slot finds the channel busy, and block 0 is over: its other three pair rounds never start.',
      zh: '一次“忙”不是一次退避：按草案的中止规则，该设备在本测距块剩下的时间里不再发出任何窄带帧；而没有 POLL，整个周期根本不会发生。t = 0 时信道恰好空闲，POLL 发了出去，1.000 ms 处 RESP 作答，八个片段来回穿过房间，余量 34.5 dB——anchor-1 才 4.76 m 远，UWB 这一侧从来不成问题——12.000 ms 处锚点的 REPORT 让标签拿到了整段运行里唯一一次测距。接着 13.000 ms，标签自己的报告时隙读到了“忙”，第 0 个块就此结束：它剩下的三个配对轮次一个也没开始。',
    } },
    { kind: 'table', heading: { en: 'Four ways to place one control channel', zh: '同一个控制信道的四种放法' }, head: [
      ...
      { en: 'Blocks the tag skipped', zh: '标签跳过的块' }, { en: 'Tag ranges', zh: '标签测距' },
      { en: 'Fixes', zh: '定位' },
    ], rows: [
      [{ en: 'Inside the router’s 80 MHz', zh: '落在路由器的 80 MHz 之内' }, N('200 · 6301.25 MHz'), N('7 of 7'), N('1'), N('0')],
    ...
    { heading: { en: 'Hopping averages; it does not avoid', zh: '跳变是在平均，不是在躲开' }, text: {
      en: 'The allow list [100, 150, 200, 210] holds two channels outside the router’s 80 MHz and two inside — 6326.25 MHz is inside too. Block b takes list[hash(“7:b”) mod 4], giving 100, 210, 200, 150, 100, 210, 200 over the run. Blocks 0, 3 and 4 land outside and run in full; 1, 2, 5 and 6 land inside and are skipped, though block 5 gets one range out before its report slot is stopped. Three fixes instead of seven: hopping buys exactly the share of blocks the hash puts somewhere quiet.',
    ...
    { heading: { en: 'What the narrowband radio costs Wi-Fi', zh: '窄带电台让 Wi-Fi 付出什么' }, text: {
      en: 'In “Sharing 6 GHz” a ranging frame’s −14 dBm, spread over 499.2 MHz, only trips Wi-Fi’s −62 dBm energy detection within about 40 cm. This radio is different: 10 dBm inside 2.5 MHz, free space at 6301.25 MHz, and 10 − 48.44 − 20·log10 d = −62 puts the radius at 15.07 m, longer than the room. Every narrowband frame is audible to every Wi-Fi radio here: with the rule off, 44 times in 1.3 seconds one goes clear-channel busy on nothing but a 576 µs POLL or a 608 µs REPORT.',
      zh: '在《共享 6 GHz》里，一个测距帧的 −14 dBm 摊在 499.2 MHz 上，只在约 40 cm 以内才能触动 Wi-Fi 的 −62 dBm 能量检测。这部电台完全是另一种东西：10 dBm 全落在 2.5 MHz 内，按 6301.25 MHz 的自由空间传播，10 − 48.44 − 20·log10 d = −62 给出的半径是 15.07 m，比房间还长。这里每一个窄带帧，每一台 Wi-Fi 收发机都听得见；把规则关掉之后，1.3 秒里有 44 次，某台 Wi-Fi 收发机仅仅因为一帧 576 µs 的 POLL 或 608 µs 的 REPORT 就判信道忙。',
    } },
    { text: {
      en: 'Listening does not make a narrowband frame harmless, only rarer. With the rule on, six reach the air in 1.3 seconds and five Wi-Fi PPDUs fail behind them; with it off, 108 reach the air and 87 fail. About the same share either way; the laptop’s throughput drops from 407.215 Mb/s to 362.631, a loss of 10.95 %. The model’s simplification shows here: one reading at the slot start says nothing about the 576 µs that follow, so a PPDU beginning during a narrowband frame still lands on top of it.',
  ...
  observe: [
    ...
    { en: 'The one thing that worked: “uwb-1 range → anchor-1 (SS): 4.74 m (true 4.76 m, raw 7.51 m)” at 12.608 ms. One range in 1.3 seconds and no position — a fix needs three — against 29 timeouts. Anchors 1 and 2 each have a busy check too, both in block 3.',
  ...
  tryThis: [
    { en: 'Load “Outside the router’s channel”. Channel 100 is 6051.25 MHz and the coupling disappears: not one busy check, 28 ranges, and a fix in all seven blocks on four anchors — “uwb-1 position (3.99, 3.50) m, true (4.00, 3.50), error 0.01 m, GDOP 1.05, 4 anchors”. The Wi-Fi side is identical to a run with no ranging session: 407.215 Mb/s, not one failed PPDU. That gap of empty spectrum cost nothing and bought everything.',
    ...
    { en: 'Now “Hop over four channels”: blocks 0, 3 and 4 land outside and give three fixes; the other four are skipped. Then “No LBT”, which keeps channel 200 and stops yielding — 21 ranges and 5 fixes, and the interference runs the other way. Seven narrowband frames die at the tag, five REPORTs and two RESPs, the first “uwb-1 UWB frame from anchor-3 lost to Wi-Fi: SIR -10.9 dB (foreign -42.2 dBm)”: the router’s own 20 MHz control frame, 1.50 m away.',
  ...
  quiz: [
    {
      ...
      options: [
        ...
        { en: 'Listen before talk is an energy test, not a margin test: at 1.50 m anything the router sends is 22.8 dB over −71.02 dBm, and one busy check costs the whole block', zh: '先听后发测的是能量，不是余量：在 1.50 m 处，路由器发什么都高出 −71.02 dBm 22.8 dB，而一次判忙要赔上整整一个块' },
        { en: 'The narrowband receiver is below its sensitivity at that distance', zh: '在那个距离上窄带接收机低于灵敏度' },
      ],
      answer: 1,
      explain: { en: 'Both links are excellent here; what fails is a rule. The device must not transmit while it hears −71.02 dBm in its 2.5 MHz, and the saturated laptop alone puts −63.72 dBm there. Seven blocks, seven busy checks, seven skipped.', zh: '两条链路在这里都好得很。失败的是一条规则：只要在自己的 2.5 MHz 里听到 −71.02 dBm，设备就不许发；而光是那台饱和的笔记本，在那里就有 −63.72 dBm。七个块，七次判忙，七个块被跳过。' },
    },
    {
      ...
      options: [
        ...
        { en: 'All of them — hopping means a busy channel is used for only part of a block', zh: '全都活着——跳变意味着一个忙信道只在块的一部分里被用到' },
      ],
      answer: 1,
      explain: { en: 'The draft hops with AES-128-CTR keyed by the session seed; the simulator’s string hash stands in. Either way the choice is made once per block and holds for all of it: half the list outside bought three blocks of seven, and another seed would buy a different share.', zh: '草案用以会话种子为密钥的 AES-128-CTR 来跳变，仿真器以字符串散列代之；无论哪一种，选择都是每块一次，并且在整块之内不变。列表里一半在外面，买到的是七个块里的三个；换一个种子，买到的比例就不同。' },
    },
    {
