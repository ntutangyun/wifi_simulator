...
export const uwbNba: Lesson = {
  ...
  module: 15,
  title: '另一部射频负责开口说话',
  why: '测距设备做的是量时间，它并不商量事情。可总得有人说清楚：是谁在问谁，对方到底有没有听见。若把这些话交给那部宽带测距射频去说，它的空口时间就会花在字句上，而不是花在测量上。于是设备在它旁边又带了一部小得多的射频。这一课要跟着这部小射频，听它在一轮测距里究竟说了些什么。',
  outcomes: [
    '按顺序说出小射频在一轮里发的那三条消息',
  ...
  picture: [
    ...
    { kind: 'steps', heading: '一轮里小射频说的话', items: [
      ...
      '接着是那串片段，走宽带射频：整轮里唯一被真正测量的部分',
      'Report——作答的一方把自己的回复时延捎回来，手机据此把两个时间戳变成一个距离',
    ] },
    { kind: 'watch', jump: 0, heading: '听这一轮怎么开场', text: '载入仿真、按下播放，然后跳到第一条窄带消息。它出现在任何一帧测距帧之前：一轮测距总是先在小射频上谈妥，之后才在宽带射频上量出来。' },
    { heading: '为什么不干脆在宽带射频上说', text: '也不是不行。只是一帧测距帧的价值，在于两端都能把它的某一道边沿说到码片的零头；而往里塞进去的每一个字节，只会让帧更长，却半点也不会让那道边沿更锐利。窄带射频论字节更慢，可它反而更便宜——因为它的字节不是从测量里扣出来的。' },
    { heading: '从外面看一个块', text: '盯住一个测距块看，形状就很清楚了。Poll 在最开头出发，并在里面点名它要问的每一个锚点；一个时隙之后，它们各自作答；片段在这期间穿过房间；大约第二十毫秒上，第一条收尾消息到达，手机这才终于拿到一个距离。一个块里就装这么一轮，余下的大半时间是空的。' },
  ...
  numbers: [
    { kind: 'table', heading: '一轮里的三条消息', head: [
      ...
      ['这一轮容得下的应答者', '3 of 4', '—', 'Poll 每多一个应答者就长 3 字节，而两个 600 RSTU 时隙必须装得下它，于是第四个角落这一轮不参加'],
    ] },
    { kind: 'formula', heading: '576 µs 是怎么来的', text: '(10 + 2 + 2 × 字节数) 个符号 × 16 µs\n12 字节 → 36 × 16 = 576 µs\n13 字节 → 38 × 16 = 608 µs      23 字节 → 58 × 16 = 928 µs', note: '每个符号载四个比特，一个符号持续 16 µs，也就是 250 kb/s。一个字节要两个符号，而消息前面还有十二个符号的头部。于是这部射频哪怕说最短的一句话，也要占住空口半毫秒有余。' },
