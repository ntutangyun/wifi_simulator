...
export const uwbBlocks: Lesson = {
  ...
  picture: [
    { heading: '一张无需商量的时间表', text: '超宽带（UWB）的测距会话在任何人发送之前就把时间分配完毕。整张日程就是一个测距块（ranging block），只要会话还在，这个块就原样重复下去。它里面没有谁需要先听信道是否空闲、没有谁要等一段间隔、没有谁要取退避值、也没有谁要重传：每台设备在第一帧起飞之前，就已经知道自己要发的每一帧发生在哪一刻。' },
    { text: '块的内部，每部手机分到属于自己的一轮——它的回合，别人插不进来。而一轮的内部，时间被切成等长的测距时隙（ranging slot），每个时隙恰好走一帧：先是手机的提问，然后是各个锚点（anchor）的回答，每一帧都落在时间表分给它的那个位置上；这里用的测距方法是双边双向测距（DS-TWR）。' },
    ...
    { heading: '醒着，和分到手，是两回事', text: '一台设备付的是空口时间（airtime）的账，不是时间表分给它那段时间的账。它只在“预期有帧到来”的时隙起点打开接收机，并在那一帧落地的瞬间关掉。于是，服务着十轮中三轮的锚点，射频开启时长也不过占整块的百分之一上下。' },
  ...
  observe: [
    ...
    '每一次发送都压在时隙边界上，精确到纳秒。这些泳道上没有帧间间隔、没有退避取值，也没有网络分配向量（NAV）——而那七个没有主人的轮次，从头到尾都不曾开启。',
  ],
  tryThis: [
