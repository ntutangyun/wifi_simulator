...
export const uwbDstwr: Lesson = {
  ...
  picture: [
    { heading: '再多发一条消息', text: '上一课的办法是把晶振（crystal）测出来再修正。还有另一条路：让手机也答一次。等所有锚点都回过话，手机再发第三条消息——Final——每个锚点给这段间隔计时，做法和手机给应答那段计时完全一样。至此两端都既问过也答过。' },
    ...
    { heading: '代价是时隙，不是空口时间（airtime）', text: '两次往返比一次多用两类消息——手机发的 Final，和每个锚点发的 Report——所以同样四个锚点，一轮要占十个时隙，原来只要五个。翻倍的是一轮持续多久，以及每台设备要醒来多少次。' },
    { heading: '它修不好的那些', text: '这一切只拿掉了一项误差：晶振。每个接收时间戳依旧带着一点噪声，现在误差正是由它构成的——但它不再随等待时长增长，所以最后作答的锚点和最先作答的一样准。被挡住的直射路径同样没被解决：首径（first path）来得晚，这段迟到会原封不动加到距离上。' },
  ],
  numbers: [
    { kind: 'formula', heading: '双边双向测距', text: 'Tprop = (Tround1·Tround2 − Treply1·Treply2) / (Tround1 + Tround2 + Treply1 + Treply2)', note: '分子里的每个乘积都各带两只钟的一个因子，无论两段等待多久。分母根本不是什么等待，而是整场交互，两端各计一次。活下来的是被百万分之几（ppm）缩放的飞行时间——不到一皮秒。' },
    ...
    { kind: 'steps', heading: '一次测距，一步一步', items: [
      ...
      '手机给一帧 Final 的离开打戳，并把它对每个作答过的锚点所量的那次往返，连同 Treply2（它自己的等待：从 Response 到达，到 Final 发出）一起写进这一帧的测距测量信息（ranging measurement information, RMI）里——这两段只有手机量得到。',
      ...
      '这套算式把两次往返相乘、两段等待相乘，再把两个乘积相减，最后除以四段时间之和。每个乘积都各含两端各一段，所以那次除法把两块晶振一并除掉了。什么都不用估计，时钟偏差（clock offset）压根没被读过。',
    ] },
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: 1,
      explain: '占用率几乎没变，代价不在空口时间，而在时延与功耗。晶振那一项被压到皮秒量级，接收噪声却留了下来。',
    },
  ],
}
