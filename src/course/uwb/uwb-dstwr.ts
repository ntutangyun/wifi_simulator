...
export const uwbDstwr: Lesson = {
  ...
  module: 11,
  title: '两次往返，把时钟消掉',
  why: '把对方的时钟测出来确实管用，但它留下的剩余误差会随锚点多等的每一毫秒一起变大。还有一条路能把等待甩掉：多发一条消息，让两端都既问过也答过。这样一来，每只钟在算式两边出现的次数一样多，误差是被抵消掉的，而不是被估计掉的。',
  outcomes: [
  ...
  needs: ['uwb-sstwr'],
  terms: [
    { term: 'DS-TWR', plain: '双边双向测距：做两次往返而不是一次，让两只钟自己抵消' },
    { term: 'Final', plain: '手机发的第三条消息，它结束这次交互，带上只有手机量得到的那些数' },
    { term: 'Report', plain: '锚点的最后一条消息，把只有它自己量得到的两段时间告诉手机' },
    { term: 'RMI', plain: '测距测量信息：帧里专门用来写这些时间段的那一部分' },
  ],
  picture: [
    { heading: '再多发一条消息', text: '上一课的办法是把晶体测出来再修正。还有另一条路：让手机也答一次。等所有锚点都回过话，手机再发第三条消息——Final——每个锚点给这段间隔计时，做法和手机给应答那段计时完全一样。至此两端都既问过也答过。' },
    { kind: 'watch', jump: 1, heading: '找到第三条消息', text: '载入仿真，跳到 Final 帧。它从这一轮的正中间发出，一帧同时发给所有锚点。它之后的一切都只是记账。' },
    { heading: '三条消息，外加一张回执', text: '于是这次交互是三条消息，再加一张回执。手机提问；每个锚点在自己的时隙里作答；手机再用一帧 Final 为所有锚点收尾；最后每个锚点把只有自己量得到的那两段时间送回来——这就是 Report。总共四段时间，两端各量两段，没有哪一段用了别人的单位。' },
    { heading: '两次往返，都是错的', text: '把两次往返分别算出来，会发现哪一次都不是距离。第一次正是上一课那个未修正的估计，错法也一样：一道同样的斜坡。第二次是它的镜像——这回轮到手机等待，而手机是那只快钟，于是这一半算出来是负的。答案也不是两者的平均。' },
    { heading: '两只钟为什么会抵消', text: '双边双向测距（DS-TWR）既不挑其中一半，也不求平均。它把两次往返相乘，把两段等待相乘，再把两个乘积相减。每一对里都是手机量一段、锚点量一段，所以两个乘积被同一对速率拉伸了同样的倍数——而随后的那次除法正好把这份拉伸再除掉。两段等待对不对称，无关紧要。' },
    { heading: '代价是时隙，不是空口时间', text: '两次往返比一次多用两类消息——手机发的 Final，和每个锚点发的 Report——所以同样四个锚点，一轮要占十个时隙，原来只要五个。翻倍的是一轮持续多久，以及每台设备要醒来多少次。' },
    { heading: '它修不好的那些', text: '这一切只拿掉了一项误差：晶体。每个接收时间戳依旧带着一点噪声，现在误差正是由它构成的——但它不再随等待时长增长，所以最后作答的锚点和最先作答的一样准。被挡住的直射路径同样没被解决：首径来得晚，这段迟到会原封不动加到距离上。' },
  ],
  numbers: [
    { kind: 'formula', heading: '双边双向测距', text: 'Tprop = (Tround1·Tround2 − Treply1·Treply2) / (Tround1 + Tround2 + Treply1 + Treply2)', note: '分子里的每个乘积都各带两只钟的一个因子，无论两段等待多久。分母根本不是什么等待，而是整场交互，两端各计一次。活下来的是被百万分之几缩放的飞行时间——不到一皮秒。' },
    ...
    { text: '把第一行的两个半场平均一下，得到 −5.49 m。还有一件本该出现、这里却没有出现的事：最后作答的锚点等的时间是第一个的四倍，每个结果却都落在 6 cm 以内。' },
    ...
    { heading: 'Response 变短了', text: '这里的 Response 是 14 个字节，不是 20 个：它那段回复时延如今改由 Report 捎回去。' },
    { text: '在一个 20 000 µs 的轮里辐射 1 934.23 µs，占 9.67 %，而上一课那个单边轮是 9.56 %。信道占用率几乎没动，翻倍的是时延和醒来的次数。' },
    ...
    { text: '每个结果里进来三个带噪声的接收时间戳，而这一列不再是斜坡：锚点 4 等了四倍长，拿到的数值却一样。上一课抱怨的正是这一列。' },
    { kind: 'steps', heading: '一次测距，一步一步', items: [
      '时隙 0。手机用自己的计数器给 Poll 帧的离开打戳；每个锚点也给同一帧的到达打戳，各用各的计数器。',
      '锚点在自己的时隙里给 Response 的离开打戳。此刻它手里有 Treply1（它自己的等待：从 Poll 到达，到自己的 Response 发出），却只字不提——这一帧根本不带任何时间。',
      '手机给这一帧 Response 的到达打戳，再减去自己的那一对计数值：Tround1（它的往返：从 Poll 发出，到这一帧 Response 到达）。',
      '手机给一帧 Final 的离开打戳，并把它对每个作答过的锚点所量的那次往返，连同 Treply2（它自己的等待：从 Response 到达，到 Final 发出）一起写进这一帧——这两段只有手机量得到。',
      '每个锚点给 Final 的到达打戳，再一减：Tround2（它自己的往返：从 Response 发出，到这一帧 Final 到达）。此刻它手里四段时间齐了，当场就把距离算了出来。',
      '接着每个锚点发一帧 Report，把自己量的那两段送回去。至此手机也凑齐了四段，拿同样的四个计数值走同一套算式：两条泳道的结果连最后一位都一样。',
      '这套算式把两次往返相乘、两段等待相乘，再把两个乘积相减，最后除以四段时间之和。每个乘积都各含两端各一段，所以那次除法把两块晶体一并除掉了。什么都不用估计，时钟偏差压根没被读过。',
    ] },
  ...
  tryThis: [
    '载入“最差晶振，±20 ppm”：两端偏差翻倍，别的什么都不改。两个半场随即失控——第一个锚点读到 15.51 m 与 −44.47 m，原先是 9.51 与 −20.49——而四个结果的变化不到 3 mm。',
    '在帧检视器里打开 Final：一个 RMI 列出四个锚点，其后每个锚点各有一个短字段，装着手机量的第二段时间。再打开一份报告：一个 RMI，装着锚点自己量的那两段。',
  ],
  quiz: [
    {
      q: '锚点 4 作答前等四个时隙，锚点 1 只等一个。为什么两者一样准？',
      options: [
        '公式按等待时长给两次往返加权，等得久的分量更小',
        '分子里每个乘积都各带两只钟的一个因子，与等待多久无关',
        '一个时隙足够短，速率误差落在它上面已小于噪声',
      ],
      answer: 1,
      explain: '对称并非必要条件：分母是整场交互，不是某一段等待；剩下的只是被百万分之几缩放的飞行时间。',
    },
    {
      q: '锚点 1 的两个半场读作 9.51 m 与 −20.49 m，答案却是 3.51 m。这说明了什么？',
      options: [
        '有一次往返被破坏，公式把它丢掉了',
        '两个半场都不是距离：每个都把飞行时间埋在“半段等待乘以时钟误差”之下，而且符号相反',
        '答案就是两个半场的平均',
      ],
      answer: 1,
      explain: '两者的平均是 −5.49 m。每个半场都是某一方向上未经修正的估计；上一课靠测量修正的那道斜坡，这里改用相乘来抵消。',
    },
    {
      q: '多发的这条消息花了什么代价，又买不到什么？',
      options: [
        '没有可测的代价：辐射占比 9.67 % 对 9.56 %，噪声也一并没了',
        '十个时隙而不是五个，醒来次数翻倍——噪声和被遮挡的路径都活了下来',
        '四倍的空口时间，因为 Final 与报告是最大的帧',
      ],
      answer: 1,
      explain: '占用率几乎没变，代价不在空口时间，而在时延与功耗。晶体那一项被压到皮秒量级，接收噪声却留了下来。',
    },
  ],
}
