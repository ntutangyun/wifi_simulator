...
export const uwbDstwr: Lesson = {
  ...
  picture: [
    ...
    { heading: { en: 'Three messages and a receipt', zh: '三条消息，外加一张回执' }, text: {
      en: 'So the exchange is three messages and a receipt. The phone polls; each anchor answers in its own slot; the phone closes with one Final for all of them; and each anchor sends back the two intervals only it could measure — the Report. Four intervals in the end, two timed at each end, and none of them in anybody else’s units.',
      zh: '于是这次交互是三条消息，再加一张回执。手机提问；每个锚点在自己的时隙里作答；手机再用一帧 Final 为所有锚点收尾；最后每个锚点把只有自己量得到的那两段时间送回来——这就是 Report。总共四段时间，两端各量两段，没有哪一段用了别人的单位。',
    } },
    ...
    { heading: { en: 'Why the clocks cancel', zh: '两只钟为什么会抵消' }, text: {
      en: 'Double-sided two-way ranging (DS-TWR) neither picks a half nor averages them. It multiplies the two round trips together, the two waits together, and subtracts one product from the other. The phone measured one interval of each pair and the anchor the other, so both products are stretched by the same pair of rates — and the division that follows takes the stretch out again. The waiting need not be symmetric.',
      zh: '双边双向测距（DS-TWR）既不挑其中一半，也不求平均。它把两次往返相乘，把两段等待相乘，再把两个乘积相减。每一对里都是手机量一段、锚点量一段，所以两个乘积被同一对速率拉伸了同样的倍数——而随后的那次除法正好把这份拉伸再除掉。两段等待对不对称，无关紧要。',
    } },
  ...
  numbers: [
    ...
    { kind: 'steps', heading: { en: 'One range, step by step', zh: '一次测距，一步一步' }, items: [
      { en: 'Slot 0. The phone stamps its Poll leaving on its own counter; every anchor stamps it arriving on its own.',
        zh: '时隙 0。手机用自己的计数器给 Poll 帧的离开打戳；每个锚点也给同一帧的到达打戳，各用各的计数器。' },
      { en: 'In its own slot the anchor stamps its Response leaving. It now holds Treply1 (its wait, Poll in to Response out) and says nothing about it: this Response carries no time at all.',
        zh: '锚点在自己的时隙里给 Response 的离开打戳。此刻它手里有 Treply1（它自己的等待：从 Poll 到达，到自己的 Response 发出），却只字不提——这一帧根本不带任何时间。' },
      { en: 'The phone stamps that Response arriving and subtracts its own pair: Tround1 (its round trip, Poll out to Response in).',
        zh: '手机给这一帧 Response 的到达打戳，再减去自己的那一对计数值：Tround1（它的往返：从 Poll 发出，到这一帧 Response 到达）。' },
      { en: 'The phone stamps one Final leaving and writes into it, per anchor that answered, that round trip and Treply2 (its wait, Response in to Final out) — the two only it could measure.',
        zh: '手机给一帧 Final 的离开打戳，并把它对每个作答过的锚点所量的那次往返，连同 Treply2（它自己的等待：从 Response 到达，到 Final 发出）一起写进这一帧——这两段只有手机量得到。' },
      { en: 'Each anchor stamps the Final arriving and subtracts: Tround2 (its round trip, Response out to Final in). It now holds all four, and computes the range then and there.',
        zh: '每个锚点给 Final 的到达打戳，再一减：Tround2（它自己的往返：从 Response 发出，到这一帧 Final 到达）。此刻它手里四段时间齐了，当场就把距离算了出来。' },
      { en: 'Each anchor then sends a Report carrying its own two. The phone now holds all four as well and runs the same arithmetic on the same counts: both lanes agree to the last digit.',
        zh: '接着每个锚点发一帧 Report，把自己量的那两段送回去。至此手机也凑齐了四段，拿同样的四个计数值走同一套算式：两条泳道的结果连最后一位都一样。' },
      { en: 'That arithmetic multiplies the two round trips together and the two waits together, subtracts, and divides by the sum of all four. Every product carries one interval from each end, so the division takes both crystals back out. Nothing is estimated; no clock offset is read.',
        zh: '这套算式把两次往返相乘、两段等待相乘，再把两个乘积相减，最后除以四段时间之和。每个乘积都各含两端各一段，所以那次除法把两块晶体一并除掉了。什么都不用估计，时钟偏差压根没被读过。' },
    ] },
    { kind: 'table', heading: { en: 'Anchor 1, run through the steps', zh: '锚点 1，照着步骤走一遍' }, head: [
      ...
      [{ en: 'its Response leaves', zh: '它的 Response 离开' }, N('26 509 391 059')],
      [{ en: 'it reaches the phone', zh: '它到达手机' }, N('336 335 291 933')],
      [{ en: 'the Final leaves the phone', zh: 'Final 离开手机' }, N('336 846 477 093')],
      [{ en: 'it reaches anchor 1', zh: '它到达锚点 1' }, N('27 020 567 485')],
      [{ en: '= Treply1 (the anchor’s wait)', zh: '= Treply1（锚点的等待）' }, N('127 793 174')],
      [{ en: '= Tround2 (the anchor’s trip)', zh: '= Tround2（锚点的往返）' }, N('511 176 426')],
      [{ en: '= Tround1 (the phone’s trip)', zh: '= Tround1（手机的往返）' }, N('127 797 230')],
      [{ en: '= Treply2 (the phone’s wait)', zh: '= Treply2（手机的等待）' }, N('511 185 160')],
      [{ en: 'the answer, either lane', zh: '答案，两条泳道相同' }, N('749.0 · 3.51 m')],
    ] },
  ],
  deeper: [
