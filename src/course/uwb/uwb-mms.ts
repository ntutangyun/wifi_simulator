...
export const uwbMms: Lesson = {
  ...
  module: 15,
  title: '十六毫秒的能量',
  why: '把同样的标签和同样的锚点摆好，中间隔上两道砖墙。每一帧测距帧其实都到了——只是到达时比接收机能听见的还轻，于是什么也量不出来。这一课讲的就是仍然把测量做成的那个办法：不发一帧，改发一串短的、摊在好几毫秒里的片段，让对端把它们加起来。',
  outcomes: [
  ...
  picture: [
    ...
    { heading: '一个片段，再来一个', text: '于是把这个分组拆开。一个片段就是其中一毫秒的那一片，而且被剥得只剩骨头：没有要搜索的前导、没有头、没有地址、没有数据——只有用来取时间戳的那段序列，所以它的全名叫测距序列片段（RSF）。每台设备每轮到一次就发一个，标签和它的锚点交错着来，谁都用得上别人留下的空隙。' },
    ...
    { heading: '够得着不等于测得准', text: '这一串片段买来的是距离，而且只有距离。穿过砖墙的首径依旧迟到，于是每次测距都偏长，而且每轮偏得一样多——这是固定的偏移，不是噪声，附在每条距离上的品质字节会把它标成被遮挡的路径。这里没有任何东西让测量变得更准，它只是让测量得以存在。' },
  ],
  numbers: [
    ...
    { text: '慢的依旧是窄带那一侧。它那七条消息占掉整轮 7.106 ms 空口时间里的 4.480 ms，而三十二个片段加起来才 2.626 ms。' },
    ...
    { kind: 'steps', heading: '一轮，一步一步来', items: [
      '这个块先从允许列表里抽出自己的窄带信道；随后一轮在它的第一个时隙里开场：标签广播一帧 Poll，把它要问的每个锚点都点上名。',
      ...
      '一个片段就是一个短符号重复若干遍，而它把整整一毫秒的能量，花在自己短得多的那段长度里。接收机在一串里只打两个时间戳——听到的第一个片段和最后一个——各取在它的第一个脉冲上。',
      '阶段末尾，每个接收机把自己听到的片段加起来，再从这个和里减去自己的灵敏度；剩下的不为负，就判这一串检出。',
      '再往后是报告窗口，每个锚点两个：锚点发出自己掉头所用的回复时延，标签回话报出它量到的往返时间，然后由标签把这一对时间变成一个距离。',
      '而这一切都得先过三关，否则这个时隙根本跑不起来：它要是 300 RSTU 的整数倍；一个时隙要装得下最长的那个片段再加飞行余隙；两个时隙要装得下最长的那条窄带消息——也就是那帧 Poll，而它每多点一个锚点就长一截。',
    ] },
