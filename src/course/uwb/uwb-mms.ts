...
export const uwbMms: Lesson = {
  ...
  body: [
    { text: {
      en: 'Almost nothing here is IEEE Std 802.15.4-2024. The units are — RSTU, RCTU, block, round, slot — but the multi-millisecond packet and the narrowband control radio come from P802.15.4ab, at D5.0 in Sponsor-ballot recirculation in September 2026. That draft is members-only, so this is paraphrased from four TG4ab contributions: 15-22/0381r5 (the cycle), 15-23/0100r2 (fragments, narrowband PHY), 15-23/0502r3 (parameter sets), 15-22/0205r0 (the budget). The balloted draft may differ. One number is regulation — the −41.3 dBm/MHz mean EIRP averaged over a millisecond — and the rest is model: a fragment’s power, the combining rule, this room’s path loss.',
      zh: '本课几乎没有一处出自 IEEE Std 802.15.4-2024。单位是——RSTU、RCTU，以及块、轮次与时隙——但多毫秒分组和那部窄带控制电台都来自 P802.15.4ab：截至 2026 年 9 月，它仍处于 Sponsor 投票再循环阶段，版本为 D5.0。该草案文本仅对会员开放，所以这里都改写自 TG4ab 的四篇提案文稿：15-22/0381r5（测距周期）、15-23/0100r2（片段与窄带 PHY）、15-23/0502r3（参数集）以及 15-22/0205r0（能量预算）。已投票的草案可能与之不同。只有一个数字来自法规——按毫秒平均的 −41.3 dBm/MHz 平均 EIRP——其余都是模型：片段的功率、合成规则、这个房间的路径损耗。',
    } },
    { heading: { en: 'Thirty-seven nanojoules a millisecond', zh: '每毫秒三十七纳焦' }, text: {
      en: '−41.3 dBm/MHz over 499.2 MHz is −14.3 dBm, and −14.3 dBm for a millisecond is 37 nJ: an energy budget, not a power ceiling. How you spend it inside the millisecond is yours, and the simulator’s 4z transmitter does not choose — it holds −14 dBm whatever it sends, so a Poll to three anchors (36 octets, 203.782 µs) spends 8.11 nJ and throws the other 29 away.',
      zh: '−41.3 dBm/MHz 乘 499.2 MHz 是 −14.3 dBm，而 −14.3 dBm 持续一毫秒就是 37 nJ：这是能量预算，不是功率红线。这一毫秒之内怎么花由你决定，而仿真器里那台 4z 发射机并不做选择——不管发什么都保持 −14 dBm，于是一帧发往三个锚点的 Poll（36 字节、203.782 µs）只花掉 8.11 nJ，剩下的 29 nJ 扔掉了。',
    } },
    { heading: { en: 'One fragment', zh: '一个片段' }, text: {
      en: 'An MMS ranging packet is a train of fragments, one per millisecond. A fragment carries no preamble, no SFD, no PHY header and no data — only a ranging sequence: N_MSR = 40 repetitions of an MMRS symbol with a gap of 64 zeros, spread by four. 40 × 4 × (128 + 2 × 64) = 40 960 chips, 82.051 µs; a millisecond’s 37 nJ inside it is −3.46 dBm. With no preamble to search for there is no SHR offset: the timestamp is the first pulse of the first fragment.',
    ...
    { heading: { en: 'A room one 4z frame cannot cross', zh: '一个 4z 帧过不去的房间' }, text: {
      en: 'Fragments are one millisecond apart, and the narrowband exchange has told the receiver the train’s shape, so it accumulates blind and counts at the end: X equal-power fragments combine to 10·log10(X) dB, and the train is detected when the received power plus that gain clears −93 dBm. The hall is 22 × 8 m, cut into three bays by full-height brick partitions at x = 5 and x = 10. Three anchors stand in the first bay at 2.20 m, all about equally far from the tag at (13.00, 4.00) in the third: 13.04, 13.04 and 12.76 m through 24 dB of brick. Every fragment arrives at −100.26 dBm, or −100.07 from the near anchor: seven decibels under the receiver.',
      zh: '片段彼此相距一毫秒，而窄带交互已经告诉接收机这一串的形状，所以它盲目地累加，到结束时再数：X 个等功率片段合成为 10·log10(X) dB，当接收功率加上这份增益越过 −93 dBm，这一串就算检出。大厅 22 × 8 m，被 x = 5 与 x = 10 处两道通顶砖墙切成三个隔间。三个锚点立在第一个隔间里，高 2.20 m，到标签的距离大致相同；标签在第三个隔间的 (13.00, 4.00)：13.04、13.04 与 12.76 m，中间隔着 24 dB 的砖墙。每个片段到达时是 −100.26 dBm，近处那个锚点是 −100.07 dBm：比接收机低七个分贝。',
    } },
    ...
    { heading: { en: 'What the narrowband radio carries', zh: '窄带电台负责运什么' }, text: {
      en: 'The UWB side only measures. Everything else rides a 250 kb/s O-QPSK radio in the 5–6 GHz bands — here channel 3, 5733.75 MHz. The initiator opens with a POLL of 12 octets and 576 µs, the responder answers with a RESP of the same size, and only then is either side primed to listen. Twenty slots later the responder’s REPORT, 13 octets and 608 µs, carries the reply time it measured. Those three are 1.760 ms of the round’s 3.073 ms of air; all sixteen fragments are 1.313 ms.',
      zh: 'UWB 一侧只负责测量。其余一切都跑在 5–6 GHz 的一部 250 kb/s O-QPSK 电台上——这里是 3 号信道，中心 5733.75 MHz。发起方用一帧 12 字节、576 µs 的 POLL 开场；响应方用同样大小的 RESP 作答，而只有到这时两边才算就绪，才会去听片段。二十个时隙之后，响应方那帧 13 字节、608 µs 的 REPORT 捎上它测到的回复时间。这三条消息占 1.760 ms，而整轮空口时间是 3.073 ms；十六个片段加起来才 1.313 ms。',
    } },
    ...
    { heading: { en: 'Three metres, and fifteen millimetres', zh: '三米，和十五毫米' }, text: {
      en: 'The clock error enters multiplied by half the reply time — one slot, 0.5 ms. Tag and anchor 1 are 40 ppm apart, so uncorrected that is ½ × 0.5 ms × 40 ppm × c = 3.00 m, and the log prints both: “range → anchor-1 (SS): 14.26 m (true 13.04 m, raw 17.25 m)”. One crystal at the ±20 ppm of §16.4.9 would be 1.5 m of it; the carrier estimate 4z falls back on leaves 1.5 cm at 0.2 ppm; the train’s 0.0202 ppm leaves 1.5 mm. So no double-sided exchange — and no seeing the 1.5 mm: two receive stamps alone are c·σ_ts/√2 = 2.1 cm, and over 21 ranges the error about this room’s bias has an RMS of 2.05 cm.',
      zh: '单边测距会把时钟误差乘上回复时间的一半，也就是一个时隙，0.5 ms。标签与 anchor-1 相差 40 ppm，不修正就是 ½ × 0.5 ms × 40 ppm × c = 3.00 m——日志把两个数一起印了出来：“range → anchor-1 (SS): 14.26 m (true 13.04 m, raw 17.25 m)”。若只有一块晶振偏到 §16.4.9 允许的 ±20 ppm，那是其中的 1.5 m；4z 退而用的载波估计，以 0.2 ppm 的残差留下 1.5 cm；而序列给出的 0.0202 ppm 只留下 1.5 mm。所以不需要双边交互——也所以这 1.5 mm 看不见：光是两个接收时间戳就值 c·σ_ts/√2 = 2.1 cm，而 21 次测距扣掉这房间自带的偏差后，误差均方根是 2.05 cm。',
    } },
    { heading: { en: 'Where the gain actually comes from', zh: '增益究竟从哪里来' }, text: {
      en: 'A 4z Poll from anchor 1 reaches the tag at −110.80 dBm, an eight-fragment train at an effective −91.23: 19.57 dB better. Only 9.03 dB of that is the multi-millisecond idea. Another 3.95 dB is that a fragment is shorter than a Poll — 82.051 µs against 203.782 — so the same energy is louder; and 6.59 dB is that this 4z transmitter never spends its budget, holding −14 dBm where a burst-mode one could hold −7.41 dBm and be as legal. The honest claim is the 9 dB.',
      zh: 'anchor-1 发出的一帧 4z Poll 到达标签时是 −110.80 dBm；八个片段的序列合成后等效为 −91.23 dBm，好了 19.57 dB。其中只有 9.03 dB 属于“多毫秒”这个想法。另有 3.95 dB 来自片段比 Poll 短——82.051 µs 对 203.782 µs——同样的能量在更短的时间里更响；还有 6.59 dB，只是因为这台 4z 发射机从不把预算花完：它保持 −14 dBm，而突发式的发射机可以保持 −7.41 dBm，一样合规。诚实的说法是那 9 dB。',
    } },
    { heading: { en: 'What the walls charge anyway', zh: '墙照样要收费' }, text: {
      en: 'Every range is long by the same amount: 14.26 m against a true 13.04. Brick adds 2 ns of excess delay to a first path, the ray crosses two walls each way, and a two-way range keeps what a time difference would cancel — 4 ns, 1.199 m, every round; the figure of merit says “75 % within 12 ns”, the NLOS byte. The block’s fix inherits it: (14.22, 4.05) m against a true (13.00, 4.00), 1.22 m east, GDOP 2.93, ellipse 6.1 × 1.3 cm — which knows only the noise. Reach is not accuracy. With Y = 0 there is no integrity flag.',
      zh: '每一次测距都偏长，而且长得一样多：14.26 m 对真值 13.04 m。这个模型里砖墙给首径添 2 ns 额外时延，射线来回各穿两道墙，而双向测距会保留时间差本可抵消的那一部分——4 ns、1.199 m，每一轮都有；品质因子写着 “75 % within 12 ns”，那是 NLOS 的字节。整块的定位继承了它，落在 (14.22, 4.05) m，真值 (13.00, 4.00)——向东 1.22 m，GDOP 2.93，椭圆 6.1 × 1.3 cm，而椭圆只知道噪声。合成换来的是距离；距离不等于精度。Y = 0，也没有完整性标志。',
    } },
  ],
  ...
  observe: [
    ...
    { en: 'At 9.500 ms, the slot after the tag’s last fragment, anchor 1 rules: “anchor-1 RSF train ← tag-1: 8/8 heard, -100.3 dBm + 9.0 dB = margin 1.8 dB → detected, ratio -39.995 ppm”, and only then a receive stamp: “anchor-1 RX RMARKER ← tag-1 RSF: counter 26504711136”. The tag rules at 10.000 ms, the ratio the other way up: 39.970 ppm.',
      zh: '9.500 ms 处，也就是标签最后一个片段之后的那个时隙，anchor-1 给出判定：“anchor-1 RSF train ← tag-1: 8/8 heard, -100.3 dBm + 9.0 dB = margin 1.8 dB → detected, ratio -39.995 ppm”，接收时间戳直到这时才出现：“anchor-1 RX RMARKER ← tag-1 RSF: counter 26504711136”。标签自己的判定在 10.000 ms 处，比值是反过来的：39.970 ppm。' },
    { en: 'At 12.000 ms “anchor-1 → tag-1 NBREPORT 13 B @0.25 Mbps (608.0 µs)”, and at 12.608 ms “tag-1 range → anchor-1 (SS): 14.26 m (true 13.04 m, raw 17.25 m)”. The tag’s inspector then reads 8 × RSF, 8 / 8, +1.8 dB, detected (+2.0 dB for the near anchor); channel “3 · 5733.75 MHz”; fix (14.22, 4.05) m, error 122.4 cm, GDOP 2.93, ellipse 6.1 × 1.3 cm.',
      zh: '12.000 ms 处是 “anchor-1 → tag-1 NBREPORT 13 B @0.25 Mbps (608.0 µs)”，12.608 ms 处是 “tag-1 range → anchor-1 (SS): 14.26 m (true 13.04 m, raw 17.25 m)”。跑过几个块之后打开标签的检视面板：两个远处锚点是 8 × RSF、8 / 8、+1.8 dB、检出，近处那个是 +2.0 dB；窄带信道一行是 “3 · 5733.75 MHz”；定位 (14.22, 4.05) m，误差 122.4 cm，GDOP 2.93，椭圆 6.1 × 1.3 cm。' },
  ],
  tryThis: [
    { en: 'Load “Four fragments”. Nothing changes on the air — every train still heard in full, 4/4, at −100.3 dBm — but four combine to 6.0 dB instead of 9.0: “anchor-1 RSF train ← tag-1: 4/4 heard, -100.3 dBm + 6.0 dB = margin -1.2 dB → lost”. Each side loses all three of the other’s trains. No RMARKER, no reply time, no report: 21 timeouts, every one “tag-1 UWB slot 24: no nb-report from anchor-1”. Not one range in 1.3 seconds — three decibels between a working system and a dead one.',
      zh: '载入“四个片段”。空口上什么也没变——每一串仍被完整收到，4/4，仍是 −100.3 dBm——但四个合成的是 6.0 dB 而不是 9.0：“anchor-1 RSF train ← tag-1: 4/4 heard, -100.3 dBm + 6.0 dB = margin -1.2 dB → lost”。三个锚点都丢了标签那一串，标签也把三串全丢了。没有 RMARKER 就没有回复时间，也就没有报告：21 次超时，每一条都是 “tag-1 UWB slot 24: no nb-report from anchor-1”。1.3 秒里一次测距、一次定位都没有——能用与报废之间只隔着三个分贝。' },
  ...
  quiz: [
    {
      ...
      answer: 1,
      explain: { en: 'Each fragment spends its own millisecond’s budget, so its power does not depend on how many follow: −100.26 dBm in both scenes. Four combine to −94.24 and eight to −91.23: the margin moves from −1.24 dB to +1.77.', zh: '每个片段花的是自己那一毫秒的预算，功率与后面还有几个无关：两个场景里远处锚点都是 −100.26 dBm。四个合成为 −94.24，八个为 −91.23，余量从 −1.24 dB 变成 +1.77 dB。' },
    },
    {
      ...
      options: [
        ...
        { en: '12.04 dB, the largest combining gain the model allows', zh: '12.04 dB，模型允许的最大合成增益' },
      ],
      answer: 1,
      explain: { en: '10·log10(8) = 9.03 dB is the only part that comes from spending eight milliseconds instead of one. The rest compares two transmitters, not two ideas.', zh: '10·log10(8) = 9.03 dB 才是“花八个毫秒而不是一个”带来的部分。其余比较的是两台发射机，而不是两个想法。' },
    },
    {
      q: { en: 'Single-sided ranging usually needs a double-sided exchange, or a very good crystal. Why does this mode need neither?', zh: '单边测距通常要么做双边交互，要么靠一块很好的晶振。这个模式为什么两样都不需要？' },
      options: [
      ...
      answer: 1,
      explain: { en: 'σ_ratio is 0.0202 ppm over the train’s 7 ms, and half of the 0.5 ms reply turns it into 1.5 mm — against 1.5 cm from the carrier estimate and 3.00 m uncorrected.', zh: '整串 7 ms 的跨度给出 σ_ratio = 0.0202 ppm，乘上 0.5 ms 回复时间的一半就是 1.5 mm——而载波估计对应 1.5 cm，不修正则是 3.00 m。' },
    },
  ],
}
