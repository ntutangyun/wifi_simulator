...
export const uwbMms: Lesson = {
  ...
  body: [
    { text: {
      en: 'Almost nothing here is IEEE Std 802.15.4-2024. The units are — RSTU, RCTU, the block and its slots — but the multi-millisecond packet and the narrowband control radio come from P802.15.4ab, at D5.0 in Sponsor-ballot recirculation. That draft is members-only, so this paraphrases four TG4ab contributions: 15-22/0381r5 (the cycle), 15-23/0100r2 (fragments, narrowband PHY), 15-23/0502r3 (parameter sets), 15-22/0205r0 (the budget). The balloted draft may differ. One number is regulation — the −41.3 dBm/MHz mean EIRP averaged over a millisecond — and the rest is model: a fragment’s power, the combining rule, this room’s path loss.',
      zh: '本课几乎没有一处出自 IEEE Std 802.15.4-2024。单位是——RSTU、RCTU、块与时隙——但多毫秒分组和那部窄带控制电台都来自 P802.15.4ab：截至 2026 年 9 月，它仍处于 Sponsor 投票再循环阶段，版本为 D5.0。该草案文本仅对会员开放，所以这里都改写自 TG4ab 的四篇提案文稿：15-22/0381r5（测距周期）、15-23/0100r2（片段与窄带 PHY）、15-23/0502r3（参数集）以及 15-22/0205r0（能量预算）。已投票的草案可能与之不同。只有一个数字来自法规——按毫秒平均的 −41.3 dBm/MHz 平均 EIRP——其余都是模型：片段的功率、合成规则、这个房间的路径损耗。',
    } },
