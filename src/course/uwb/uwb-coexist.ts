...
export const uwbCoexist: Lesson = {
  ...
  module: 13,
  title: '共享 6 GHz',
  why: '到目前为止，测距会话一直独占着整个房间。可真实的房间里有一台 Wi-Fi 路由器，而在最新的那个 Wi-Fi 频段上，两种射频完全可能用着同一段兆赫。它们谁也听不懂对方：在路由器耳里，一帧测距帧只是微弱的噪声；在手机耳里，一次 Wi-Fi 猝发却是一声吼。这一课要算清两个方向上各自的损失，并看看那几个一眼就能想到的办法里，究竟哪一个真的管用。',
  outcomes: [
  ...
  terms: [
    ...
    { term: 'noise floor', plain: '器声地板：没人说话时接收机听到的那份功率，任何信号都要高过它才会被听见' },
    { term: 'energy detect', plain: 'Wi-Fi 那条粗放的“忙”判据：信道上功率太大，就先别发' },
  ],
  picture: [
    { heading: '两种射频，同一片频谱', text: '此前几课把测距会话放在了一条没人用的信道上。把它挪进 6 GHz 路由器工作的那个频段，两者就共用起频谱来。在本课的房间里，路由器的整条信道都躺在测距信道之内：一个频带落进另一个频带的那一部分（重叠）是全部，每一次 Wi-Fi 猝发都不偏不倚地压在会话头上。' },
    { heading: '一声吼，一声耳语', text: '路由器的发射功率比测距设备高出好几千倍，而且全部塞进一条窄信道里。测距设备正相反：它把本就微小的功率摊在宽出许多倍的信道上，能真正到达路由器的只是其中一小片。再把几何摆进来：路由器和笔记本都离手机很近，锚点却远在四角。' },
    ...
    { heading: '手机听到了什么', text: '在手机这一侧，最强的“想要”是从房间对角作答的那个锚点，最响的“不想要”是几步之外的那台笔记本。两者之间的差距就是 SIR——信干比——这里的它是一个很深的负数，远远低于接收机还解得出来的那个电平，这就是门限。' },
    { heading: '路由器听到了什么', text: '反过来看。一帧测距帧到达路由器时比路由器自己的噪声还弱：它把没人说话时路由器听到的那份功率——噪声地板——抬高几个分贝，除此之外什么也没做。Wi-Fi 在这里判忙只看信道上的原始功率，这就是能量检测（energy detect），而这样距离上的测距帧离它的门限差得很远。于是路由器从不为它退让，与这个会话打照面的只有解调器。' },
  ...
  numbers: [
    ...
    { kind: 'formula', heading: '只有重叠的那一片算数', text: 'in-band EIRP = EIRP + 10·log10(W_overlap / W_own)      Wi-Fi: 20 + 10·log10(80/80) = 20 dBm      UWB: −14 + 10·log10(80/499.2) = −21.95 dBm', note: '路由器这一侧一点也没损失：它的信道整个落在测距信道里面。而一帧测距帧的功率摊在 499.2 MHz 上，进到 80 MHz 接收机里的只有 80 MHz——路径损耗还没开始，就先丢掉 7.95 dB。' },
    ...
    { kind: 'steps', heading: '两种射频之间究竟发生了什么，一步一步', items: [
      ...
      '这个块之内不做任何重传。那个时隙超时，打印出 UWB_TIMEOUT，而定位就用已经到齐的距离来解：三条仍然解得出，那一行末尾写的是三个锚点。',
      '反方向读的是同一个中介，而故事早一步就结束：一帧测距帧只给 Wi-Fi 接收机添点噪声，而这里没有哪一帧能摸到 −62 dBm 能量检测门限的 18.57 dB 以内。界线就在这里——两种射频的载波侦听都看不见对方，越界的只有功率。',
    ] },
