...
export const uwbDlTdoa: Lesson = {
  ...
  picture: [
    ...
    { heading: '量的是差，不是距离', text: '不发信号的胸牌，永远量不出某个锚点有多远：那需要一次往返，而它一次也不做。它能做的只是相减——这一帧比那一帧早落地，早了这么多。靠这些间隔来定位，就是到达时间差（time difference of arrival, TDoA）；而本课要跑的，正是它的下行形态（downlink TDoA, DL-TDoA）——发送全由锚点承担的那一种。' },
    ...
    { heading: '它自己的时钟碍事了', text: '双向测距里，标签的晶振（crystal）在一次往返之内就被抵消掉了，根本到不了结果里。而这里，胸牌相减的是自己相隔若干毫秒的两个到达时刻，于是晶振只要快上百万分之几（ppm），就会把这整段间隔一起拉长。拉长的量是几十米。放着不管，这个模式不成立。' },
    { heading: '拿自己的钟去比他们的钟', text: '解法藏在同一段时间里，而这段时间两边都各自描述过。参考锚点（reference anchor）在帧里报出自己何时发出第一帧、何时发出最后一帧，而胸牌手里有这两帧各自的到达时刻。两段跨度之比就是时钟修正（clock correction）：除以它，就把胸牌的晶振和参考锚点的晶振一起消掉了。' },
  ...
  numbers: [
    ...
    { text: '未修正那一列，正是这个胸牌与参考晶振之间 36.69 ppm 的差距，乘上旁边那段等待。剩下的是每个应答锚点自己那一点时钟偏差（clock offset）剩余误差，为它所修正的那段应答时延的 0.2 ppm——每个时隙 0.12 m，所以最后作答的锚点最差。63 个时间差全部落在最后一列之内。' },
    ...
    { kind: 'steps', heading: '一个时间差，一步一步', items: [
      ...
      '再逐个应答锚点来算：先把胸牌两次到达之间的间隔除以那个比值，再扣掉不属于几何的部分——它报出的应答时延，按它的时钟偏差换算过来，加上 Poll 沿那段已勘测锚点基线（anchor baseline）飞行的时间。',
