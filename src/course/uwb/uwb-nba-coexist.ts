...
export const uwbNbaCoexist: Lesson = {
  ...
  module: 15,
  title: '窄带射频也共享 6 GHz',
  why: '那部小小的控制射频总得住在某个地方，而分给它的，正是最新的那个 Wi-Fi 频段。想用这个频段，就得守一条规矩：先听，若空口上已经有人在说话，就闭嘴。在一间摆着忙碌路由器的屋子里，这条规矩几乎从不放行——而一个说不出话的测距会话，什么也量不出来。这一课要把这笔交易的两面都算清楚。',
  outcomes: [
    '说出一台设备在共享的窄带信道上开口之前必须先做什么',
    '解释为什么一次“不许发”赔上的是整整一个测距块，而不是一条消息',
    '把跳变、挪走控制信道、以及干脆关掉规则这三条路放在一起权衡',
  ],
  ...
  picture: [
    ...
    { heading: '这个频段附带的规矩', text: '在那里，设备不能想发就发。每发一条窄带消息之前，它都得先量一量信道——先听后发，即 LBT——若量到的能量超过一个固定的电平——门限——就必须保持安静。这项测试说到底只是一次功率读数：它问的是“此刻是否有人在发”，而不是“这条消息发出去能不能活下来”。' },
    ...
    { heading: '一次“不许发”赔上的是一个块', text: '而且“不许发”并不是一次退避。设备不会稍等片刻再试：它在本测距块余下的时间里，索性不再发出任何窄带帧。没有 Poll 就没有一轮对话，于是这个块后面几轮照样按格子跑——锚点们按时到场、干等着——而手机在每一轮里都一声不吭。' },
    { heading: '跳变是在平均，不是在躲开', text: '一个显而易见的办法是别老待在原地：给会话一张单子，上面写着它可以用的那几个控制信道——这就是允许列表——再让它一个块一个块地换用其中另一个，这一换就叫跳变。这里列表的一半是避开路由器的。于是会话拿回的，正是“跳变恰好把块丢到清静处”的那个比例，其余照赔——因为这个选择在整块之内不变，一个块要么完整，要么全没。' },
    { heading: '控制信道该放在哪里', text: '这笔交易的另一面并不好看：把规则关掉，会话立刻又能干活，代价由 Wi-Fi 链路来付。守规矩的那套配置恰恰是没用的那套，而你能出货哪一套，只有监管说了算。真正的教训因此在这儿：控制信道根本不必待在 Wi-Fi 所在的频段里，它要的只是一薄片没人要的频谱——而这样的薄片有好几百。' },
  ],
  numbers: [
    ...
    { kind: 'table', heading: '手机在“听”的时候听到了什么', head: [
      ...
      ['路由器的 20 MHz 控制帧', '−42.21 dBm', '28.8 dB over'],
    ...
    { heading: '反过来的那一侧', text: '反过来看，这部窄带射频也远称不上无害。它那 10 dBm 全挤在一个 2.5 MHz 的信道里，按此推算，它在 15.07 m 外仍够得着 Wi-Fi 的能量检测门限——比这个房间还长。于是这里发出的每一条控制消息，屋里每一台 Wi-Fi 收发机都听得见。' },
  ...
  observe: [
    ...
    '手机的检视面板上写着 “200 · 6301.25 MHz”，而“先听后发”一行是 “7 次忙 · 跳过 7 个块”。它那几行片段纹丝不动，而超时有 21 次，测出的距离只有四个。',
  ],
  tryThis: [
