...
export const uwbNbaCoexist: Lesson = {
  ...
  numbers: [
    ...
    { text: {
      en: 'Every one of the phone’s seven busy checks reads −63.72 dBm: the laptop, uploading, three paces away. Seven blocks, seven checks, seven skipped, and two anchors lose a block to a check of their own as well.',
      zh: '手机那七次判忙，每一次读到的都是 −63.72 dBm：三步之外那台正在上传的笔记本。七个块，七次检测，七个被跳过；另外还有两个锚点，也各自因一次判忙赔掉一个块。',
    } },
  ...
  observe: [
    { en: 'At 13.000 ms the phone’s own report slot: “uwb-1 NB LBT busy on ch 200: -63.7 dBm ≥ -71.0 — skipping the block”. Nothing of the phone’s is heard again for the rest of that block.',
      zh: '13.000 ms 处轮到手机自己的报告时隙：“uwb-1 NB LBT busy on ch 200: -63.7 dBm ≥ -71.0 — skipping the block”。此后直到这个块结束，空口上再也听不到手机。' },
    { en: 'The anchors say what that silence costs: “anchor-1 UWB slot 26: no nb-report from uwb-1”, and in the next round “anchor-2 UWB slot 0: no nb-poll from uwb-1”. The grid runs on without it.',
      zh: '锚点们替这段沉默开出了账单：“anchor-1 UWB slot 26: no nb-report from uwb-1”，下一轮又是 “anchor-2 UWB slot 0: no nb-poll from uwb-1”。格子照样往下跑，只是少了它。' },
    { en: 'The phone’s inspector reads “200 · 6301.25 MHz” and, under listen before talk, “7 busy · 7 blocks skipped”. Its fragment rows are untouched, and there are 29 timeouts against a single distance.',
      zh: '手机的检视面板上写着 “200 · 6301.25 MHz”，而“先听后发”一行是 “7 次忙 · 跳过 7 个块”。它那几行片段纹丝不动，而超时有 29 次，测出的距离只有一个。' },
  ],
  tryThis: [
