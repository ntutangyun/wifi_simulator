...
export const uwbSstwr: Lesson = {
  ...
  picture: [
    ...
    { heading: '一块从没被校准过的晶振', text: '每台射频都用自己那块晶振数时间，而没有哪块晶振真的走在标称速率上——这点误差用百万分之几（ppm）来说。这里的手机走得偏快，每个锚点都走得偏慢，而且谁也不知道。两端减的依然是自己计数器上的两次读数，未知的起点因此被约掉；速率却没有被约掉。' },
    ...
    { heading: '问接收端：对面那只钟走得多快', text: '接收端必须先锁住进来的脉冲，才谈得上找到 RMARKER（ranging marker）；而完成这次锁定的环路顺带就知道：这些脉冲相对自己那块晶振跑得有多快。这个比值就是时钟偏差（clock offset）——是测出来的，不是假定的；仿真器把它挂在每个收到的帧上，叫作 Coffs。拿它去缩放应答时长，那次“几乎抵消”就成了真的抵消。' },
  ...
  numbers: [
    ...
    { kind: 'steps', heading: '一次测距，一步一步', items: [
      ...
      '锚点在自己的时隙里给 Response 的离开打戳，把自己的两个计数值一减，再把差值写进这一帧：应答时长 Treply，完全是在锚点自己的晶振上量出来的。',
      '手机给 Response 的到达打戳，再减去自己的那两个计数值：往返时长 Tround。一共四个戳，两端各两个，谁也没有去减对方的那一个。',
      '同一次接收的载波锁定顺带报出：锚点的晶振比手机快多少。这就是 Coffs，这里是 −20.24 ppm——两块晶振本身相差的 20 ppm，再加上估计器弄错的 0.2 ppm。',
      '把这两段时间的差折半。不算 Coffs，得到 2028 格，就是测距行上打印的 raw 值；先用 Coffs 缩放应答时长再折半，得到 734.6 格。',
      '乘以一格的 15.65 ps，再乘以光速：3.45 m，而真值是 3.50 m。',
      '测距行上带的就是这个数，旁边是 raw 值，后面跟一个品质因数（figure of merit, FoM）字节——从来没有误差棒。画误差椭圆用的那个 1σ 是另外算的，只由时间戳噪声决定：100 ps ÷ √2 × 光速，合 2.1 cm。定位本身对每条距离一视同仁。',
    ] },
  ...
  deeper: [
    ...
    { heading: '用来说明“这有多可信”的那个字节', text: '每一条描述接收帧的 UWB_TS 行末尾都跟着 "(97 % within 0.5 ns)"。那是品质因数字节，此处为 0x16：3 位置信水平（6 → 97 %）、2 位区间（2 → 1 ns）、2 位比例因子（0 → ×0.5）。半纳秒宽的区间就是 ±0.25 ns，约合 7.5 cm 的单向飞行距离。它随测量结果一起传递，好让定位解算器给可信的距离更高的权重；而它对晶振只字未提：上面那个 27.42 m 的读数，正出自一个置信度恰好如此的时间戳。' },
  ],
  sources: [
