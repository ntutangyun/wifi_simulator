...
export const uwbSstwr: Lesson = {
  ...
  module: 11,
  title: '应答时间里藏着的那只时钟',
  why: '前面几课让两台射频都守着完美的时间，现实里没有哪一对是这样：石英晶体总会走得偏快或偏慢，用走得慢的钟量出来的一段时间就偏短。偏偏交互里最长的那一段是锚点量的，手机却把它当成自己量的直接减掉。这一课里，这件事会错出好几米。',
  outcomes: [
  ...
  needs: ['uwb-frame'],
  terms: [
    { term: 'crystal', plain: '射频用来数时间的那一小片石英；没有哪两片走得一样快' },
    { term: 'ppm', plain: '百万分之几，时钟速率误差的单位：一百万格里差一格' },
    { term: 'clock offset', plain: '一台射频的时钟比另一台快多少，写成一个比值' },
    { term: 'SS-TWR', plain: '单边双向测距：一问一答' },
    { term: 'Coffs', plain: '接收端对自己正在听的那台射频测出的时钟偏差' },
  ],
  picture: [
    { heading: '四个锚点，一个距离', text: '把一部手机放在实验室正中，四个锚点摆在它周围的一个圆环上，每个锚点离它一样远、也一样高。手机只问一次，四个锚点都听见了；随后它们逐一作答，各占一个时隙。一次提问，换来对同一个距离的四次测量。' },
    { kind: 'watch', jump: 2, heading: '它以为这段距离是多少？', text: '载入仿真，跳到第一条测距行。手机离那个锚点三米半，而它自己以为是多少？读一读，再往下读接着的三行。' },
    { heading: '一块从没被校准过的晶体', text: '每台射频都用自己那块晶体数时间，而没有哪块晶体真的走在标称速率上——这点误差用百万分之几（ppm）来说。这里的手机走得偏快，每个锚点都走得偏慢，而且谁也不知道。两端减的依然是自己计数器上的两次读数，未知的起点因此被约掉；速率却没有被约掉。' },
    { heading: '出问题的是等待，不是飞行', text: '手机用自己的钟量一次往返；锚点用自己的钟量自己的作答时长，再把这个数发过来让手机减掉。两个数都被拉长了，一减之下大部分拉长量互相抵消。剩下的是应答时长的一半乘以两端速率之差。而飞行时间是几十亿分之一秒，应答时长却是整整一个时隙的等待：同一个误差落在前者上看不见，落在后者上就是好几米。' },
    { heading: '等得越久，谎话越大', text: '这就是场景里要摆四个锚点而不是一个的原因。第一个时隙里的锚点等一个时隙才作答，第四个要等四个，而剩下的那点误差与等待时长成正比。四个一模一样的距离，读出来却是一道斜坡。不作修正的话，单边双向测距（SS-TWR）量的不是距离有多远，而是应答方等了多久。' },
    { heading: '问接收端：对面那只钟走得多快', text: '接收端必须先锁住进来的脉冲，才谈得上找到 RMARKER；而完成这次锁定的环路顺带就知道：这些脉冲相对自己那块晶体跑得有多快。这个比值就是时钟偏差——是测出来的，不是假定的；仿真器把它挂在每个收到的帧上，叫作 Coffs。拿它去缩放应答时长，那次“几乎抵消”就成了真的抵消。' },
    { heading: '修正之后还剩下什么', text: 'Coffs 自己也是测出来的，因此本身就带着一点误差。修正把一个大的速率误差换成了一个很小的，但残下来的那一项依旧是应答时长的一半乘以这个小误差，所以它还是随等待时长一起变大。下一课改用算术把这段等待直接消掉。' },
  ],
  numbers: [
    { kind: 'formula', heading: 'raw 估计里真正装着什么', text: 'T̂prop = (Tround·(1 + eA) − Treply·(1 + eB)) / 2\n       = Tprop + Tprop·eA + ½·Treply·(eA − eB)', note: 'eA 是发起方的速率误差，eB 是应答方的。两次测量一减，应答时长几乎被抵消，但有两项活了下来。第一项缩放飞行时间，只有几十微米。第二项缩放等待：2 ms 的应答取一半再乘 20 ppm，就是 20 ns，合 6.0 m。' },
    ...
    { text: '这不是散布：对同一个距离的四次读数全都偏长，且每一个都是第一个的整数倍。手机距每个锚点都是 3.50 m，却认定自己离其中一个有 9.51 m、离另一个有 27.42 m。' },
    { kind: 'table', heading: '同一道斜坡，三种晶体', head: [
      '晶体', 'eA − eB',
    ...
    { kind: 'formula', heading: '经过修正的单边双向测距', text: 'T̂prop = (Tround − Treply·(1 − Coffs)) / 2', note: '只多了一次乘法。Treply 送来时是按锚点的计数单位量的；缩放一下，就把它换算成手机的单位，也就是 Tround 本来所用的单位。圆环上四个距离本是一样的，而四条测距行现在读作 3.45、3.42、3.41 与 3.51 m。射频本身什么都没有变。' },
    ...
    { text: '最后一列只是从这些分布里各抽了一次，所以它并不一路变大：最宽的那个分布这次恰好落得离真值最近。' },
    { kind: 'steps', heading: '一次测距，一步一步', items: [
      '时隙 0。手机用自己的计数器给 Poll 帧离开时的 RMARKER 打一个戳；每个锚点也给同一个 RMARKER 的到达打戳，各用各的计数器。',
      '锚点在自己的时隙里给 Response 的离开打戳，把自己的两个计数值一减，再把差值写进这一帧：应答时长 Treply，完全是在锚点自己的晶体上量出来的。',
      '手机给 Response 的到达打戳，再减去自己的那两个计数值：往返时长 Tround。一共四个戳，两端各两个，谁也没有去减对方的那一个。',
      '同一次接收的载波锁定顺带报出：锚点的晶体比手机快多少。这就是 Coffs，这里是 −20.24 ppm——两块晶体本身相差的 20 ppm，再加上估计器弄错的 0.2 ppm。',
      '把这两段时间的差折半。不算 Coffs，得到 2028 格，就是测距行上打印的 raw 值；先用 Coffs 缩放应答时长再折半，得到 734.6 格。',
      '乘以一格的 15.65 ps，再乘以光速：3.45 m，而真值是 3.50 m。',
      '测距行上带的就是这个数，旁边是 raw 值，后面跟一个品质因数字节——从来没有误差棒。画误差椭圆用的那个 1σ 是另外算的，只由时间戳噪声决定：100 ps ÷ √2 × 光速，合 2.1 cm。定位本身对每条距离一视同仁。',
    ] },
    { kind: 'table', heading: '锚点 1，照着步骤走一遍', head: [
      ...
      ['= Treply，锚点的晶体', '127 793 174'],
      ['它到达手机', '336 335 291 933'],
      ['= Tround，手机的晶体', '127 797 230'],
  ...
  deeper: [
    { heading: '活下来的那两项', text: '把 Tround = Treply + 2·Tprop 代进 raw 公式，应答时长就抵消到只剩上面那两项。Tprop·eA 是拿 10 ppm 去缩放 11.675 ns 的飞行时间：0.12 皮秒，合 35 微米，可以直接忘掉。另一项缩放的是应答时长，而 2 ms 的应答时长大约是飞行时间的十七万倍。四个锚点在几何上毫无分别，区别只在等了多久。' },
    { heading: '手机减掉的那个数', text: '锚点 1 打出两个测距计数值：26 381 597 885 与 26 509 391 059。两者之差 127 793 174 RCTU，正是它的应答帧所携带的应答时长——这个数是在锚点自己的钟上算出来的，麻烦正出在这里。手机自己那一对读数与它相差 4056 格。' },
    ...
    { heading: '用来说明“这有多可信”的那个字节', text: '每一条描述接收帧的 UWB_TS 行末尾都跟着 "(97 % within 0.5 ns)"。那是品质因数字节，此处为 0x16：3 位置信水平（6 → 97 %）、2 位区间（2 → 1 ns）、2 位比例因子（0 → ×0.5）。半纳秒宽的区间就是 ±0.25 ns，约合 7.5 cm 的单向飞行距离。它随测量结果一起传递，好让定位解算器给可信的距离更高的权重；而它对晶体只字未提：上面那个 27.42 m 的读数，正出自一个置信度恰好如此的时间戳。' },
  ],
  sources: [
    ...
    '“再深一层”里的品质因数字节及其三张查找表出自 §10.29.1.7。',
    '有三个数字是仿真器自己的模型取值：每个接收时间戳上 100 ps 的 1σ 噪声、时钟偏差估计中残留的 0.2 ppm 误差，以及 2 ms 的测距时隙——最后这个来自 FiRa，不是标准正文。',
  ],
  ...
  tryThis: [
    ...
    '再载入“±1 ppm 的温补晶振”：偏差只有基准的十分之一，也是认真的产品真会选用的器件。斜坡依然在，每个时隙约 0.60 m：更好的晶体买来的是一个数量级，不是正确性。',
  ],
  quiz: [
    {
      q: '锚点 1 与锚点 4 一样远。为什么锚点 4 的 raw 误差是锚点 1 的四倍？',
      options: [
        '又过了三个时隙，衰落让它的应答更弱',
      ...
      answer: 1,
      explain: '这不是计数器里的漂移，而是加在一段被测时间上的比例误差：同样的速率之差乘以更长的时间，误差更大。',
    },
    {
      q: 'Coffs 这个值是从哪里来的？',
      options: [
        '来自锚点的数据手册，在会话建立时交换',
        '来自接收端对进来的脉冲的锁定，拿自己的晶体比出来',
        '由往返时间与应答时间之差得到',
      ],
      answer: 1,
      explain: '这个比值是接收端无论如何都要完成的那次锁定的副产品。若想从那个差值推出它就成了循环：那个差正是我们要求的答案。',
    },
    {
      q: '修正之后，锚点 4 的距离反而最准。在时隙 4 作答是不是最好？',
      options: [
      ...
      answer: 1,
      explain: '剩余误差像 raw 误差一样随应答时长增长；而从宽分布里抽样落在哪儿都有可能，包括比窄分布的样本更靠近真值。',
    },
  ],
}
