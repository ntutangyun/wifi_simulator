...
export const uwbSts: Lesson = {
  ...
  picture: [
    ...
    { kind: 'watch', jump: 0, heading: '先看一轮诚实的测距', text: '载入仿真，走完一轮。这里没有人在攻击：Poll 发出去，锚点作答，测距行落在离真值几厘米的地方。把这一行记住——下面的一切都拿它做对照。' },
    ...
    { heading: '为什么帧的开头是一份礼物', text: '一帧测距帧的头部，按设计就是公开的。SYNC 是每个接收端本来就必须握有的图案，否则它根本锁不住信号；结束 SYNC 的那个 SFD 同样人尽皆知。攻击者当然也握着它们。于是，一个只对 RMARKER 计时、别的什么都不看的接收端，它计的正是攻击者压根不用听就能造出来的东西。' },
    { heading: '一段只有两个人写得出的脉冲', text: '解法是：在取时间的那个位置，放上一段猜不出来的东西。会话的两端共同持有一个秘密——一把密钥——并各自用它生成同一段很长的脉冲：加扰时间戳序列（STS）。真实的接收端手里有自己的那一份，它拿它和收到的东西逐段比对，只有对得上才认下这个时间戳。而中间那个盒子没有这把密钥，听到的只是噪声，也就没有任何东西可以提前发出去。' },
  ...
  numbers: [
    ...
    { kind: 'steps', heading: '接收端拿这段序列做什么', items: [
      '在真实的射频里，这一轮开始之前，两端各自用共同持有的那把密钥生成同一段脉冲。本仿真器的帧里留着它该在的那一段——SFD 之后，夹在两段 512 码片的静默间隔中间——但并没有真的往里面生成脉冲。',
      ...
      '序列一关，同一道闸门就放行了：接收端把这 50 ns 从自己测到的到达时刻里减掉，时间戳因此偏低 3195 格——这一轮的两次接收都如此。',
  ...
  sources: [
    ...
    '一次拒绝对这一轮的后果——接收端丢掉读数，由时隙超时去报告这次缺席——同样是模型取值。标准要求接收端校验这段序列并给出品质因子，但并没有规定为这次失败留下一条时间线记录。',
