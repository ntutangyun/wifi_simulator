 ...
 * below totals 1715 words, so there is room for nine more and no more:
 ...
 * (4, 3.5, 1.0) — off centre on purpose, so the four bearings are not four
 * right angles and GDOP has something to say. The session is the default one
...
export const uwbPosition: Lesson = {
  ...
  body: [
    ...
    { heading: { en: 'Four rings, one point', zh: '四个圆环，一个点' }, text: {
      en: 'Four ranges, two unknowns. If every range were exact the circles around the anchors would cross at one point — and the scene draws those circles, one amber ring per anchor at the range it just measured. They never quite meet: a centimetre of noise on each radius leaves an untidy region, and the solver picks the place that fits all four least badly.',
    ...
    { kind: 'formula', text: {
      ...
      en: 'Anchor i’s residual at a trial point p is how much further p is from that anchor than the measurement claims; the best fix minimises the four squared residuals. The gradient of ‖p − a_i‖ is the unit vector from anchor to p, so a Jacobian row is that unit vector: a direction, the distance divided out. One step is a 2 × 2 system. The engine starts at the anchors’ centroid and stops when a step falls under 1 mm, or after 20 iterations; on exact ranges it converges to within a micrometre. Rows carry only the horizontal part; the tag’s height is not solved for. Under three ranges, or with anchors in a line, there is no fix.',
    ...
    { text: {
      en: 'Two things lift this scene to 1.05. The anchors are at 2.20 m and the tag at 1.00 m, so each Jacobian row is the horizontal shadow of a slanted unit vector, only 0.968 to 0.985 long. And the tag is at (4, 3.5), not the room’s centre, so its bearings are 64.6°, 89.4°, 95.2° and 110.8° apart, not four right angles. Neither costs much: with anchors in the corners, GDOP stays inside 1.03 to 1.26 everywhere in this 10 × 8 m room.',
      zh: '有两件事把本场景抬到了 1.05。锚点在 2.20 m 而标签在 1.00 m，于是雅可比的每一行都是斜向单位向量的水平投影，长度只有 0.968 到 0.985。加上标签位于 (4, 3.5) 而非房间正中，它看四个锚点的方位角间隔是 64.6°、89.4°、95.2° 与 110.8°，而不是四个直角。两者的代价都不大：锚点装在四个角上，这间 10 × 8 m 房间里任何位置的 GDOP 都落在 1.03 到 1.26 之间。',
    } },
    { heading: { en: 'The ellipse around the cross', zh: '十字上的那个椭圆' }, text: {
      en: '(JᵀJ)⁻¹ times the variance of one range is the covariance of the fix, Σ = σ_r²(JᵀJ)⁻¹. Lesson 2 fixed σ_r at c·σ_ts/√2, 2.12 cm at 100 ps, and Σ’s eigenvectors are the ellipse drawn around the amber cross: 1.7 × 1.4 cm, long axis nearly north–south at −86.8°. Too small to see beside a 4.76 m ring, so the scene draws it ten times over — a 17 cm semi-axis — while the inspector prints the true 1.7 × 1.4 cm. Seven blocks put the fix 0.5 cm to 3.3 cm from the truth, every one inside 4σ_r·GDOP = 8.9 cm.',
      zh: '把 (JᵀJ)⁻¹ 乘上单次测距的方差，得到的就是定位结果的协方差：Σ = σ_r²(JᵀJ)⁻¹。第 2 课已经定下 σ_r = c·σ_ts/√2，在 100 ps 的时间戳噪声下即 2.12 cm；Σ 的特征向量就是场景围着琥珀色十字画出的那个椭圆：1.7 × 1.4 cm，长轴几乎南北向，与 +x 轴成 −86.8°。按真实尺寸，它在 4.76 m 的圆环旁不过一小团污迹，所以场景把它放大十倍来画——长半轴 17 cm——而检视面板在一旁老实印出 1.7 × 1.4 cm。七个块里，定位与真值相差 0.5 cm 到 3.3 cm，每一次都落在 4σ_r·GDOP = 8.9 cm 以内。',
    } },
    { kind: 'table', head: [
    ...
    { text: {
      en: 'The FoM says so out loud: that range carries 0x7b, “75 % within 12 ns”, the other three 0x16, “97 % within 0.5 ns”. Two things about the byte are worth keeping. It reports geometry, not the delay — clear the session’s NLOS switch and the range returns to centimetres while the byte still reads 0x7b — and this solver never reads it: all four ranges weigh the same.',
      zh: 'FoM 把这件事直接喊了出来：这一条距离带着 0x7b，“75 % 落在 12 ns 内”，另外三条带着 0x16，“97 % 落在 0.5 ns 内”。关于这个字节有两点值得记住：它报告的是几何而不是那段时延——把会话的 NLOS 开关关掉，距离会回到厘米级，而 FoM 依旧读作 0x7b；以及，本解算器根本不看它，四条距离一视同仁。',
    } },
    { text: {
      en: 'What does 0.60 m of error on one range in four do to the point? Not 0.60 m. The fix lands at (4.18, 3.75) — 31 cm out, +0.18 m in x and +0.25 m in y, away from the blocked anchor in both. Noise-free the shift is 0.316 m, 53 % of the bias, on a bearing of 54°. Three honest ranges pull back against one that lies, and least squares splits the difference, leaving a residual: 21 cm RMS, where a clean round leaves a micrometre.',
      zh: '那么，四条距离里有一条错了 0.60 m，对这个点意味着什么？不是 0.60 m。定位落在 (4.18, 3.75)——偏了 31 cm，x 方向 +0.18 m，y 方向 +0.25 m，两个方向都在远离那个被挡住的锚点。扣掉噪声，位移恰为 0.316 m，是偏差的 53 %，方位角 54°。三条诚实的距离与一条说谎的距离互相拉扯，最小二乘取了折中，于是留下残差：均方根 21 cm，而干净的一轮只留下一微米。',
    } },
    { text: {
      en: 'The expensive part is that nothing else on screen moves. GDOP still reads 1.05 and the ellipse still 1.7 × 1.4 cm, because both are built from directions and an assumed σ_r. They describe the scatter of honest ranges; about a range that is simply wrong they say nothing — and 31 cm is nearly four times the 8.9 cm envelope they promise.',
      zh: '真正昂贵的地方在于：屏幕上别的什么都没动。GDOP 依旧 1.05，椭圆依旧 1.7 × 1.4 cm，因为两者都由方向和一个假定的 σ_r 拼成。它们描述的是诚实距离的散布；对于一条干脆就是错的距离，它们无话可说——而 31 cm 已近它们所许诺的 8.9 cm 包络的四倍。',
    } },
    { heading: { en: 'Three anchors', zh: '三个锚点' }, text: {
      en: 'Load “Three anchors”: the corner at (9.5, 7.5) is gone. Three ranges and two unknowns still leave one spare measurement, so there is still a fix and an ellipse, and the seven stay within 3.1 cm. What changes is the price: GDOP goes from 1.05 to 1.26 — a fifth more error for the same radio — the ellipse grows to 2.2 × 1.5 cm, its axis ratio from 1.25 to 1.44, and its long axis swings from −86.8° to +61.5°, towards the corner that sat at 36° from the tag.',
      zh: '载入“三个锚点”：(9.5, 7.5) 角上的锚点没有了。三个距离配两个未知数，仍多出一个测量，所以定位还在，椭圆也还在，七次定位的误差都不超过 3.1 cm。变的是价码。GDOP 从 1.05 涨到 1.26——同样的射频，多两成的误差；椭圆胀到 2.2 × 1.5 cm，长短轴之比从 1.25 变成 1.44，长轴从 −86.8° 摆到 +61.5°，朝那个空出来的角倾斜——从标签看过去，它原本在 36° 方向上。',
    } },
    { text: {
      en: 'The direction nobody measures is the direction the fix is least sure of — and the 1.03-to-1.26 band that four corner anchors held everywhere is gone with it: drag the tag onto the anchor at (9.5, 0.5) and the three-anchor GDOP passes 2.3, because two of the three bearings nearly coincide and JᵀJ flattens.',
      zh: '再没有人去测量的那个方向，正是定位最没把握的方向——而四角锚点在任何位置都守得住的那条 1.03 到 1.26 的区间，也跟着一起没了：把标签拖到 (9.5, 0.5) 的锚点上，三锚点的 GDOP 会越过 2.3，因为那里三个方位角有两个几乎重合，JᵀJ 开始变扁。',
    } },
  ],
  ...
  observe: [
    ...
    { en: 'In the scene: four amber rings at the ranges just measured, a cross at the fix, the ellipse around it, all fading over one block. The ellipse is drawn ten times life size — 17 cm for a 1.7 cm semi-axis — while the inspector prints GDOP 1.05 and “error ellipse (1-σ) 1.7 × 1.4 cm”.',
    ...
    { en: 'Load “A brick wall in one path”. The anchor-1 row reads 5.33 m against a true 4.76 m, error 57.1 cm, “75 % within 12 ns”; the other three are within 1.4 cm at “97 % within 0.5 ns”. The fix: “uwb-1 position (4.18, 3.75) m, true (4.00, 3.50), error 0.31 m, GDOP 1.05, 4 anchors” — 31 cm moved, GDOP and ellipse unmoved.',
      zh: '载入“一堵砖墙挡住一条路径”。anchor-1 那一行写着 5.33 m，真值 4.76 m，误差 57.1 cm，“75 % 的误差落在 12 ns 内”；另外三行误差都在 1.4 cm 以内，写着“97 % 的误差落在 0.5 ns 内”。定位那一行是 “uwb-1 position (4.18, 3.75) m, true (4.00, 3.50), error 0.31 m, GDOP 1.05, 4 anchors”——位置挪了 31 cm，GDOP 与椭圆纹丝未动。' },
  ...
  tryThis: [
    { en: 'Drag the tag around the base scene in the editor, reading GDOP after each block. In the corner at (1, 1) it only reaches 1.18, at (0.6, 0.6) 1.23; anywhere in the room it stays between 1.03 and 1.26: every point is surrounded. Now load “Three anchors” and drag the tag onto the anchor at (9.5, 0.5): GDOP passes 2.3.',
      zh: '打开编辑器，在基准场景里把标签拖来拖去，每个块之后看一眼 GDOP。拖进 (1, 1) 的角落它也只升到 1.18，到 (0.6, 0.6) 是 1.23；房间里任何位置它都在 1.03 与 1.26 之间，因为每个点都被围住了。然后载入“三个锚点”，把标签拖到 (9.5, 0.5) 的锚点上：GDOP 会越过 2.3。' },
    { en: 'On “A brick wall in one path”, open the ranging session and clear the NLOS switch. The run becomes the base run — the same seven fixes, error back inside 3.3 cm — while anchor-1 still reads “75 % within 12 ns”, because the FoM is geometry and the switch only idealises the delay.',
  ...
  quiz: [
    {
      q: { en: 'The wall makes one range of four 0.60 m too long, yet the fix moves 0.32 m. Why not 0.60 m?', zh: '砖墙让四条距离里的一条长了 0.60 m，可定位只移动了 0.32 m。为什么不是 0.60 m？' },
      options: [
      ...
      answer: 1,
      explain: { en: 'Noise-free the shift is 0.316 m, 53 % of the 0.5996 m bias, away from the blocked anchor, and it leaves a 21 cm residual.', zh: '扣掉噪声后位移是 0.316 m，即 0.5996 m 偏差的 53 %，方向远离那个被挡住的锚点，并留下 21 cm 的残差。' },
    },
    {
      ...
      options: [
        ...
        { en: 'Both come from JᵀJ and an assumed σ_r: they predict the scatter of honest ranges and have no term for a biased one', zh: '两者都出自 JᵀJ 与一个假定的 σ_r：它们预测的是诚实距离的散布，式中没有描述偏差的项' },
        { en: '31 cm is still inside the 8.9 cm envelope', zh: '31 cm 仍落在 8.9 cm 的包络之内' },
      ],
      answer: 1,
      explain: { en: '31 cm is nearly four times that envelope. What flags the range is its FoM, 0x7b, and the solver’s residual — 21 cm instead of a micrometre.', zh: '31 cm 已近该包络的四倍。真正给这条距离亮红灯的，是它 0x7b 的 FoM，以及解算器的残差——21 cm，而不是一微米。' },
    },
    {
      ...
      options: [
        ...
        { en: 'With bearings spread evenly JᵀJ is (N/2)·I, so the trace of its inverse is 4/N and GDOP is 2/√N = 1.00 at N = 4', zh: '方位角均匀分布时 JᵀJ = (N/2)·I，其逆的迹为 4/N，于是 GDOP = 2/√N，N = 4 时正好 1.00' },
        { en: 'The four ranges are nearly equal there, so their errors cancel', zh: '那里四条距离几乎相等，误差彼此抵消了' },
      ],
      answer: 1,
      explain: { en: 'It reads 1.05 because the anchors are 1.2 m above the tag, so each Jacobian row is only 0.968 to 0.985 long, and the tag is off centre, so the bearings are 64.6° to 110.8° apart.', zh: '它读作 1.05，是因为锚点比标签高 1.2 m，雅可比的每一行只有 0.968 到 0.985 长；而标签不在正中，四个方位角的间隔是 64.6° 到 110.8°。' },
    },
  ],
}
