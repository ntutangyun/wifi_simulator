...
export const uwbAoa: Lesson = {
  ...
  body: [
    { text: {
      en: 'One thing here comes from IEEE Std 802.15.4-2024: §10.29.1.1 lists angle of arrival among the results a ranging round may report, so an anchor may hand back a direction as well as a distance. The rest is the model: two antennas a half wavelength apart, a phase measurement of σ_φ = 0.15 rad, the arc sine that inverts it, its clamp, the mirror behind the anchor, and every number below — FiRa-style, not anything the standard specifies.',
    ...
    { heading: { en: 'The same noise is worth more degrees off to the side', zh: '同样的噪声，越偏越值钱' }, text: {
      en: 'Invert the line and phase noise becomes angle noise: σ_θ = σ_φ/(π·cos θ). Straight ahead that is 2.74°; at 45° it is 3.87°, at 60° 5.47°, and at ±90° it diverges, because there a degree of azimuth changes no phase at all. The model clamps it at 45°, past which the linearisation describes nothing. Nowhere is this growth chosen: it falls out of sin θ flattening.',
      zh: '把这条式子反解，相位噪声就变成角度噪声：σ_θ = σ_φ/(π·cos θ)。正前方是 2.74°；45° 处 3.87°，60° 处 5.47°；到 ±90° 处发散，因为那里方位角改变一度也不改变相位。模型把它截断在 45°，再往后这个线性化什么也描述不了。这种增长不是谁选的：它是 sin θ 变平自己长出来的。',
    } },
    { heading: { en: 'A circle and a ray', zh: '一个圆和一条射线' }, text: {
      en: 'DS-TWR already hands the anchor the badge’s range at the end of every round, with the 2.12 cm sigma it has had since module 11. A range is a circle, a bearing is a ray, and the two meet in exactly one point — so the anchor solves the fix alone, and the answer is that polar coordinate rather than anything iterative. GDOP is 1.00 by construction and has nothing to say here. Compare error ellipses instead.',
      zh: 'DS-TWR 本来就在每轮结束时把胸牌的距离交给锚点，σ 仍是第 11 模块以来的 2.12 cm。距离是一个圆，方位角是一条射线，两者恰好交于一点——于是锚点独自把定位解了出来，而答案就是那个极坐标本身，并不需要迭代。GDOP 恒为 1.00，在本课里无话可说。要比就比误差椭圆。',
    } },
  ...
  observe: [
    ...
    { en: 'Three lines land together at 4.193 534 ms: “anc-1 AoA ← badge-1: 1.8° (true 0.0°)”, “anc-1 range → badge-1 (DS): 2.30 m (true 2.33 m)”, and “anc-1 position of badge-1 (4.94, 2.47) m, true (5.00, 2.50), error 0.07 m, GDOP 1.00, 1 anchors (AoA)”. One anchor, and the line says so. The fix takes the Final’s bearing, the one measured closest to the range it is crossed with.',
      zh: '4.193 534 ms 处有三行一起落下：“anc-1 AoA ← badge-1: 1.8° (true 0.0°)”、“anc-1 range → badge-1 (DS): 2.30 m (true 2.33 m)”，以及 “anc-1 position of badge-1 (4.94, 2.47) m, true (5.00, 2.50), error 0.07 m, GDOP 1.00, 1 anchors (AoA)”。一个锚点，而且这一行明说了。定位用的是 Final 那个方位角——它与被交叉的那个距离测得时间最近。' },
    { en: 'Open the anchor in the inspector: two tables now. A range to the badge — 2.31 m against a true 2.33, −2.4 cm, 97 % within 0.5 ns, 7 rounds, DS-TWR — and a bearing, measured −3.7°, true 0.0°, error −3.7°, ± 2.7°, over 14 rounds. The 1-σ sits on the row, not in a header: it belongs to the angle it was measured at.',
      zh: '在检视面板里打开锚点：现在有两张表。一张是到胸牌的距离——2.31 m 对真值 2.33，误差 −2.4 cm，97 % within 0.5 ns，7 轮，DS-TWR；另一张是方位角，测得 −3.7°，真值 0.0°，误差 −3.7°，± 2.7°，共 14 轮。那个 1-σ 写在行上而不是表头：它属于这个方位角测得时所在的角度。' },
    { en: 'Now open the badge. A bearing belongs to the anchor that measured it; a fix belongs to whoever it is of, so the badge’s lane holds the position: (5.13, 2.47) m against (5.00, 2.50), 13.3 cm out, GDOP 1.00, ellipse 9.4 × 2.1 cm, solved from angle of arrival. On the plan an amber bearing line runs from the anchor to the cross, and the cross sits inside the range ring.',
  ...
  tryThis: [
    { en: 'Load “45° at 4 m”, then “60° at 5 m”. The true bearing reads −45.0° and −60.0° exactly; the row’s 1-σ grows 2.7° → 4.3° → 7.5°; the mean fix error goes 9.6 → 26.5 → 48.2 cm while the range error stays at 2.0 cm. Watch the ellipse, not the number: 9.4 × 2.1, 25.7 × 2.1, 43.1 × 2.1 cm — the short axis never moves. Then drag the badge to (8.94, 1.19), 80° off at 4 m: σ_θ is 15.75°, six of the fourteen bearings are pinned at the −90.0° clamp, and the worst ellipse is 3.16 m long.',
      zh: '载入“4 m 处 45°”，再载入“5 m 处 60°”。真实方位角恰好读作 −45.0° 与 −60.0°；那一行的 1-σ 从 2.7° 长到 4.3° 再到 7.5°；平均定位误差走 9.6 → 26.5 → 48.2 cm，而测距误差始终停在 2.0 cm。别只盯着数字，看椭圆：9.4 × 2.1、25.7 × 2.1、43.1 × 2.1 cm——短轴一动不动。然后把胸牌拖到 (8.94, 1.19)，即 4 m 处偏离 80°：σ_θ 为 15.75°，十四个方位角里有六个被钉在 −90.0° 的截断值上，最差的椭圆长达 3.16 m。' },
    { en: 'Load “Behind the anchor”. Nothing about the badge changed — same spot, same 2.00 m — only the anchor’s Facing, 90° to −90°, so it looks at its own wall. The true bearing is 180.0°, the fourteen measured bearings are identical to the base scene’s, and the seven fixes are 396.7 to 403.0 cm out, mean 400.2 — twice the horizontal range, every round, with a confident 9.4 × 2.1 cm ellipse around a point outside the building. Put Facing back to 90° and the error returns to 2.1–15.6 cm. An ellipse says how precise a measurement is, never whether it points the right way.',
      zh: '载入“锚点背后”。胸牌那边什么也没变——同一个位置、同样的 2.00 m——变的只有锚点的“朝向”，从 90° 改成 −90°，于是它望着自己那面墙。真实方位角是 180.0°，十四个测得的方位角与基础场景一模一样，而七次定位偏离 396.7 到 403.0 cm，平均 400.2——正是水平距离的两倍，每一轮都如此，还围着一个楼外的点画上了自信满满的 9.4 × 2.1 cm 椭圆。把“朝向”调回 90°，误差就回到 2.1–15.6 cm。椭圆说的是一次测量有多精密，从来不是它有没有指对方向。' },
  ],
  quiz: [
    {
      ...
      answer: 1,
      explain: { en: 'Nothing about the radio changes with angle: the link budget is the same and the antennas do not move. Near ±90° a degree of azimuth changes almost no phase, so the inverse magnifies whatever noise the phase carried.', zh: '随角度变化的东西里没有一样属于射频——链路预算一样，天线也没动。在 ±90° 附近，方位角改变一度也几乎不改变相位，于是反解会把相位里的噪声放大。' },
    },
    {
    ...
    },
    {
      ...
      answer: 1,
      explain: { en: 'The range is right to 2 cm there and the clamp never fires: the phase genuinely is the mirror image’s, so no inverse could recover the truth. Facing is the cheap answer, and why the field exists at all.', zh: '那里的距离准到 2 cm，截断也根本没触发：那个相位真真切切就是镜像的相位，任何反解都还原不出真值。“朝向”是便宜的答案，锚点上之所以有这一栏正是为了它。' },
    },
  ],
}
