...
export const uwbAoa: Lesson = {
  ...
  picture: [
    { heading: '两根天线，一次到达', text: '正对着锚点扑过来的波前，同时到达它的两根接收天线。而从侧面来的波前，到较远那根天线要多走一段，于是晚到一丝丝。这一丝丝在载波上表现为周期上的一点滞后，这就叫相位差（phase difference）；而它背后的那个角度，就是这一帧的到达角（AoA）。' },
    ...
    { heading: '同样的噪声，越偏越贵', text: '两根天线相距半个波长，这是仍能让正前方每个方向各有自己相位的最大间距；这段范围就是视场（field of view）。靠近正前方时，方位角动一度，相位就动得明显；到了边缘，它几乎不动，因为相位跟着角度的正弦走，而正弦越往边上越平。噪声一样大，疑虑却大得多。' },
  ...
  numbers: [
    ...
    { kind: 'steps', heading: '一个方位角、一次定位，一步一步', items: [
      '胸牌用 Poll 开启一轮双边双向测距（DS-TWR）。锚点给 RMARKER（ranging marker）打戳，抽出时间戳噪声，抽出载波偏差残差，最后才去量角度。',
