...
export const rate: Lesson = {
  ...
  picture: [
    { heading: '错的两种方式', text: '选一级这条链路撑不住的，接收端听到的就是一团糊：帧发出去了，空口时间花掉了，什么也没到。选一级远低于链路能力的，帧倒是帧帧都到——可每一帧占住信道的时间是它本来需要的好几倍，而在这段时间里，屋里其他人谁也不能开口。对的那一级，是这条链路还撑得住的最高一级；而人走来走去、门开门关，这一级还会变。' },
    { heading: '信号扣下来的那个盖子', text: '这一幕里有两台站点（STA）在向同一个接入点（AP）满速上传：一台在它旁边的桌上，一台在另一头、隔着一堵砖墙的角落里。一条链路的信号真正撑得住的最高一级，就是它的上限；远端那台再怎么试也越不过去，因为那个角落的信号撑不住更密的 MCS。定下这个上限的，是距离和那堵墙。上限以下的一切，才是发送端自己一帧一帧做的决定。' },
    { kind: 'watch', jump: 0, heading: '去看一眼', text: '载入仿真，跳到第一个数据帧，然后顺着远端站点那条泳道往后看几秒：它的方块长度一直在变，而近端站点的从头到尾一个样。屋里什么都没挪动，变的只是一台发送端的选择。' },
    { heading: '发送端唯一收到的消息', text: '发送端看不见对面那头的信号，它手里只有回来的那点东西。把一帧发出去这一次动作，就是一次尝试；它只有两种结局：要么 ACK 到了，要么 ACK 超时到了。就这一丁点消息——答了，没答——就是速率控制的全部输入。它分不清一帧是被弱信号打死的，还是被“邻居在同一个时隙开了口”打死的；站在它这个位置上，两者长得一模一样。' },
  ...
  numbers: [
    ...
    { kind: 'steps', heading: '这个回路，引擎是怎么跑的', items: [
      '每发一帧之前，发送端都重新算一次这条链路的上限：在当前带宽下、按 RSSI、含 3 dB 速率余量，能撑住的最高一级；而且绝不超过两端谈定的共同能力。',
