...
export const mlo: Lesson = {
  ...
  picture: [
    { heading: '从一扇门变成两扇门', text: 'Wi-Fi 7 让一台设备可以在同一时刻让两台电台都醒着，各待在一个频段上。每一台都有自己的信道，也自己在那条信道上排队，每一台就是一条链路；把两条链路一起跑起来，就是 MLO。哪一条链路都不比它所取代的那一台电台更快。这一对多出来的，是“去哪条队”的选择。' },
    { kind: 'watch', jump: 1, heading: '去看一眼', text: '载入仿真，跳到第一个 6 GHz 数据帧。笔记本在时间轴上有两条泳道，第二条标着 ·6G，两条上同时都在落方块。再看它下面那台邻居：一条泳道，一个频段，而这个频段是满的。' },
    { heading: '两条链路共享的是什么', text: '它们不共享信道，不共享发送机会，也不共享那个倒数：每条链路自己听、自己等、自己重传，和一台只有单电台的设备完全一样。真正放在它们之上的，是一堆等着发的帧，归整台设备所有——也就是 MLD。哪条链路先抢到空口，就先从这堆帧里领走下一批。' },
    ...
    { heading: '它没有买到什么', text: '第二条链路并不会凭空造出空口时间。它只是让自己的主人够得着一个邻居还没进去的频段——而且只在邻居还没进去的这段时间里有效。把同样的两台电台也给邻居装上，那扇安静的门就被填满了：两台设备都把活儿摊到两个频段上，于是每一台拿到的，都比当初那台独苗 MLO 设备拿到的少。' },
  ...
  numbers: [
    ...
    { heading: '不是更快的那台电台，是更空的那个频段', text: '两条链路跑的是同一级：MCS 13，172.1 Mb/s。第二台电台并不更快，它只是那个没有别人的频段。笔记本每五帧里有四帧从那边走，带走的空口时间差不多是留下那些帧的五倍。' },
    ...
    { kind: 'steps', heading: '一帧是怎么拿到链路的，一步一步', items: [
      '这一帧只入队一次，入在整台设备这一层——也就是 MLD。每台设备只有一套四条接入类别队列，两条链路的电台拿到的是同一套。到达记录（ARRIVAL）与入队记录（ENQUEUE）写的都是这台设备的第一条泳道，也就是 5 GHz 那条，不管最后是哪条链路把它送出去。',
      '接着设备会把另一条链路的电台叫醒，于是两条链路都知道有活儿了，也都开始各自倒数。到这一步为止，还没有谁挑过链路。',
      '每条链路都在自己的信道上竞争，用自己的载波侦听、自己的倒数、自己的竞争窗口，和一台只有单电台的设备一模一样。频段被占住的那条链路，在别人说话期间就停住不数（不再出 BACKOFF_DEC），等对方停了再接着数。',
      ...
      '序号取自“每个接收方、每个接入类别”一个的计数器，它跟着共享队列走，而不是跟着哪台电台走，所以两条链路绝不会把同一个序号发给两个不同的帧。',
    ...
    { kind: 'table', heading: '一帧走完这几步：MSDU 66，6 GHz 上的第一批', head: [
      ...
      ['6.0176 ms', 'TX_START · ap#6g', '接入点的 BlockAck，32 字节，一次性回答这 20 帧。'],
