...
export const txop: Lesson = {
  ...
  picture: [
    { heading: '赢一次，占住发言权', text: '在那么多等待和倒数之后，赢家买到的东西其实比“一次交互”值钱得多：在一小段时间里，它是唯一被允许说话的。其他人开口前依然必须先听到静默，可静默根本不出现——赢家只隔着交互内部那段短短的停顿就又上了空口，而这段停顿比任何竞争者要等的都短。这段“归我所有”的时间，就是 TXOP；而赢一次所带出去的那几帧，就是它的突发。' },
    { kind: 'watch', jump: 0, heading: '看一串突发', text: '载入仿真，跳到第一串突发。接入点发给一台电视，收下它的回答，转眼间就已经在发给另一台了——中间没有必等的静默，也没有任何倒数。' },
    { heading: '短租有个上限', text: '一段没有终点的短租等于直接接管，所以每一次获胜都配着一只钟，这只钟就是 TXOP 上限。持有者发出去的每样东西、收到的每个回答，都必须装进这只钟里；一旦剩下的时间不够做下一次交互，持有者就停手，回去和别人一样等待、倒数。这个上限是按类别设的，所以承载对话的那些类别有自己的尺码。' },
    { heading: '多数时候拦住你的不是上限', text: '实际上，持有者很少真的顶到自己的上限。它停下来，是因为发往那个邻居的队列已经空了——手里攒着两帧就赢下空口的站点，发完两帧就把空口还回去。只有当发送方手里攒的东西远远超过这段短租装得下的量时，上限才真正起作用，而这正是当初写下它的那种情形。' },
