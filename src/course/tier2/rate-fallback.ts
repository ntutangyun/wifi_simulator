...
export const rateFallback: Lesson = {
  ...
  picture: [
    ...
    { heading: '失败从哪里来', text: '这里几乎每一帧没等到答复，起点都是一次“撞车”：两个退避计数器在同一个时隙清零，两台站点同时开口。接下来发生的是捕获效应——在接入点（AP）那里，近端站点的信号强得多，于是它的帧被解出来并得到答复，死掉的只有远端那一帧。这条时间轴上一个碰撞标记也画不出来，因为那边没有任何一次接收失败过。' },
    { heading: '账单落在邻居头上', text: '跑在更低一级的站点，为同样的内容要占住信道更久；而信道一忙，其他人的退避计数器就原地冻结。所以一次跌落的代价不只由跌下去的那台站点承担：每一个邻居，为自己早就赢到的那次机会，都要多等。这正是速率异常又一次出现——只不过这一次，它是发送端能自己爬出来的一种状态，而不是距离带来的固定事实。' },
  ],
  numbers: [
    ...
    { heading: '这些跌落花掉了多少空口时间', text: '跑在上限之下的帧，只占远端站点发出帧数的 20.2%，却占了它空口时间的 29.7%：530.3 ms——而同样这些帧若跑在上限上，只要 318.1 ms。' },
  ...
  observe: [
    ...
    '这条时间轴上一个碰撞标记也没有，而近端站点三秒里一帧未丢：4,010 帧，一次 ACK 超时也没有。捕获效应把每一次“撞车”都判给了它。',
  ],
  tryThis: [
