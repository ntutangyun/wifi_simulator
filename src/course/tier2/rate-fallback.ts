...
export const rateFallback: Lesson = {
  ...
  why: {
    ...
    zh: '一台只知道“答了还是没答”的发送端，仍然得决定什么时候放弃一级、什么时候去试更高的一级。教科书通常从这样一条规则讲起：掉得快，几帧没答就往下掉；爬得慢，要连着好一长串成功才升回去。这种不对称是故意的，而且很贵：一台被坏运气打下去的终端，要用很长一段时间慢慢地发，而在这段时间里，屋里其他人每一次机会都得多等。',
  },
  outcomes: [
    ...
    { en: 'measure what one station’s stretch below its ceiling costs its neighbours', zh: '量出一台终端跑在上限之下的那段时间，让邻居付出了多少代价' },
  ],
  needs: ['rate', 'anomaly'],
  terms: [
    { term: 'ARF', plain: {
  ...
  picture: [
    ...
    { kind: 'watch', jump: 1, heading: { en: 'Go and look', zh: '去看一眼' }, text: {
      ...
      zh: '载入仿真，跳到第一帧没等到答复的地方，然后顺着远端终端往后看：找到连着的第二次失败，看它之后的那个方块明显变长；再数一数，要过多少帧它才重新变短。',
    } },
    { heading: { en: 'Where the failures come from', zh: '失败从哪里来' }, text: {
      en: 'Almost every failed attempt here begins as a tie. Two backoff counters reach zero in the same slot and both stations start talking at once. What happens next is capture: at the router the near station’s signal is far the stronger, so its frame is decoded and answered and only the far station’s dies. Nothing on this timeline is drawn as a collision, because no reception at the router ever failed.',
      zh: '这里几乎每一帧没等到答复，起点都是一次“撞车”：两个退避计数器在同一个时隙清零，两台终端同时开口。接下来发生的是捕获效应——在路由器处，近端终端的信号强得多，于是它的帧被解出来并得到答复，死掉的只有远端那一帧。这条时间轴上一个碰撞标记也画不出来，因为路由器那边没有任何一次接收失败过。',
    } },
    { heading: { en: 'The bill lands on the neighbours', zh: '账单落在邻居头上' }, text: {
      ...
      zh: '跑在更低一级的终端，为同样的内容要占住信道更久；而信道一忙，其他人的退避计数器就原地冻结。所以一次跌落的代价不只由跌下去的那台终端承担：每一个邻居，为自己早就赢到的那次机会，都要多等。这正是速率异常又一次出现——只不过这一次，它是发送端能自己爬出来的一种状态，而不是距离带来的固定事实。',
    } },
  ],
  numbers: [
    { kind: 'table', heading: {
      en: 'Three seconds of the far station: attempts, and how many of them failed',
      zh: '远端终端的三秒：尝试次数，以及其中失败了多少次',
    }, head: [
    ...
    { heading: { en: 'What the excursions cost in air', zh: '这些跌落花掉了多少空口时间' }, text: {
      ...
      zh: '跑在上限之下的帧，只占远端终端发出帧数的 20.2%，却占了它空口时间的 29.7%：530.3 ms——而同样这些帧若跑在上限上，只要 318.1 ms。',
    } },
    { kind: 'table', heading: {
      en: 'How long the near station’s backoff is held behind one far-station frame',
      zh: '近端终端的退避，被一个远端终端的帧冻结多久',
    }, head: [
  ...
  deeper: [
    { heading: { en: 'The death spiral that is not there', zh: '并不存在的那个死亡螺旋' }, text: {
      ...
      zh: '很容易把这个回路反过来接：帧发得越慢，在空口上待得越久，想必更容易被撞上，于是丢得更多、更慢。可一量就会发现，这个螺旋并不存在。每次尝试的失败率在三个级别上几乎不动——上限上 11.1%，MCS 1 上 11.8%，MCS 0 上 11.1%——而最长的那些帧并不是处境最差的。原因是退避那一课的冻结规则：退避计数器在别人发帧期间并不倒数。介质一转忙它就停住，空下来再从同一个数值继续——这段仿真里近端终端的 2,666 次冻结，无一例外都是以进去时的那个值出来的。所以更长的帧并不会给对方的计数器多出时间走到零，而是根本不给。决定一次失败的，是两个计数器会不会在同一个时隙清零，而这在两帧发出之前就已定下。（有一个例外恰好印证了这条规则，它来自引擎而不是标准：两帧真的同时起跑时，近端终端根本没检测到远端的前导码——那一瞬间它自己的发射机正忙着。于是它笔直地数过远端那一帧，甚至在它中间发出第二帧；但那一帧在同时起跑的那一刻就已经没了，所以并没有多丢什么。另外，MCS 0 上那九十次尝试也太少，读不出什么趋势；要紧的是，螺旋所需要的那个方向，数据里根本没有。）',
    } },
    { heading: { en: 'How a failure shows on the timeline', zh: '失败在时间轴上长什么样' }, text: {
      ...
      zh: '这三秒里远端终端有 337 次尝试失败，而它们在屏幕上并不长一个样。其中 254 次以一条 ACK 超时记录收场：该来的答复没来，站点等不下去了。另外 83 次落在别人某一帧的末尾——答复该到的时候，这台站点正在接收那一帧；它把那一帧收完，发现里面没有属于自己的答复，于是重传。两种情况都恰好调用一次 `onFailure`，所以上面那条规则只有在两种都算上时，才能一次不差地复现整段仿真。',
    } },
  ...
  sources: [
    ...
    { en: 'The freeze behind a busy medium is §10.23.2.4: the backoff counter is suspended while the medium is determined busy and resumes with the value it held. The 45 µs the far station waits before calling an attempt failed is the simulator’s ACK timeout, a model choice sized from SIFS plus the ACK.',
      zh: '“介质忙时冻结”见 §10.23.2.4：介质被判为忙时退避计数器挂起，恢复时沿用它原来的值。远端终端判定一次尝试失败前等的那 45 µs 是本仿真器的 ACK 超时，属于模型取值，按 SIFS 加上一个 ACK 的长度定出来。' },
  ...
  observe: [
    { en: 'Six times in this run the far station reaches the bottom rung. Ten frames is the fastest climb the rule allows and three of the six manage it; the others last 13, 19 and 28 frames, because a fresh loss reset the count part-way up.', zh: '这段仿真里远端终端六次跌到最底一级。十帧是规则允许的最快爬升，六次里有三次做到了；其余三次分别持续 13、19、28 帧，因为爬到一半又丢了一帧，把计数清零了。' },
    { en: 'Every change of length is one rung: the far station never skips a step, going down or coming back up.', zh: '每一次长度变化都只差一级：远端终端无论往下掉还是爬回来，从不跳级。' },
    { en: 'No collision marker appears anywhere on this timeline, and the near station loses nothing in three seconds: 4,010 frames, not one ACK timeout. Capture hands it every tie.', zh: '这条时间轴上一个碰撞标记也没有，而近端终端三秒里一帧未丢：4,010 帧，一次 ACK 超时也没有。捕获效应把每一次“撞车”都判给了它。' },
  ],
  tryThis: [
    { en: 'Open in the editor and add a third saturated uploader beside the near station. The far station now spends 26.3% of its frames on the bottom rung instead of 3.0% — nearly ten times as many, bought by one extra talker.', zh: '点“在编辑器中打开”，在近端终端旁边再加一台饱和上传的终端。远端终端落在最底一级的帧，从占 3.0% 变成占 26.3%——将近十倍，代价只是多了一个说话的人。' },
    { en: 'Move that third uploader somewhere else and it gets worse, not better: three other spots tried here all land between 36% and 43%. Crowding, not geometry, is what floors the rung.', zh: '再把这台新终端挪到别处，结果只会更糟而不会更好：这里试过的另外三个位置都落在 36% 到 43% 之间。把级别压到底的是“人多”，不是几何位置。' },
  ],
  quiz: [
    {
      q: { en: 'The far station’s signal has not changed, but its rung dropped two steps. What happened?', zh: '远端终端的信号没有变化，但它的级别降了两级。发生了什么？' },
      options: [
        ...
        { en: 'The router asked it for a lower rate to save power', zh: '路由器为了省电，要求它降低速率' },
      ],
      answer: 1,
    ...
    {
      q: { en: 'The far station’s bottom-rung frames last almost twice as long as its MCS 1 frames. Which of the two fails more often, per attempt?', zh: '远端终端在最底一级的帧，几乎是它 MCS 1 帧的两倍长。这两者里，哪一种每次尝试更容易失败？' },
      options: [
        ...
        { en: 'Exactly the same for both — failures depend only on how many stations there are', zh: '两者完全一样——失败只取决于有多少台终端' },
      ],
      answer: 1,
