...
export const edca: Lesson = {
  ...
  numbers: [
    ...
    { kind: 'formula', heading: { en: 'Where those waits come from', zh: '这些等待是怎么来的' }, text: {
      ...
      en: 'The old one-size-fits-all wait is the same sum with AIFSN fixed at two.',
    ...
    { kind: 'steps', heading: { en: 'How a frame’s wait is decided, step by step', zh: '一帧的等待是怎么定下来的，一步一步' }, items: [
      { en: 'On its way in, the frame is filed under one access category and joins that queue. Each queue keeps its own counter and window; the three constants above are all that tells the four apart.',
      ...
      { en: 'At the end of the AIFS the queue draws a whole number between zero and its current window, which starts at CWmin (the smallest window). For these four queues the end of the AIFS is itself a slot boundary, so the counter steps down by one right there — one boundary earlier than the plain countdown of the backoff lesson, which counts the same slots.',
        zh: '这段 AIFS 走完时，这条队列在零和它当前窗口之间抽一个整数；窗口从 CWmin（最小窗口）起步。对这四条队列来说，AIFS 结束的那一刻本身就是一个时隙边界，所以计数器在那里就先减一——比“退避”那一课里的朴素倒数早一个边界，而数过的时隙一样多。' },
      { en: 'Every further idle slot of 9 µs takes one more off the counter; anything on the air freezes it until a fresh AIFS has passed. A counter already at zero transmits on the next boundary, 9 µs later.',
        zh: '此后每过一个 9 µs 的空闲时隙，计数器再减一；空中一旦有东西，它就原地冻住，等下一段 AIFS 走完再接着数。已经停在零的计数器，在下一个时隙边界上发送，也就是再过 9 µs。' },
      { en: 'If two queues of one radio stand at zero in the same slot, the higher transmits and the lower is treated as though its frame had been lost in the air: it counts a retry, doubles its window towards CWmax (the largest) and draws again. That is an internal collision, and it costs no airtime.',
    ...
    { kind: 'table', heading: { en: 'The caller’s first voice frame, run through the steps', zh: '通话站点的第一帧语音，照着步骤走一遍' }, head: [
      ...
      [{ en: 'the end of the AIFS is a slot boundary', zh: 'AIFS 的结束本身就是一个时隙边界' }, N('23.1156 ms · 2 → 1')],
      [{ en: 'one more idle slot, 9 µs', zh: '再过一个 9 µs 的空闲时隙' }, N('23.1246 ms · 1 → 0')],
      [{ en: 'a counter already at zero waits one more boundary', zh: '已经停在零的计数器，再等一个时隙边界' }, N('9 µs')],
    ...
    { heading: { en: 'The head start, measured', zh: '把抢跑量出来' }, text: {
      en: 'Top row against bottom row: 45 µs of silence, five slots in which the caller counts and the backup may not; and a draw of 2.2 slots against 8.2.',
    ...
    { heading: { en: 'Who ever collects the longer wait', zh: '谁摊上过那段更长的等待' }, text: {
      en: 'Only the uploader ever hears a frame it cannot decode, so only it waits the 103 µs of step 3. No station runs two classes, so step 6 never fires at all.',
      zh: '这个房间里只有上传站点碰到过解不出来的帧，所以第 3 步那段 103 µs 的等待也只有它摊上过。而没有哪台站点同时跑两类流量，于是第 6 步同样一次也没触发。',
    } },
  ],
  deeper: [
