...
export const edca: Lesson = {
  ...
  picture: [
    { heading: '一台电台，四间候车室', text: '每一帧进入电台的时候，都会按它装的东西被归入四类之一：一通电话、一部影片、普通流量，或者压根没人在等的东西。每一类有自己的队列，每条队列各跑各的倒数，就像房间里另一台独立的站点一样。哪条先数到零，电台就发哪条的帧。这就是 EDCA；而这四条队列里的每一条，标准里称为一个接入类别。' },
    { heading: '两个旋钮，没有裁判', text: '为了在它们之间分出胜负，并没有新增任何东西——没有调度器，也不用向谁请示。四条队列玩的还是整个房间都在玩的那套等待游戏，只是有两个数按类别分别设定。第一个是：这条队列必须先听到多久的“什么都没有”，它的倒数才被允许走动，这就是它的 AIFS。第二个是：它抽倒数值时的取值范围有多宽。语音等得更短，抽得更小。抽签，只不过是被做了手脚。' },
    { kind: 'watch', jump: 0, heading: '看通话是怎么挤进去的', text: '载入仿真，跳到通话站点的第一帧。旁边的上传站点从来没停过，可通话依然上了空口。把鼠标悬在通话站点的倒数色块上，再悬在备份站点的色块上，比一比这两个块上的数。' },
    { heading: '每一轮都要兑现的抢跑', text: '这两个旋钮给的优势并不是同一种。抽取范围更窄，只是把概率往一边拨了拨——后台队列照样可能抽到很小的数然后赢。更短的静默要硬得多：当后台队列还在熬它那段必等的安静时，语音队列已经在倒数了，而在这些时隙里，后台队列连开始的资格都没有。这是抢跑，而且每一轮都要兑现一次。' },
  ...
  numbers: [
    ...
    { kind: 'formula', heading: '这些等待是怎么来的', text: 'AIFS = SIFS + AIFSN × 时隙 = 16 + AIFSN × 9 µs，AIFSN 取 2、3 或 7', note: '过去那段“一刀切”的等待，就是这同一个式子把 AIFSN 钉死在 2：一个常数被改成了参数。' },
    { kind: 'steps', heading: '一帧的等待是怎么定下来的，一步一步', items: [
      ...
      '手里有帧、空中又安静时，这条队列必须先听到完整不断的那段 AIFS，计数器才被允许走动。如果本机上一次听到的是一帧解不出来的东西，这段等待就换成 EIFS − DIFS + AIFS——对上传站点那一类来说是 103 µs。',
      '这段 AIFS 走完时，这条队列在零和它当前窗口之间抽一个整数；窗口从 CWmin（最小窗口）起步。对这四条队列来说，AIFS 结束的那一刻本身就是一个时隙边界，所以计数器在那里就先减一——比“退避”那一课里的朴素倒数早一个边界，而数过的时隙一样多。',
      '此后每过一个 9 µs 的空闲时隙，计数器再减一；空中一旦有东西，它就原地冻住，等下一段 AIFS 走完再接着数。已经停在零的计数器，在下一个时隙边界上发送，也就是再过 9 µs。',
      '如果同一台电台的两条队列在同一个时隙同时停在零，高的那一类发送，低的那一类则完全按“自己的帧在空中丢了”来处理：记一次重传，窗口朝 CWmax（最大窗口）翻倍，重新抽数。这就是内部碰撞，而它一微秒空口时间也不花。',
    ] },
  ...
  deeper: [
    ...
    { heading: '窗口为什么是翻倍而不是加一', text: '每一类都有一个最小窗口和一个最大窗口，中间每失败一次，就沿着 2ⁿ − 1 这个序列往上走：3、7、15、31……语音故意只有两级，3 和 7——一通退避到几百个时隙的电话，其实已经失败了——而后台可以一路爬到 1023。' },
  ],
  sources: [
