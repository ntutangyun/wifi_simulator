...
export const ofdmaUl: Lesson = {
  ...
  module: 6,
  title: '触发帧——接入点指挥上行',
  why: '同时发给好几台设备，是一台电台自己就能定的事：它知道自己要发什么，想怎么切信道就怎么切。反过来同时从好几台设备那里收——也就是上行，从设备指回它们共同说话的那只盒子的方向——则是另一回事。它们得在同一个瞬间开始，落在互不重叠的片上，到达时强弱还得相近，可谁也听不见别人下一步打算干什么。这就得有人来指挥，而有资格指挥的只有接入点（AP）；它用来指挥的那个帧，就叫作触发帧。',
  outcomes: [
  ...
  picture: [
    { heading: '难的那个方向', text: '把信道切片，往下行方向做得通，因为每一片都是同一台电台填的。往上行方向，各片就要由房间不同角落里的不同设备来填。它们不共用一个时钟，彼此也听不太清，各自只知道自己的队列。靠它们自己，永远凑不齐同一个起跑瞬间，于是上行只能一台一台地来——除非有人把该做的事原原本本地告诉每一台。' },
    ...
    { kind: 'steps', heading: '触发帧把哪些事定死了', items: [
      ...
      '答多长：每个回答都填充到同样的长度，于是它们一起结束，一个确认就能把整组收掉。',
      '答多响：每台设备都被告知要怎样修正自己的功率，好让近的和远的到达接入点时强弱相近——这条指令本仿真器只写进帧里，并不执行，它的上传设备仍按自己一贯的功率作答。',
      '什么时候答：触发帧结束后隔一小段，并且在时间和频率上都对齐到接入点。',
    ] },
    { heading: '一个什么都不做主的回答', text: '回来的这一帧叫作 TB PPDU：基于触发，是唯一一种“只因为被要求了才可以发”的发送。关于它的一切都是接入点定的——用哪一片、用哪档调制、发多长、什么时候开始。设备只负责把字节装进去，别的一概不管。诀窍就在这里：要商量出“同时”几乎不可能，要指定“同时”却很容易。' },
    { heading: '设备保住了什么', text: '在一次被触发的回合里，没有人再去数退避：这一轮的机会早就分配好了。但“听”并没有停——设备在回答前仍然会看一眼空口，如果房间已经被别的网络占着，它就闭嘴。而在两次回合之间，根本没有谁在指挥：还是那两台上传设备，像以前一样排队、一样竞争，因为接入点只在它想把上行组织一下的时候才发触发帧。' },
  ],
  numbers: [
    ...
    { kind: 'steps', heading: '一次触发回合是怎么安排出来的，一步一步', items: [
      ...
      '等它赢下这一轮、而自己手上又没有要往下发的东西时，就列出积压非空、并且和它协商过 OFDMA 的设备。名单上不到两台，这件事就先作罢。',
      '名单上只留前四台，把子载波切成同样多的等分资源单元，再给整个回合定死一套格式：每台可以用哪一级，以及每个回答占多宽——取受邀各台协商过的最窄的那个宽度。',
      '接着为每台设备把积压折算成空口时间，上限是一个 2 ms 的回答装得下的量，再取其中最长的那个。然后所有设备都被指定这同一个长度，于是几个回答一起结束，一个确认就能把整个回合收掉。',
      ...
      '几个回答一起结束后再隔 16 µs，接入点发一个多站点 BlockAck 把回合收掉：32 字节，此后每多一台设备再加 8 字节。要是触发帧结束后 45 µs 仍没有任何回答开始，这一回合就什么也没换回来，空口随即被收回。',
    ] },
  ...
  observe: [
    ...
    '一帧就把两个回答都确认了：两个回答一起结束后隔一小段，接入点发出一个 36 µs 的多站点 BlockAck。两次回合之间指挥就不见了——有 54 次发送走的是普通竞争的老路，而被触发的只有 28 次。',
  ],
  tryThis: [
