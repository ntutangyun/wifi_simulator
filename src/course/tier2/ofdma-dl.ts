...
export const ofdmaDl: Lesson = {
  ...
  why: '一台正在放片子的电视，每次并不需要占多少空口，但它需要一个“轮次”：自己的前导、自己的回执、之前还要自己等一轮。同一个房间里放三台，接入点（AP）整晚花在这些包装上的工夫，就多过花在片子上。这一课要看的是：接入点如何不再一轮只服务一台设备，而是在一次发送里同时服务好几台。',
  outcomes: [
    '说出接入点在一次发送里服务多台设备时，被切分的到底是什么',
    '在时间轴上读出一个多用户（MU）PPDU：里面有谁、花了多久、怎么被确认的',
  ...
  picture: [
    { heading: '一次发送，切成几片', text: '信道本来就是由一根根很窄的子载波拼起来的，而且谁也没规定它们必须都为同一段对话服务。换个分法：把它们成块地分出去——这一块给那部手机，那一块给电视——于是一次发送就能同时装着发给好几台设备的数据。每台设备只读属于自己的那一块，其余的一概不管。这就是 OFDMA，而其中的一块，就是一个资源单元。' },
    { kind: 'watch', jump: 0, heading: '去看一眼', text: '载入仿真，跳到第一次多用户发送。把鼠标停在那个宽蓝块上：一帧里点着两台电视的名字。它结束之后隔一小段，两个回执在同一个瞬间一起开始，并排落在不同的泳道上。' },
    { heading: '谁来决定怎么切', text: '切片不是谈出来的。赢下这一轮之后，由接入点一个人决定：它看一眼此刻有哪些设备的东西正等着发，最多挑四台，给每台分一个同样大的资源单元（RU），然后发出去。没被挑中的设备不是被拒绝了——只是它们的队列里此刻根本没有东西可装。发出去的这一帧，记录里叫作 MU PPDU。' },
    { heading: '切片买来了什么', text: '每一次发送，开头都有一段前导让接收端锁定，结尾都要收一个回执。两帧一前一后地发，就要付两段前导、两个回执；同样这两帧装进一次发送，只付一段前导，而两个回执是一起回来的，不必排队。每一片都更窄，所以同一帧要用比单独发时更多的符号——但包装只付一次。' },
    { heading: '切片买不来的东西', text: '切片不会让任何一条链路变快。电视要多少片子就是多少，它收到的帧和本来会收到的一模一样；省下来的是空口，而省下的这点空口属于房间里其他想说话的人。接入点也没法把队列是空的设备凑成一组：必须有两台设备在同一瞬间都有东西等着发，而在一屋子平稳的视频流里，这样的瞬间并不多。' },
  ],
  numbers: [
    { kind: 'table', heading: '同样两个视频帧发给 Wi-Fi 6 电视，两种发法', head: [
      ...
      ['一帧一帧发', '44 µs × 2', '6 + 6', '125.6 µs × 2',
        '两个 ACK 帧，各 28 µs，一前一后'],
      ['一个 MU PPDU', '48 µs', '12', '211.2 µs',
        '两个 BlockAck 帧，各 32 µs，同一瞬间'],
    ] },
    { kind: 'formula', heading: '为什么是十二个符号而不是六个', text: '符号数 = ⌈(16 + 8·字节数 + 6) ÷ (每符号比特数 × RU 占比)⌉', note: '子载波少一半，符号数就翻一倍。44 µs 是一个 Wi-Fi 6 帧的前导——Wi-Fi 7 帧的前导则是带宽那一课里的 48——而“这一发里有谁”的分配表又添了四微秒。即便如此，两帧一起走只要 211.2 µs，而一前一后地发要 251.2 µs。' },
    ...
    { heading: '同样的片子，更少的空口', text: '每台电视收到的帧和原来一模一样，所以屏幕上什么也不会变。变的是空口。每凑成一对，就省下 40 µs 的前导，又因为回执变大而还回去 8 µs；这段仿真里的 45 对，合起来是 1.44 ms，还给了房间里其他所有人。' },
    { kind: 'steps', heading: '一次多用户发送是怎么攒出来的，一步一步', items: [
      ...
      '整次发送的长度，取最长的那个成员所需的长度：44 µs 前导，再加 4 µs 装“这一发里有谁”的分配表，然后每个符号 13.6 µs。前导和分配表整组只付一次——省下来的就是这一笔。',
      '结束后隔 16 µs，每个成员各在自己的资源单元上回一个 32 µs 的 BlockAck，全都落在同一瞬间。只要有一个成员答了，这次交互就算成，其余成员那一份重新排队。',
    ] },
  ...
  deeper: [
    ...
    { heading: '多出来的那四微秒去哪儿了', text: '单用户发送的前导是 44 µs，多用户是 48 µs。差出来的这一段装的是每用户分配表：哪个资源单元属于哪台设备、每一片用的是哪一档调制。没有它，接收端就不知道该去读信道的哪一段，所以这张表是整个想法的价钱——而且是一口价，成员越多越划算，道理就在这里。' },
  ],
  sources: [
    ...
    '44 µs 的前导、多用户信令多出的 4 µs 以及 13.6 µs 的符号，是本仿真器为这类 PPDU 取的单一代表值，并非逐字段相加；它们代入的空口时间公式是 §17.4.3 的 TXTIME。',
