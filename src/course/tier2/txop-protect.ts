...
export const txopProtect: Lesson = {
  ...
  picture: [
    ...
    { heading: '把时间还回去', text: '覆盖整串的预告，通常比这串帧最后真正需要的更长：队列（queue）空了，或者下一帧已经塞不下。于是持有者用几个字节把剩下的还回去，意思是“我提前结束了”——这就是 CF-End（contention-free end）。听见的人当场把预约作废。可远处那个房间听不见持有者——于是接入点替它把这个 CF-End 重复一遍。这和那个回答是同一个手法，只是方向反了过来。' },
    { kind: 'list', heading: '预告的三种说法', items: [
      '单次保护：除了手上这一帧，什么也不多说。远端站点一次只被告知一次交互，然后就径直数进了这一串剩下的部分。',
      '边界保护（boundary protection）：开头一问一答，把这一串预告到末尾为止。本课载入的就是这种。',
      '多重保护（multiple protection）：开头同样是一问一答，而且每个数据帧（data frame）都携带剩余时间，于是错过那个回答的站点，也能从数据帧里把预约接上。',
    ] },
    { heading: '没人替你回答的时候', text: '有时候持有者只想要预告，不想要那次问答。它可以把“允许发送”（clear to send, CTS）那一帧直接发给自己的地址：这就是 CTS-to-self（CTS to self），本来两帧，现在一帧。代价少了一半——而它传得多远，和持有者自己的嗓门一模一样，也就是说，传不进远处那个房间。' },
  ],
  numbers: [
    ...
    { heading: '同样的账单，瞄得更准', text: '保护本身并不是花钱的地方：两种策略花在预告帧上的空口时间（airtime）都在二十分之一上下，边界还略少一点，因为一次开场就管住了一整串。变的是覆盖范围：隐藏站点装上的预约平均是 2.45 ms，而不是 1.05 ms，于是它把整串等完；整个房间送达的帧数接近三倍，而每送达一帧所花的空口时间只有原来的三分之一。' },
    ...
    { kind: 'steps', heading: '一句提问怎么管住整串', items: [
      ...
      '它先发出那句简短的提问：一帧 20 字节的请求发送（request to send, RTS），用全屋都解得开的速率发出。它的 Duration 字段写的是整轮减去提问自身的 28 µs：2 500 µs。',
      ...
      '等到队列空了，或者下一次交互再也装不下了，持有者就把剩下的还回去：只要剩下的比“一段停顿 + 一个 CF-End + 一个时隙（slot time）”还长，它就发出 CF-End，接入点隔一段停顿再重复一遍；两份里听见任一份的人，当场把预约作废。',
    ] },
