...
| Backscatter DL PPDU: two excitations | SFD PM-15, PM-38, PM-63, PM-65 note, PM-72…PM-90 | legacy preamble + U-SIG (32 µs) → WUP-Excitation (≥ 1 ms, `wupMs`) → AMP-Sync (16 µs) → AMP-Data (Manchester OOK, no AMP-SIG for mono-static) → BST-Excitation → 6 µs signal extension; transmitted at `chargeDbm` through AMP-Data and `bsDbm` during BST-Excitation |
| Backscatter UL PPDU | SFD PM-24, PM-35, PM-17, PM-20 | 24-chip sync + Manchester OOK data, one chip per Manchester half-bit (2 µs / 0.5 µs at 250 kb/s / 1 Mb/s); reply starts T1 = 16 µs after AMP-Data ends |
| BST-Excitation timing, T1…T4 | SFD PM-74, PM-75, PM-86…PM-88 | at least 1.2·T1 + 1.1·T4 for an immediate reply, 1.1·T3 + 1 µs + 1.1·T4 for a delayed one (Write, T3 = 2 ms) |
